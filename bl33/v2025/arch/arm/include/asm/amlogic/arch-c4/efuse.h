...
typedef struct __attribute__((__packed__)) efuse_cali {
	...
	unsigned long long tsensor_data:15;
	unsigned long long tsensor_flag:1;
	...
	unsigned long long aoosc_data:3;
	unsigned long long aoosc_flag:1;
...
uint32_t efuse_mrk_get_checknum(char *name, uint32_t longmrk, uint32_t *checksum);

...
/* spinand & spiflash customer partition */
typedef struct partition_info {
	const char part_name[16];
	/* bit0-bit23: part size (unit: 4k) */
	/* bit31-bit24: part flags */
	uint32_t part_size;
} __attribute__ ((packed)) partition_info_t;

#define MAX_PART_COUNT		(10)
typedef struct partition_info_table {
	partition_info_t part_table[MAX_PART_COUNT];
} __attribute__ ((packed)) partition_info_table_t;
...
typedef struct common_storage_parameter {
	...
	/* bl3 ree size */
	uint32_t bl3_ree_container_size;
	/* bl3 tee size*/
	uint32_t bl3_tee_container_size;
	/*rtos run finish*/
	uint32_t rtos_finish;
...
typedef struct dev_param_hdr {
	...
	char			part_set_magic[6];
	unsigned short		part_set_length;

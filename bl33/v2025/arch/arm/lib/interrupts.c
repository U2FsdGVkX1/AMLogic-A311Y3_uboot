...
static void dump_backtrace(unsigned long fp)
{
	struct stack_frame {
		unsigned long fp;
		unsigned long lr;
	...
	offset = gd->reloc_off;
	printf("ARM 32-bit Backtrace: \n");

	if (fp & 0x3) {
		printf("  fp fp pointer (not 4-byte aligned)\n");
		return;
	}

	frame = (struct stack_frame *)(fp - 4);
	int level = 0;

	while (1) {
		if ((long)frame < (long)CONFIG_SYS_SDRAM_BASE ||
			(long)frame > (long)(CONFIG_SYS_SDRAM_BASE + gd->ram_size)) {
			printf("  [Stack out of SDRAM range]\n");
			break;
		}

		unsigned long where = frame->lr;

		debug("  #%d: LR = 0x%08lx ", level++, frame->lr);
		print_symbol(where, offset);

		if (frame->fp == (ulong)frame) {
			break;
		}

		frame = (struct stack_frame *)(frame->fp - 4);

		if (level > 32) {
			printf("  [Max backtrace level reached]\n");
			break;
		}
	}
}
...
{
#define GET_CURRENT_FP() ({ \
		unsigned long __fp; \
		__asm__ __volatile__("mov %0, fp" : "=r"(__fp)); \
		__fp; \
})
	unsigned long fp = GET_CURRENT_FP();
	dump_backtrace(fp);
}

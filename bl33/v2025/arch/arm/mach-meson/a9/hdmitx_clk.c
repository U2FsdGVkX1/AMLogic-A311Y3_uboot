...
static void set_hpll_sspll_a9(enum hdmi_vic vic);
/* local frac_rate flag */
static u32 frac_rate;

void disable_hdmitx_a9_plls(struct hdmitx_dev *hdev)
{
	hd21_write_reg(ANACTRL_HDMIPLL_CTRL0_A9, 0);
	hd21_write_reg(ANACTRL_HDMIPLL_CTRL3_A9, 0);
}

/* htx pll VCO output: (3G, 6G), for tmds */
static void set_a9_htxpll_clk_other(const u32 clk, const bool frl_en)
{
	u32 quotient;
	u32 remainder;
	...
	/*
	 * ctrl0 bit 14 set 0: tx_pll_load_en
	 */
	hd21_write_reg(ANACTRL_HDMIPLL_CTRL0_A9, 0x00013000 | (quotient << 0));
	hd21_write_reg(ANACTRL_HDMIPLL_CTRL1_A9, 0x9040137d);
	hd21_write_reg(ANACTRL_HDMIPLL_CTRL2_A9, 0x04000000);
	hd21_write_reg(ANACTRL_HDMIPLL_CTRL3_A9, 0x01160000 | remainder);
	hd21_set_reg_bits(ANACTRL_HDMIPLL_CTRL0_A9, 1, 28, 1);
	usleep_range(10, 20);
	hd21_set_reg_bits(ANACTRL_HDMIPLL_CTRL0_A9, 1, 18, 1);
	usleep_range(10, 20);
	hd21_set_reg_bits(ANACTRL_HDMIPLL_CTRL0_A9, 1, 30, 1);
	hd21_set_reg_bits(ANACTRL_HDMIPLL_CTRL0_A9, 0, 18, 1);
	usleep_range(80, 90);
	hd21_set_reg_bits(ANACTRL_HDMIPLL_CTRL0_A9, 1, 29, 1);
	usleep_range(80, 90);
	WAIT_FOR_PLL_LOCKED(ANACTRL_HDMIPLL_CTRL0_A9);

}

void set21_a9_htxpll_clk_out(const u32 clk, u32 div)
{
	u32 pll_od0 = 0;
	u32 pll_od00 = 0;
	u32 pll_od01 = 0;
	...
	set_a9_htxpll_clk_other(clk, hdev->para->frl_rate ? 1 : 0);
	...
	hd21_set_reg_bits(ANACTRL_HDMIPLL_CTRL3_A9, 1, 19, 1);
	pr_info("pll_od0 = %d, pll_od2 = %d, pll_od1 = %d\n",
		pll_od0, pll_od2, pll_od1);
	//tx_spll_lock_by_pass_alo
	if (hdev->clk_analog_path)
		hd21_set_reg_bits(ANACTRL_HDMIPLL_CTRL3_A9, pll_od1, 22, 2);
	hd21_set_reg_bits(ANACTRL_HDMIPLL_CTRL3_A9, pll_od2, 24, 6);
	hd21_set_reg_bits(ANACTRL_HDMIPLL_CTRL0_A9, pll_od0, 20, 6);
}
...
void hdmitx21_set_audioclk(bool en)
{
	 ...
	 */
	hd21_set_reg_bits(CLKCTRL_HTX_CLK_CTRL1_A9, 3, 9, 2);// FIXPLL/5
	hd21_set_reg_bits(CLKCTRL_HTX_CLK_CTRL1_A9, 1, 0, 8);//div2
	// [    8] clk_en for cts_hdmitx_aud_clk
	hd21_set_reg_bits(CLKCTRL_HTX_CLK_CTRL1_A9, en, 8, 1);
}

void hdmitx21_set_default_clk(void)
{
	/* Enable clk81_hdmitx_pclk */
	hd21_set_reg_bits(CLKCTRL_SYS_CLK_EN0_REG2_A9, 1, 4, 1);

	hd21_write_reg(CLKCTRL_HDMI_CLK_CTRL_A9, 0x303);
	hd21_write_reg(CLKCTRL_HTX_CLK_CTRL0_A9, 0x7010701);

	// Bring HDMITX MEM output of power down
	hd21_set_reg_bits(PWRCTRL_MEM_PD11_A9, 0, 8, 8);
	// Bring out of reset
...
void hdmitx21_set_hdcp_pclk(struct hdmitx_dev *hdev)
{
	...
	hd21_set_reg_bits(CLKCTRL_SYS_CLK_EN0_REG2_A9, 1, 3, 1);
}
...
static void set_tmds_vid_clk_div(u32 div_val)
{
	...
	div_reg = CLKCTRL_VID_PLL_CLK_DIV_A9;
...
static void set_hdmitx_a9_htx_pll(struct hdmitx_dev *hdev)
{
	...
	set21_a9_htxpll_clk_out(htx_vco, div);
}
...
static void set_hpll_sspll_a9(enum hdmi_vic vic)
{
	...
	case HDMI_20_1920x1080i50_16x9:
		...
		hd21_set_reg_bits(ANACTRL_HDMIPLL_CTRL3_A9, 3, 20, 2);//enable ssc
		hd21_set_reg_bits(ANACTRL_HDMIPLL_CTRL2_A9, 2, 16, 4);//set ssc 1000 ppm
		hd21_set_reg_bits(ANACTRL_HDMIPLL_CTRL2_A9, 1, 4, 1);//SS strength multiplier
		break;
	default:
		break;
	}

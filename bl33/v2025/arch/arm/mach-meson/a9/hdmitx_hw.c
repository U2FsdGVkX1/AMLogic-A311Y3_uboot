...
int hdmitx_get_hpd_state(void)
{
	...
	st = !!(hd21_read_reg(PADCTRL_GPIOH_I) & (1 << 6));
	return st;
}
...
{
	/* hdmitx_sda */
	hd21_set_reg_bits(PADCTRL_PIN_MUX_REGB, 1, 16, 4);
	/* hdmitx_scl */
	hd21_set_reg_bits(PADCTRL_PIN_MUX_REGB, 1, 20, 4);
}
...
static u32 get_enc_paddr(unsigned int addr)
{
	...
	//printf("hdev->enc_idx=%d, idx=%d, addr = %x, offset = %x\n", hdev->enc_idx, idx, addr, offset);
	if (hdev->enc_idx == 2 && idx == VPUCTRL_REG_IDX) {
		if (offset >= 0x1b00 && offset < 0x1d00)
			return addr + (0x800 << 2);
	} else if (hdev->enc_idx == 1 && idx == VPUCTRL_REG_IDX) {
		if (offset >= 0x1b00 && offset < 0x1d00) {
			return addr + (0x600 << 2);
		}
	}
...
void hdmitx_set_phypara(enum hdmi_phy_para mode)
{
	...
	hd21_write_reg(ANACTRL_HDMIPHY_CTRL0_A9, 0x0);
	/* P_ANACTRL_HDMIPHY_CTRL1	bit[1]: enable clock	bit[0]: soft reset */
	hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL1_A9, 0x0390, 16, 16);
	hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL1_A9, 0x0, 0, 4);

	/* hdmitx phy reset twice */
	hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL1_A9, 0xf, 0, 4);
	mdelay(2);
	hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL1_A9, 0xe, 0, 4);
	mdelay(2);
	hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL1_A9, 0xf, 0, 4);
	mdelay(2);
	hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL1_A9, 0xe, 0, 4);
	...
	case HDMI_PHYPARA_3p7G:
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL0_A9, 0x8003cafb);
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL5_A9, 0x2555);
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL3_A9, 0x884ef241);
		break;
	/* 2.97Gbps */
	case HDMI_PHYPARA_3G:
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL0_A9, 0x800380dd);
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL5_A9, 0x2555);
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL3_A9, 0x884ef241);
		break;
	/* 1.485Gbps, and below */
	...
	default:
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL0_A9, 0x82038088);
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL5_A9, 0x2555);
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL3_A9, 0x884ef241);
		break;
	}
	...
	if (rterm != 0xff) {
		pr_info("rterm = %d\n", rterm);
		hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL0_A9, rterm, 28, 4);
	} else {
	...
	hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL3_A9, 3, 10, 2);
	hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL3_A9, 3, 3, 2);
	hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL3_A9, 1, 1, 1);
	/* finally config bit[30:28] */
	usleep_range(1000, 1010);
	hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL3_A9, 3, 28, 2);
	hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL3_A9, 1, 30, 1);
}

void hdmitx_turnoff(void)
{
	/* Close HDMITX PHY */
	hd21_write_reg(ANACTRL_HDMIPHY_CTRL0_A9, 0);
	hd21_write_reg(ANACTRL_HDMIPHY_CTRL3_A9, 0);
	hd21_write_reg(ANACTRL_HDMIPHY_CTRL5_A9, 0);
	/* Disable HPLL */
...
void hdmitx_test_prbs(void)
{
	...
	for (i = 0; i < 4; i++) {
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL1_A9, 0x0390000f);
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL1_A9, 0x0390000e);
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL1_A9, 0x03904002);
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL4_A9, 0x0001efff | (i << 20));
		hd21_write_reg(ANACTRL_HDMIPHY_CTRL1_A9, 0xef904002);
	}
}

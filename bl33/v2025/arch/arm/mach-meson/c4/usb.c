...
#define USB_GEN_BIT		4
#define USB_PHY_APB_BIT		7
#define USB_PHY_BIT	    	8
#define USB_2_DRD_BIT	    10

#define USB2_PLL_RST_BIT 		0
#define USB2_PLL_LOCK_RST_BIT 	1
#define USB2_PLL_EN_BIT 		2
#define USB2_PLL_EN_CTRL_BIT 	29
...
{
	writel(readl(RESET_LEVEL_BASE) | (1 << USB_PHY_BIT), RESET_LEVEL_BASE);
}
...
__retry:
	writel(pll_val0 | (1 << USB2_PLL_RST_BIT) | (1 << USB2_PLL_LOCK_RST_BIT),
		(phy_reg_base + 0x40));
	usb_udelay(100);

	writel(pll_val1 | (1 << USB2_BGR_EN_BIT), (phy_reg_base + 0x5c));
	usb_udelay(100);

	writel(pll_val0 | (1 << USB2_PLL_RST_BIT) | (1 << USB2_PLL_LOCK_RST_BIT) |
		(1 << USB2_PLL_EN_CTRL_BIT), (phy_reg_base + 0x40));
	usb_udelay(100);

	writel(pll_val0 | (1 << USB2_PLL_RST_BIT) | (1 << USB2_PLL_LOCK_RST_BIT)
		| (1 << USB2_PLL_EN_BIT) | (1 << USB2_PLL_EN_CTRL_BIT),
		(phy_reg_base + 0x40));
	usb_udelay(100);

	writel(pll_val0 | (1 << USB2_PLL_LOCK_RST_BIT) | (1 << USB2_PLL_EN_BIT) |
		(1 << USB2_PLL_EN_CTRL_BIT), (phy_reg_base + 0x40));
	usb_udelay(100);

	writel(pll_val0 | (1 << USB2_PLL_EN_BIT) | (1 << USB2_PLL_EN_CTRL_BIT),
		(phy_reg_base + 0x40));
...
int usb2_phy_init(struct phy *phy)
{
	...
	if (priv->usb_phy2_pll_base_addr[0] == PHY_20_BASE) {
		...
		writel((1 << USB_2_DRD_BIT) | (1 << USB_GEN_BIT), priv->reset_addr);

		usb_udelay(500);
		priv->usbphy_reset_bit[0] = USB_PHY_BIT;
	}
...
void usb_device_mode_init(int phy_num)
{
	...
	//step 1: power on usb2 phy
	writel((readl(RESET_LEVEL_BASE) & (~(1 << USB_PHY_BIT))), RESET_LEVEL_BASE);
	usb_udelay(500);
	writel((readl(RESET_LEVEL_BASE) | (1 << USB_PHY_BIT)), RESET_LEVEL_BASE);

	//step 2: usb controller reset
	writel((1 << USB_GEN_BIT) | (1 << USB_2_DRD_BIT), reset_addr);

	/*step 3: config phy device mode*/
	writel(readl(PHY_COMP_BASE + 0x88) | (1 << 25), (PHY_COMP_BASE + 0x88));
	usb_udelay(10);
	...
	writel((1 << USB_PHY_APB_BIT), reset_addr);
	usb_udelay(10);

	/*step 6: phy reset*/
	writel((1 << USB_PHY_BIT), reset_addr);
	usb_udelay(10);

...
int ts_a = 9164, ts_b = 2747, ts_m = 370, ts_n = 270;
unsigned int ts_cfg_reg1[3] = {TS_NNA_CFG_REG1, TS_ISP_CFG_REG1, TS_PLL_CFG_REG1};
unsigned int ts_state0_reg[3] = {TS_NNA_STAT0, TS_ISP_STAT0, TS_PLL_STAT0};
char *sensor_name[3] = {"nna", "isp", "pll"};
...
unsigned int r1p1_temptocode(unsigned long value, int tempbase)
{
	...
	tmp1 = (((int64_t)tempbase * 10 + ts_b) * (1 << 16)) / ts_a;	/*ideal u */
	printf("%s : tmp1: 0x%lx\n", __func__, tmp1);
	tmp2 = (ts_m * (int64_t)value * (1 << 16)) / ((1 << 16) * 100 + ts_n * value);
	printf("%s : tmp2: 0x%lx\n", __func__, tmp2);
...
int r1p1_temp_trim(int tempbase, int tempver, int type)
{
	...
	case 2:
		...
		value_ts = value_all_ts / cnt;
		printf("epp tsensor avg: 0x%x\n", value_ts);
		if (value_ts == 0) {
			printf("epp tsensor read temp is zero\n");
			return -1;
		}
		u_efuse = r1p1_temptocode(value_ts, tempbase);
		printf("ts efuse:%d\n", u_efuse);
		...
		printf("ts efuse:0x%x, index: %d\n", u_efuse, index_ts);
		if (tsensor_tz_calibration(index_ts, u_efuse) < 0) {
			printf("epp tsensor thermal_calibration send error\n");
			return -1;
		}
		break;
	default:

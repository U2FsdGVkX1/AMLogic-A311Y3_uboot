...
void usb_aml_detect_operation(int argc, char * const argv[])
{
	...
	for (i = 0; i < AMLOGIC_CTR_COUNT; i++) {
		...
		if (usb2_priv) {
			...
			printf("device base addr: 0x%08x\n", usb2_priv->dwc2_a_addr);
		}
	}
}
...
__retry:
	...
	writel(pll_val0 | (1 << USBPLL_LK_RST_BIT) |
		(1 << USB2_MPLL_EN_CTRL_BIT) | (1 << USBPLL_EN_BIT), (phy_reg_base + 0x40));
	usb_udelay(100);
...
void set_usb_power_off(void)
{
	...
	tmp = 0;
	tmp |= 1 << PHY20_RESET_LEVEL_BIT;
	tmp |= 1 << PHY21_RESET_LEVEL_BIT;
	tmp |= 1 << PHY22_RESET_LEVEL_BIT;
	val &= ~tmp;

...
#include <asm/amlogic/arch/hdmirx_reg.h>
...
u32 rx_rd_bits_reg(unsigned long addr, u32 mask)
{
	return get(rx_rd_reg(addr), mask);
}

void rx_wr_bits_reg(unsigned long addr, u32 mask, u32 value)
{
	rx_wr_reg(addr, set(rx_rd_reg(addr), mask, value));
}
...
unsigned int hdmirx_rd_top(unsigned long addr)
{
	...
	if (addr >= TOP_EDID_ADDR_S && addr <= TOP_EDID_PORT3_ADDR_E) {
					...
					unsigned int *phy_addr)
{
	...
	for (i = 0; i <= 255; i++) {
		...
		if (pedid[i] == 0x03) {
			if (pedid[i + 1] == 0x0c && pedid[i + 2] == 0x00) {
				if (brepeat)
					pedid[i + 3] = 0x00;
				else
					pedid[i + 3] = 0x10;
	...
	unsigned char *pchecksum)
{
	...
	for (i = 0; i <= 255; i++) {
		/* fill first edid buffer */
		hdmirx_wr_top(TOP_EDID_PORT2_ADDR_S + i, pedid[i]);
		/* fill second edid buffer */
		hdmirx_wr_top(TOP_EDID_PORT2_ADDR_S + 0x100 + i, pedid[i]);
	}
	for (i = 0; i <= 255; i++) {
		/* fill first edid buffer */
		hdmirx_wr_top(TOP_EDID_PORT3_ADDR_S + i, pedid[i]);
		/* fill second edid buffer */
		hdmirx_wr_top(TOP_EDID_PORT3_ADDR_S + 0x100 + i, pedid[i]);
	}
	...
	if (brepeat) {
		...
		for (i = 0; i < E_PORT_NUM; i++) {
			pchecksum[i] = (0x100 - (checksum + (pphy_addr[i] - 0x10))) & 0xff;
		}
	}
}
...
void rx_set_phy_rterm(void)
{
	...
	rx_rterm_val = meson_trustzone_efuse_caliItem("hdmirx");
	if (rx_rterm_val >= 0) {
		data32 = hdmirx_rd_amlphy(HHI_RX_PHY_MISC_CNTL1);
		data32 &= (~(0xf << 12));
		/* rterm val */
		data32 |= (rx_rterm_val << 12);
		/* rterm flag */
		data32 |= 0x1 << 0;
		hdmirx_wr_amlphy(HHI_RX_PHY_MISC_CNTL1, data32);
		printf("rx trim:0x%x\n", data32);
	} else {
		printf("no rx trim val!\n");
	}
}

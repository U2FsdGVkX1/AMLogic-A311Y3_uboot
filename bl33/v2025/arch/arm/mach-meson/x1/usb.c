...
#define PHY30_RESET_LEVEL_BIT	6
#define USB_RESET_BIT		9
#define USB_DRD_BIT	    8

#define USB2_PLL_RST_BIT 		18
#define USB2_PLL_LK_RST_BIT 	28
#define USB2_PLL_EN_BIT 		23
#define USB2_PLL_EN_CTRL_BIT 	11
#define USB2_BGR_EN_BIT			0
...
__retry:
	writel(pll_val0 | (1 << USB2_PLL_RST_BIT), (phy_reg_base + 0x44));
	usb_udelay(100);

	writel(pll_val1 | (1 << USB2_PLL_LK_RST_BIT), (phy_reg_base + 0x40));
	usb_udelay(100);

	writel(pll_val2 | (1 << USB2_BGR_EN_BIT), (phy_reg_base + 0x5c));
	usb_udelay(100);

	writel(pll_val0 | (1 << USB2_PLL_RST_BIT) | (1 << USB2_PLL_EN_BIT),
		(phy_reg_base + 0x44));
	usb_udelay(100);

	writel(pll_val1 | (1 << USB2_PLL_EN_CTRL_BIT) | (1 << USB2_PLL_LK_RST_BIT),
		(phy_reg_base + 0x40));
	usb_udelay(100);

	writel(pll_val0 | (1 << USB2_PLL_EN_BIT), (phy_reg_base + 0x44));
	usb_udelay(100);

	writel(pll_val1 | (1 << USB2_PLL_EN_CTRL_BIT), (phy_reg_base + 0x40));
	// wait for 200us
	usb_udelay(200);
	//check lock bit
...
int usb2_phy_init(struct phy *phy)
{
	...
	writel((readl(priv->reset_addr + 0x40) & (~(1 << PHY30_RESET_LEVEL_BIT))),
		priv->reset_addr + 0x40);
	usb_udelay(10);

	if (priv->usb_phy2_pll_base_addr[0] == PHY_20_BASE) {
		debug("priv->reset_addr is 0x%x\n", priv->reset_addr);
		writel((1 << USB_DRD_BIT) | (1 << USB_RESET_BIT), priv->reset_addr);
...
void usb_device_mode_init(int phy_num)
{
	...
	writel((readl(RESET_LEVEL_BASE) & (~(1 << PHY30_RESET_LEVEL_BIT))), RESET_LEVEL_BASE);
	...
	// step 4: config phy20 device mode
	...
	writel(readl(CLKCTRL_USB_CLK_CTRL) | 0x101, CLKCTRL_USB_CLK_CTRL);
	usb_udelay(12);
	writel((readl(PHY_COMP_BASE + 0X9C) | (1 << 13)), PHY_COMP_BASE + 0X9C);
	usb_udelay(12);

...
static char md5_str[MD5_DIGEST_LENGTH * 2 + 1] = {0};

unsigned int bl3x_secure_start;
unsigned int bl3x_secure_end;
...
static unsigned char *get_stride(size_t  *stride,
			   ...
			   int wrap_size, unsigned int total_ram)
{
	...
	while (rci->curr_processed < rcs->section_size) {
		...
		if (debug_enable) {
			...
			serial_put_hex((unsigned long)srcaddr, 48);
			serial_puts(", free space:0x");
			serial_put_hex(mem_avail, 32);
			serial_puts(", out_buf:0x");
			//serial_put_hex((unsigned long)out_buf, 48);
	...
	for (i = 0; i < rcf->section_count; i++) {
		...
		case RAM_COMPRESS_COPY:
			...
			if (rcs->copy_dest_addr != 0) {
				serial_puts(" type: RAM_COMPRESS_COPY");
		...
		/* fix up DMA-copy on panic */
		if (i == 1 && rcs->phy_addr != 0x0 &&
			rcs->compress_type == RAM_COMPRESS_NORMAL &&
			rcs->phy_addr < rcf->store_phy_addr) {
			memset(rcs->phy_addr, 0, rcs->section_size);
			serial_puts("memset Panic-DMA-copy area, addr=0x");
			serial_put_hex((unsigned long)rcs->phy_addr, 32);
			serial_puts(", size=0x");
			serial_put_hex(rcs->section_size, 32);
			serial_puts("\n");
		}
...
int check_reboot_reason(unsigned long *mem_size)
{
	...
	unsigned long nTotal = cfg & (AMLOGIC_USERAM_MASK & ~RAMDUMP_STICKY_DMA_MASK);
	*mem_size = (((nTotal - 1) / 64 + 1) * 64) << 20;
	serial_puts("nTotal:0x");
	serial_put_hex(nTotal, 16);
...
void calculate_ramdump_md5(u64 ramdump_base, u64 ramdump_size)
{
	static const char hex_table[] = "0123456789abcdef";
	int i;

	serial_puts("\n[debug] get RAMDUMP md5sum: ");

	/* calculate raw md5 */
	md5((unsigned char *)ramdump_base, ramdump_size, md5_hex);

	/* convert md5 to hex string (no sprintf) */
	for (i = 0; i < MD5_DIGEST_LENGTH; i++) {
		md5_str[i * 2]     = hex_table[(md5_hex[i] >> 4) & 0xF];
		md5_str[i * 2 + 1] = hex_table[md5_hex[i] & 0xF];
	}
	...
	/* save to BL33Z area */
	memcpy((void *)MD5_BL33Z_RAMDUMP_STR,
	       md5_str,
	       MD5_DIGEST_LENGTH * 2 + 1);
}

void fix_up_big_store_addr(unsigned long addr, unsigned long size)
{
	unsigned long addr_tmp, size_tmp;

	serial_puts("compress size:0x");
	serial_put_hex(size, 32);
	serial_puts("\n");
	if (size > 0) {
		/* Calculate md5sum of crash file before updating ramdump_base register */
		calculate_ramdump_md5(addr, size);
		/*
		 * ramdump_base(store_addr) register can only hold a 32-bit value.
		 * Update the handling to support physical addresses above 4GB.
		 */
		addr_tmp = addr;
		size_tmp = size;
		...
		if (addr >= REG_SPACE_END_ADDR) {
			...
			serial_puts("\n[debug] addr_tmp:0x");
			serial_put_hex(addr_tmp, 40);
			serial_puts(", size_tmp:0x");
			serial_put_hex(size_tmp, 40);
			serial_puts("\n");
		}
#endif
			    ...
			    unsigned long size)
{
	...
	if (size <= CONFIG_DDR_SIZE_128MB) {
		...
		rcf->sections[3].phy_addr = (void *)0x2000000;
		rcf->sections[3].section_size = 0x200000;
		rcf->sections[2].section_size = (unsigned long)rcf->sections[3].phy_addr -
						(unsigned long)rcf->sections[2].phy_addr;
...
#if defined(SOC_C4) || defined(SOC_C5)
		/*
		 * On C4/C5, BL32 reserved memory is located below 256MB.
		 * Set compression physical address beyond the 256MB boundary.
		 */
		rcf->store_phy_addr = (void *)CONFIG_COMPRESS_AFTER_256MB;
#else
		rcf->store_phy_addr = (void *)CONFIG_SMALL_COMPRESS_ADDR;
#endif
		rcf->full_memsize   = size;
		rcf->sections[0].phy_addr = rcf->store_phy_addr;
		rcf->sections[0].section_size = size - (unsigned long)rcf->sections[0].phy_addr;
		rcf->sections[5].section_size = 0;
		rcf->sections[6].section_size = 0;
		...
		rcf->store_phy_addr = rcf->store_phy_addr;
		...
		rcf->sections[6].section_size = rcf->full_memsize - CONFIG_BIG_COMPRESS_ADDR;
	} else {
		/* > 7.0G ~, use 7 sections */
		serial_puts("ddr size range 3: > 7.0G\n");
#ifdef SYSCTRL_CHIP_ID
		if ((readl((void __iomem *)SYSCTRL_CHIP_ID) & 0xFF) == 0x36) {
			ddr_hole_count = 2;
			serial_puts("Info: T7c have 2 ddr holes.\n");
		}
#endif
		/* 8GB DDR layout:
		 * 1 holes: 3.5G + 0.5G + 4.5G = 8.5GB
		 * 2 holes: 3.5G + 0.5G + 3.5G + 0.5G + 1G = 9GB (only for T7C)
		 */
		rcf->full_memsize   = size + REG_SPACE_RANGE_SIZE * ddr_hole_count;
		rcf->store_phy_addr = (void *)CONFIG_BIG_COMPRESS_ADDR;
		// overwrite rcf->sections[0] to index 7
		rcf->sections[0].phy_addr       = (void *)CONFIG_SECTION7_ADDR;
		rcf->sections[0].section_size   = rcf->full_memsize - CONFIG_BIG_COMPRESS_ADDR;
		rcf->sections[0].section_index  = 7;
		rcf->sections[0].compress_type  = RAM_COMPRESS_NORMAL;
		rcf->sections[0].copy_dest_addr = (void *)0;
		// overwrite rcf->sections[6] to index 5
		rcf->sections[6].phy_addr       = (void *)CONFIG_SECTION5_ADDR;
		rcf->sections[6].section_size   = CONFIG_SECTION5_SIZE;
		rcf->sections[6].section_index  = 5;
		rcf->sections[6].compress_type  = RAM_COMPRESS_NORMAL;
		rcf->sections[6].copy_dest_addr = (void *)0;
	}
...
static void get_bl3x_secure_meminfo(void)
{
	...
	serial_puts("+1MB\n");
	bl3x_secure_start = bl31_rsvmem_start;
	if (bl32_rsvmem_start) {
		bl3x_secure_end = bl32_rsvmem_start + bl32_rsvmem_size + 0x100000;
	} else {
		bl3x_secure_end = bl31_rsvmem_start
				+ bl31_rsvmem_size + bl32_rsvmem_size + 0x100000;
	}
#else
	bl3x_secure_start = 0;
	bl3x_secure_end = 0;
...
{
#ifdef REG_MDUMP_RSVMEM_SIZE
	if (bl3x_secure_end > (unsigned long)rcf->store_phy_addr) {
		serial_puts("Err: bl3x secure region conflicts with ramdump. exit.\n");
		return -1;
	}
	rcf->sections[3].phy_addr = (void *)(unsigned long)bl3x_secure_start;
	rcf->sections[2].section_size =
				(unsigned long)(rcf->sections[3].phy_addr) -
				(unsigned long)(rcf->sections[2].phy_addr);
	rcf->sections[4].phy_addr = (void *)(unsigned long)bl3x_secure_end;
	rcf->sections[3].section_size = (unsigned long)(rcf->sections[4].phy_addr)
				- (unsigned long)(rcf->sections[3].phy_addr);
#else
	serial_puts("REG_RSVMEM_SIZE is not defined. set bl31 bl32 size = 0.\n");
	rcf->sections[3].section_size = 0;
	rcf->sections[4].phy_addr = rcf->sections[3].phy_addr;
#endif
	if ((unsigned long)rcf->store_phy_addr < CONFIG_BIG_COMPRESS_ADDR) {
		rcf->sections[4].section_size =
				(unsigned long)(rcf->sections[0].phy_addr) -
				(unsigned long)(rcf->sections[4].phy_addr);
	} else {
		serial_puts("Big DDR: section[0] moved to section[6]\n");
		rcf->sections[4].section_size =
				(unsigned long)(rcf->sections[6].phy_addr) -
				(unsigned long)(rcf->sections[4].phy_addr);
	}

	if (debug_enable) {
		serial_puts("Fix up sections[3].phy_addr:0x");
		serial_put_hex((unsigned long)(rcf->sections[3].phy_addr), 32);
		serial_puts(", size:0x:");
		serial_put_hex(rcf->sections[3].section_size, 32);
		serial_puts("; sections[4].phy_addr:0x:");
		serial_put_hex((unsigned long)rcf->sections[4].phy_addr, 32);
		serial_puts(", size:0x:");
		serial_put_hex(rcf->sections[4].section_size, 32);
		serial_puts(".\n");
	}
	return 0;
}

static int fix_up_panic_dma_copy(struct ram_compress_full *rcf)
{
	unsigned long cfg;

	cfg = readl(REG_MDUMP_CPUBOOT_STATUS) & RAMDUMP_STICKY_DMA_MASK;
	if (cfg) {
		serial_puts("Fix up DMA-copy sections[1].phy_addr: 0x");
		serial_put_hex(cfg * RAMDUMP_DMA_ALIGNED_4MB, 32);
		serial_puts(" (0x");
		serial_put_hex(cfg, 8);
		serial_puts(")\n");
		rcf->sections[1].phy_addr = (void *)(cfg * RAMDUMP_DMA_ALIGNED_4MB);
		rcf->sections[1].compress_type = RAM_COMPRESS_NORMAL;
	}
			 ...
			 unsigned long size)
{
	...
	if (fix_up_bl32_size(rcf) < 0)
		return -1;
	/* update rcf 0-24MB area with DMA-copy addr. */
	fix_up_panic_dma_copy(rcf);
...
#ifdef REG_MDUMP_RSVMEM_SIZE
	skip2_start	= bl3x_secure_start;
	skip2_end	= bl3x_secure_end;
#else
	       ...
	       void *arg, unsigned long flag, unsigned long ctrl)
{
	...
	if ((ret != 0) && size) {
		...
		serial_put_hex(PLATFORM_STACK_TOP, 32);
		serial_puts("\n");
		...
		if (ret > 0)
			fix_up_big_store_addr((unsigned long)rcf->store_phy_addr, ret);

...
#endif
#ifdef CONFIG_AMLOGIC_DPTX_DRIVER
#include <amlogic/media/vout/meson_tx_connector/dptx_common/dptx_module.h>
#endif
...
int dram_init(void)
{
	gd->ram_size = (readl(SYSCTRL_SEC_STATUS_REG4) & ~0xfffffUL) << 4;
	return 0;
}
...
int board_init(void)
{
	...
	// apb2 2000M
	// writel(0xfe000378, 0x701);

#ifdef CONFIG_AML_HDMITX21
	hdmitx21_chip_type_init(MESON_CPU_ID_A9);
	hdmitx21_init();
#endif
#ifdef CONFIG_AMLOGIC_DPTX_DRIVER
	dptx_init();
...
int board_boot_freertos(void)
{
	...
	//if hibernate boot, just finish rtos
	// if (hibernate_boot_check() == 0) {
	// 	printf("freertos will not work because hibernate\n");
	// 	return 0;
	// }
...
int board_late_init(void)
{
	...
	struct udevice *lcd0_dev;
	struct udevice *lcd1_dev;
#endif
#ifdef CONFIG_AML_LCD_BACKLIGHT
	struct udevice *backlight_dev;
	struct udevice *backlight1_dev;
#endif
	int ret;
...
#ifdef CONFIG_AML_eDPTX
	run_command("mw 0xfe000378 0x701", 0);
	edptx_driver_probe();
#endif
#ifdef CONFIG_AML_LCD
	ret = uclass_get_device_by_name(UCLASS_MISC, "lcd", &lcd0_dev);
	if (ret)
		printf("device lcd not found\n");
	ret = uclass_get_device_by_name(UCLASS_MISC, "lcd1", &lcd1_dev);
	if (ret)
		printf("device lcd1 not found\n");
#endif
#ifdef CONFIG_AML_LCD_BACKLIGHT
	ret = uclass_get_device_by_name(UCLASS_MISC, "backlight", &backlight_dev);
	if (ret)
		printf("device backlight not found\n");
	ret = uclass_get_device_by_name(UCLASS_MISC, "backlight1", &backlight1_dev);
	if (ret)
		printf("device backlight1 not found\n");
#endif
...
phys_size_t get_effective_memsize(void)
{
	...
	return ((readl(SYSCTRL_SEC_STATUS_REG4) & 0xFFF00000UL) << 4) > 0xe0000000UL ? 0xe0000000UL :
	    ((readl(SYSCTRL_SEC_STATUS_REG4) & 0xFFF00000UL) << 4) - CONFIG_SYS_MEM_TOP_HIDE;
#else
	return ((readl(SYSCTRL_SEC_STATUS_REG4) & 0xFFF00000UL) << 4) > 0xe0000000UL ? 0xe0000000UL :
	    ((readl(SYSCTRL_SEC_STATUS_REG4) & 0xFFF00000UL) << 4);
...
int checkhw(char *name)
{
	...
	strcpy(dtb_name, "a9_a311y3_venus\0");

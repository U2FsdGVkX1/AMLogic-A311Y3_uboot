...
int get_page_region(long* region_info, u32 max_region)
{
	region_info[0] = get_sharemem_info(GET_SHARE_MEM_INPUT_BASE);
	region_info[1] = get_share_inout_size(GET_SHM_IN_SIZE);
	region_info[2] = get_sharemem_info(GET_SHARE_MEM_OUTPUT_BASE);
	region_info[3] = get_share_inout_size(GET_SHM_OUT_SIZE);
...
phys_size_t get_effective_memsize(void)
{
	...
	rsv_size = (rsv_size + 0xFFFFF) & (~0xFFFFF);

	return ((readl(SYSCTRL_SEC_STATUS_REG4) & 0xFFF00000) << 4) > 0xe0000000 ? 0xe0000000 :
	    (((readl(SYSCTRL_SEC_STATUS_REG4) & 0xFFF00000) << 4) - rsv_size);
#else
	return ((readl(SYSCTRL_SEC_STATUS_REG4) & 0xFFF00000) << 4) > 0xe0000000 ? 0xe0000000 :
	    ((readl(SYSCTRL_SEC_STATUS_REG4) & 0xFFF00000) << 4);
...
int checkhw(char *name)
{
	...
	switch (ddr_size) {
		case 0x40000000UL:
			strcpy(dtb_name, "c4_c302x2_bz409\0");
			break;
		case 0x60000000UL:
			strcpy(dtb_name, "c4_c302x2_bz409\0");
			break;
		default:
			strcpy(dtb_name, "c4_c302x2_bz4049\0");
			break;
	}

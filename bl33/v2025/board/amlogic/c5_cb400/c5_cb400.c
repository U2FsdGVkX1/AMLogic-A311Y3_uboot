...
#if defined(CONFIG_FASTBOOT_MEDIUM_SPINOR)
	int i, node_offset;
	struct fix_table table[] = {
		{"/soc/apb@fe000000/emmc", FDT_STATUS_DISABLED},
		{"/soc/spi@0xfe08d000", FDT_STATUS_OKAY},
	};

	for (i = 0; i < ARRAY_SIZE(table); i++) {
		node_offset = fdt_path_offset(fdt, table[i].node_path);
		if (node_offset < 0) {
			printf("%s node not found : %s\n", table[i].node_path, fdt_strerror(node_offset));
			return -1;
		}

		fdt_set_node_status((void *)fdt, node_offset, table[i].status);
	}
#endif
	return 0;
}
...
extern int charger_power_init(void);
...
phys_size_t get_effective_memsize(void)
{
	...
	return ((readl(SYSCTRL_SEC_STATUS_REG4) & ~0xfffffUL) << 4) >= 0xe0000000 ? 0xe0000000 :
	    ((readl(SYSCTRL_SEC_STATUS_REG4) & ~0xfffffUL) << 4) - CONFIG_SYS_MEM_TOP_HIDE;
#else
	return ((readl(SYSCTRL_SEC_STATUS_REG4) & ~0xfffffUL) << 4) >= 0xe0000000 ? 0xe0000000 :
	    ((readl(SYSCTRL_SEC_STATUS_REG4) & ~0xfffffUL) << 4);
...
int checkhw(char *name)
{
	...
	strcpy(dtb_name, "c5_a123x_cb400\0");

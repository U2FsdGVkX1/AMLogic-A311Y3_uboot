...
static bool _read_in_bootconfig(struct andr_vnd_boot_img_hdr *boot_info, uint32_t ramdisk_size);
...
#define KNLIMG_DEC_ADDR	0x1A00000
#else
...
static const unsigned char zstd_magic[] = {
	0x28, 0xb5, 0x2f, 0xfd
};
...
int is_android_v3_header_image(void *img_addr)
{
...
#if defined(CONFIG_SUPPORT_BL33Z) && CONFIG_IS_ENABLED(AMLOGIC_RAMDUMP)
...
{
	if (is_android_v3_header_image((void*)hdr))
...
{
	return memcmp(VENDOR_BOOT_MAGIC, hdr->magic, ANDR_VENDOR_BOOT_MAGIC_SIZE);
}
...
ulong android_image_get_comp(const struct andr_boot_img_hdr_v0 *os_hdr)
{
	if (is_android_v3_header_image((void*)os_hdr))
			     ...
			     ulong *os_data, ulong *os_len)
{
	...
	if (p_vender_boot_img)
	{
		...
		ulong dtb_addr = env_get_ulong("dtb_mem_addr", 16, 0x1000000);
#endif
		if (*vb_hdr->cmdline) {
...
static bool _read_in_bootconfig(struct andr_vnd_boot_img_hdr *boot_info, uint32_t ramdisk_size)
{
	...
	printf("bootconfig_size: 0x%x\n", boot_info->bootconfig_size);

	if (boot_info->bootconfig_size > 0) {
		u32 vramdisk_size_page_aligned =
		...
		ret = store_logic_read(partname, offset, boot_info->bootconfig_size,
			(void *)(pRAMdisk + ramdisk_size));
		if (ret) {
			printf("Fail to read 0x%xB from part[%s] at offset 0x%x\n",
				boot_info->bootconfig_size, partname, offset);
			return false;
		}

		bootconfig_size += boot_info->bootconfig_size;
	} else {
	...
	boot_info->bootconfig_size = bootconfig_size;
#else
	printf("--- copy bootconfig to bootargs ---\n");
	copy_bootconfig_to_cmdline();
#endif
...
{
	int i;
	unsigned char *src = (unsigned char *)os_hdr + BOOT_IMG_V3_HDR_SIZE;
...
{
	/*
	 * Compress format boot.img need relocate to high address, If not relocate
	 * decompree boot.img would overlap origin data, margin space is 32M.
	 */
...
int android_image_need_move(ulong *img_addr, const struct andr_boot_img_hdr_v0 *hdr)
{
	if (is_android_v3_header_image((void*)hdr))

			   ...
			   ulong *os_data, ulong *os_len, const void **kernp)
{
	...
	*os_data = *os_len = 0;
	...
	case IMAGE_FORMAT_ANDROID: {
		...
		ret = android_image_need_move(&img_addr, buf);
		if (!ret)
			buf = map_sysmem(img_addr, 0);
		else
			return ret;

		ret = android_image_get_kernel(buf, images->verify,
				os_data, os_len);
#else
...
static void set_rootfs_dtb_high_addr(void)
{
	phys_size_t ddr_size = env_get_bootm_size();
	const char *high_addr = NULL;

	if (env_get("reset_high_addr")) {
		high_addr = env_get("reset_high_addr");
	} else {
		switch (ddr_size) {
		case 0x8000000:
			high_addr = "6000000";
	...
	env_set("initrd_high", high_addr);
	env_set("fdt_high", high_addr);
}
#endif
...
static int do_fdt_overlay(void)
{
	...
	dtbo_num = fdt32_to_cpu((
		(const struct dt_table_header *)(uintptr_t)dtbo_mem_addr)->dt_entry_count);
	...
	free((void *)(uintptr_t)dtbo_mem_addr);
	return 0;
}
#endif

...
#include "../drivers/mtd/ubi/crc32.c"
#endif
...
extern int mmc_key_write(unsigned char *buf, unsigned int size, uint32_t *actual_length);
extern int mmc_key_read(unsigned char *buf, unsigned int size, uint32_t *actual_length);
...
static int amlmmc_erase_in_dev(int argc, char *const argv[])
{
	...
	printf("dev # %d, several blocks erased %s\n", dev, (n == cnt) ? "OK" : "ERROR");
...
static int amlmmc_erase_non_cache(int arc, char *const argv[])
{
	...
	if (blk > 0) {
		...
		blk >>= blk_shift;
		blk -= start_blk;
		// erase all the area before reserve-partition
		if (blk > 0)
			n = blk_derase(mmc_get_blk_desc(mmc), start_blk, blk);
		if (n == 0)
			return 1;
...
static int amlmmc_go_idle(struct mmc *mmc)
{
	struct mmc_cmd cmd;
	int err;
...
int mmc_ffu_op(int dev, u64 ffu_ver, void *addr, u64 cnt)
{
	...
	err = mmc_send_ext_csd(mmc, ext_csd_ffu);
	if (err)
		return err;
...
static int do_amlmmc_ffu(cmd_tbl_t *cmdtp, int flag,
		int argc, char * const argv[])
{
	...
	if (gd->flags & GD_FLG_SMP)
		spin_lock(&mmc->lock);
#endif
	ret = mmc_ffu_op(dev, ver, addr, cnt);
#ifdef CONFIG_ARMV8_MULTIENTRY
	if (gd->flags & GD_FLG_SMP)
...
static cmd_tbl_t cmd_amlmmc[] = {
	...
	U_BOOT_CMD_MKENT(ffu,           5, 0, do_amlmmc_ffu,           "", ""),
...
U_BOOT_CMD(
	...
	"amlmmc ffu <device_num> <ver> <addr> <cnt> - update ffu fw\n"

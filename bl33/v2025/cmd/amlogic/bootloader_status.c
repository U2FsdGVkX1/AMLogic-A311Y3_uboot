...
void update_rollback(void)
{
	...
	if (strcmp(slot, "0") == 0) {
		if (ret != 0) {
			...
			run_command("check_uboot_state 0", 0);
		...
		if (ret != 0) {
			...
			wrnP("gpt or disable user bootloader mode\n");
			run_command("check_uboot_state 1", 0);
			write_bootloader_back("2", 1);
			env_set("expect_index", "1");
		}
		wrnP("back to slot a\n");
...
static int update_gpt(int flag)
{
	...
	if (mmc) {
		...
		gpt_h = (void *)buffer + 0x3DFE00 + (GPT_PRIMARY_PARTITION_TABLE_LBA *
		       dev_desc->blksz);
		if (le64_to_cpu(gpt_h->signature) != GPT_HEADER_SIGNATURE_UBOOT) {
			...
			check_gpt_part(dev_desc, buffer + 0x3DFE00);
...
int ab_update_rollback(char *rebootmode, int gpt_flag)
{
	...
	if (write_boot && !strcmp(write_boot, "1")) {
		...
		run_command("check_uboot_state boot1", 0);
		if (write_bootloader(1)) {
			printf("write boot0 fail, need to rollback!\n");
			env_set("rollback_reason", "boot0_update_fail");
#if CONFIG_IS_ENABLED(AML_UPDATE_ENV)
	...
	if (rebootmode && (strcmp(rebootmode, "rescueparty") == 0)) {
		...
		env_set("rollback_reason", "rescueparty");
#if CONFIG_IS_ENABLED(AML_UPDATE_ENV)
		run_command("update_env_part -p rollback_reason;", 0);
#else
		run_command("saveenv", 0);
	...
	if (rebootstatus && !strcmp(rebootstatus, "reboot_next")) {
		...
		if (match_flag != 0) {
			...
			if (CONFIG_IS_ENABLED(AML_OTA_ERASE_DDR_PARAMETER)) {
				run_command("echo erase ddr-parameter for OTA", 0);
				run_command("store rsv erase ddr-parameter;reboot", 0);
			}
		}
	}
...
//usb_burn check for env and display
static void factory_usb_burn_status_check(void)
{
	if (env_get_hex("usbDiskNewBoot", 0) && env_get_hex("usbDiskBootCpy", 0xff) != 0xff) {
		...
		if (want_cpy != boot_cpy) {
			pr_err("Fail check boot index, want %d but %d\n", want_cpy, boot_cpy);
			env_set_ulong("usbDiskChkOk", 0);
			//delete usbDiskBootCpy to avoid always enter check
			run_command("env set usbDiskBootCpy", 0);
			run_command("env set usbDiskNewBoot", 0);
			run_command("env default preboot; saveenv", 0);
			printf("USB_BURN img bootloader cannot bootup, so END upgrade\n");
			run_command("reboot", 0);//usb_burn quiescent without bootup logo
			return;
		}

		printf("okay check img bootloader for usb_burn\n");
		printf("update env for usb_burn\n");
		run_command("printenv usbDiskNewBoot sdcburncfg usbDiskUpgrade preboot", 0);
		run_command("defenv_reserve usbDiskNewBoot sdcburncfg usbDiskUpgrade preboot", 0);
		run_command("printenv usbDiskNewBoot sdcburncfg usbDiskUpgrade preboot", 0);
		run_command("env set usbDiskBootCpy ff", 0);
		run_command("env set usbDiskChkOk 1;saveenv", 0);
		return;
	}
}

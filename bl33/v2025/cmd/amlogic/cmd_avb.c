...
void set_avb_parts(const char *partname, uint8_t *addr, size_t length)
{
	...
	unlocked = is_device_unlocked();

	if (!unlocked && avb_preload) {
	...
	*out_is_trusted = false;
	...
	if (!IS_FEAT_BOOT_VERIFY()) {
		...
		if (keybuf && (strncmp(keybuf, "AVBK", 4) == 0)) {
			 ...
			 */
			printf("AVB2 verify with default kpub:%d, vbmeta kpub:%zu\n",
		...
		printf("AVB2 verify with production kpub:%d, vbmeta kpub:%zu\n",
...
#define PERSISTENT_FILENAME "avb/avb_persist"
...
uint32_t create_csrs(void)
{
	...
	if (type == BOOT_NAND_MTD || type == BOOT_SNAND) {
		printf("NAND does not support creating csrs.\n");
		return AVB_IO_RESULT_OK;
	}
...
static AvbIOResult write_persistent_to_factory(uint8_t *buf, uint32_t size)
{
	...
	/* assuming directory is already there */
	sprintf(cmd, "fatwrite %s 0x%X:0x%X 0x%08X %s 0x%X", DEV_NAME, DEV_NO,
			get_partition_num_by_name(part_name),
			(uint32_t)virt_to_phys((void *)buf), PERSISTENT_FILENAME, size);
	if (run_command(cmd, 0)) {
		/* if write fails, let's try creating directory */
		sprintf(cmd, "fatmkdir %s 0x%X:0x%X %s", DEV_NAME, DEV_NO,
				get_partition_num_by_name(part_name), "avb");
		if (run_command(cmd, 0)) {
			printf("command[%s] failed\n", cmd);
			return AVB_IO_RESULT_ERROR_IO;
		}
...
int avb_verify_single(const char *part_name)
{
	...
	result = avb_slot_verify(&avb_ops_, requested_partitions, ab_suffix,
				 flags, hashtree_error_mode, NULL);
...
//					security_patch,len = %d\n", (int)len);
			} else if (strcmp(p->partition_name, "boot") == 0) {
				if (p->verify_result != AVB_VBMETA_VERIFY_RESULT_OK) {
					// not verified
					printf("boot verify_result %d\n", p->verify_result);

					/*device lock, treat as error*/
					if (!is_device_unlocked()) {
						printf("device lock, but boot verify fail\n");
						return 0;
					}
				}

...
#define GPT_OFFSET              0x3DFE00
#define DTS_GPT_OFFSET          0x3E0000
...
static void dump_boot_info(bootloader_control *boot_ctrl)
{
	...
	if (loglevel >= 8 &&
		...
		printf("boot_ctrl->retest_times = %d\n",
			boot_ctrl->retest_times);
		for (slot = 0; slot < 4; ++slot) {
			...
			printf("boot_ctrl->slot_info[%d].uboot_status = %d\n",
					slot, boot_ctrl->slot_info[slot].uboot_status);
		}
...
static int check_bootloader(int slot)
{
	int iRet = 0;
	int ret = 0;
	unsigned char *buffer_src = NULL;
	int capacity_boot = 0;
	char back_partname[32] = {0};
	...
	if (slot == 0) {
		strcpy((char *)back_partname, "bootloader_b");
	} else if (slot == 1) {
		strcpy((char *)back_partname, "bootloader_a");
	}

	iRet = store_boot_read("bootloader", 2,
		BOOTLOADER_MAX_SIZE - BOOTLOADER_OFFSET, buffer_src);
	if (iRet) {
		printf("Fail read boot1\n");
		goto exit;
	}
	sha1_csum(buffer_src + 512, capacity_boot - 512, gensum0);

	memset(buffer_src, 0, capacity_boot);

	iRet = store_logic_read(back_partname, 0,
		BOOTLOADER_MAX_SIZE - BOOTLOADER_OFFSET, buffer_src);
	if (iRet) {
		printf("Fail read %s\n", back_partname);
		goto exit;
	}
	sha1_csum(buffer_src + 512, capacity_boot - 512, gensum1);

	ret = memcmp(gensum0, gensum1, SHA1SUMLEN);

	printf("boot1 & %s %s same\n", back_partname, ret ? "NOT" : "DO");

#ifdef CONFIG_MMC_MESON_GX
	dev_desc = blk_get_dev("mmc", 1);
	if (dev_desc && dev_desc->type != DEV_TYPE_UNKNOWN) {
		printf("valid mmc device, check gpt\n");

		gpt_h1 = (void *)buffer_src + GPT_OFFSET +
			(GPT_PRIMARY_PARTITION_TABLE_LBA *
			dev_desc->blksz);

		gpt_h2 = (void *)buffer_src + DTS_GPT_OFFSET +
			(GPT_PRIMARY_PARTITION_TABLE_LBA *
			dev_desc->blksz);

		printf("gpt_h1 signature is: 0x%llX\n",
		       le64_to_cpu(gpt_h1->signature));

		printf("gpt_h2 signature is: 0x%llX\n",
		       le64_to_cpu(gpt_h2->signature));

		if (le64_to_cpu(gpt_h1->signature) == GPT_HEADER_SIGNATURE_UBOOT ||
			le64_to_cpu(gpt_h2->signature) == GPT_HEADER_SIGNATURE_UBOOT) {
			printf("backup slot has gpt, set dts_to_gpt to 0\n");
#if CONFIG_IS_ENABLED(AML_UPDATE_ENV)
			env_set("dts_to_gpt", "0");
			run_command("update_env_part -p dts_to_gpt;", 0);
#else
			run_command("defenv_reserve;setenv dts_to_gpt 0;saveenv;", 0);
#endif
		}

		if (ret) {
			printf("boot1 doesn't = %s, rewrite it\n", back_partname);
			iRet = store_boot_write("bootloader", 2,
				BOOTLOADER_MAX_SIZE - BOOTLOADER_OFFSET, buffer_src);
			if (iRet) {
				printf("Failed to write bootloader\n");
				goto exit;
			} else {
				printf("write %s to boot1 ok\n", back_partname);
				ret = 0;
			}
		}
	}
...
static int is_BootSame(int srcindex, int dstindex)
{
	...
	sha1_csum_wd(buffer_src, capacity_boot, gensum0, 512);
	...
	sha1_csum_wd(buffer_dest, capacity_boot, gensum1, 512);
	...
	char * const argv[])
{
	...
	if (slot == 0) {
		if (bootable_a) {
			...
			char *reason = env_get("rollback_reason");

			if (reason && (strcmp(reason, "kernel_panic") == 0))
				env_set("rollback_reason", "kernel_panic");
			else if (reason && (strcmp(reason, "insufficient_data") == 0))
				env_set("rollback_reason", "insufficient_data");
			else
				env_set("rollback_reason", "boot_failure");
#if CONFIG_IS_ENABLED(AML_UPDATE_ENV)
			run_command("update_env_part -p rollback_reason;", 0);
#else
	...
	char * const argv[])
{
	...
	if (strcmp(argv[1], "1") == 0) {
		...
		check_bootloader(0);
	} else if (strcmp(argv[1], "boot1") == 0) {
		active_slot = get_active_slot(&info);
		if (active_slot == 1)
			backup_slot = 0;
		else
			backup_slot = 1;
		if (info.slot_info[backup_slot].successful_boot == 1 &&
			info.slot_info[backup_slot].uboot_status != 2) {
			if (gpt_partition) {
	...
	char * const argv[])
{
		...
		if (bootable_a) {
			if (boot_ctrl.slot_info[0].successful_boot == 0) {
				...
				if (rebootmode && strcmp(rebootmode, "fastboot") &&
					strcmp(rebootmode, "factory_reboot") &&
					strcmp(rebootmode, "bist_reboot")) {
	...
	if (boot_ctrl.firstboot == 1) {
		if (rebootmode && strcmp(rebootmode, "fastboot") &&
			strcmp(rebootmode, "factory_reset") &&
			strcmp(rebootmode, "factory_reboot") &&
			strcmp(rebootmode, "bist_reboot")) {
	...
	char *insufficient_data = env_get("insufficient_data");

	printf("insufficient_data : %s\n", insufficient_data);
	if (insufficient_data && !strcmp(insufficient_data, "1") &&
	    !strcmp(rebootmode, "kernel_panic")) {
		printf("Insufficient data space, rollback\n");
		env_set("insufficient_data", "0");
		env_set("rollback_reason", "insufficient_data");
#if CONFIG_IS_ENABLED(AML_UPDATE_ENV)
		run_command("update_env_part -p insufficient_data rollback_reason;", 0);
#else
		run_command("saveenv", 0);
#endif
	...
	if (boot_ctrl.slot_info[slot].successful_boot == 1 &&
		boot_ctrl.slot_info[slot].uboot_status != 2) {
		if (gpt_partition || nocs_mode) {
...
U_BOOT_CMD(
	check_uboot_state, 2, 1, do_CheckUbootState,
	"check_uboot_state",
	"\nThis command will check_uboot_state\n"
	"So you can execute command: check_uboot_state 0"
);
...
int set_successful_boot(void) {
	...
	if (AB_mode == 1) {
		...
		printf("active slot = %d\n", slot_num);
		bootable = info.slots[slot_num].successful_boot;
		//bootable_b = slot_is_bootable_normalAB(&info.slots[1]);
		/*boot_info_reset(&boot_ctrl);
		boot_ctrl.slot_info[0].successful_boot = info.slots[0].successful_boot;
		boot_ctrl.slot_info[1].successful_boot = info.slots[1].successful_boot;
		...
		printf("active slot = %d\n", slot_num);
		bootable = boot_ctrl.slot_info[slot_num].successful_boot;
		//bootable_b = slot_is_bootable(&boot_ctrl.slot_info[1]);
	}

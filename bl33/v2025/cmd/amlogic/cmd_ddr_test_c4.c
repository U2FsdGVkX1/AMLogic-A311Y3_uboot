...
#define MESON_CPU_MAJOR_ID_S6			0x48
#endif
#ifdef MESON_CPU_MAJOR_ID_C4
#undef MESON_CPU_MAJOR_ID_C4
#define MESON_CPU_MAJOR_ID_C4		0x4C
#else
#define MESON_CPU_MAJOR_ID_C4			0x4C
#endif
#ifdef MESON_CPU_MAJOR_ID_X1
#undef MESON_CPU_MAJOR_ID_X1
#define MESON_CPU_MAJOR_ID_X1		0x50
#else
#define MESON_CPU_MAJOR_ID_X1			0x50
#endif
...
//s1a
	...
	//x1
	{
		...
		.ddr_pctl_timing_end_address = ((0x00bb << 2) + 0xfe036400),    //DMC_DRAM_DFI
		.ddr_dmc_sticky0 = ((0x0000 << 2) + 0xfe036800),
		.ddr_pll_base_address = ((0x0000 << 2) + 0xfc00e000),
		//AM_DDR_PLL_CNTL0//
		//.ddr_boot_reason_address = ((0x00e1 << 2) + 0xfe010000),
		//SYSCTRL_SEC_STICKY_REG1,20210204,0xfe010384, confirm
		.ddr_dmc_lpdd4_retraining_address = ((0x0097 << 2) + 0xfe036400),
...
#if 0//(CONFIG_DDR_PHY > P_DDR_PHY_DEFAULT)
...
int do_ddr_test_copy(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	size_count = (uintptr_t)counter * test_size;
	time_start = get_us_time(); //us
	do {
		loop = 1;
		m_len = test_size / 4;
...
int get_ddr_clk(void)
{
	...
	uintptr_t stick_ddr_clk_add1 = 0;
	uintptr_t stick_ddr_clk_add2 = 0;

	stick_ddr_clk_add1 = (uintptr_t)(&ddr_set_array[0].cfg_board_SI_setting_ps.DRAMFreq);
	stick_ddr_clk_add2 = (uintptr_t)(&ddr_set_array[0]);
...
void ddr_memcpy(void *dst, const void *src, uint32_t len)
{
	...
	if (pre_fetch_enable) {
		while (len) {
			//for best test efficiency  not included much more code in the while loop
			ddr_pld_cache(s);
				    ...
				    unsigned int pattern_frequency_setting)
{
	...
	for (; (size_loop < size_loop_max); ) {
		ddr_memcpy((void *)(uintptr_t)(test_init_start +
					      ((PATTERN_MATRIX_LOOP_SIZE) *
					       ((uintptr_t)pattern_frequency_setting)) *
					      (size_loop)),
			   (void *)(uintptr_t)test_init_start,
			  ...
			  unsigned int pattern_frequency_setting)
{
	...
	ddr_memcpy((void *)(uintptr_t)start_add, (void *)(uintptr_t)src_add, test_size);
			     ...
			     char read_write_enable, char ps)
{
	...
	uintptr_t reg_add_coarse = 0;
	uint32_t reg_add_coarse_bit_mask = 0;
	uintptr_t reg_add_fine = 0;
	uint32_t reg_add_fine_bit_mask = 0;
	uintptr_t add_base = 0;
	char sub_ch_mask0 = 2;
...
int do_read_s5_ddr_training_data(char log_level, ddr_set_ps0_only_t *ddr_set_t_p)
{
	...
	//for (loop = 0; loop < loop_max; loop += 4)
	//	wr_reg(((uintptr_t)(&ddr_set_t_p->cfg_board_SI_setting_ps) + loop),
...

	{
		...
		printf("\n sticky register: ");
		{
			...
			for (count = 0; count < loop_max; count += 4)
				...
				rd_reg((uintptr_t)((ddr_dmc_sticky[dmc_ddr_config_channel_id])) +
				count),
...

		{
			...
			for (count = 0; count < loop_max; ) {
				printf("\n%8x %8x", count,
				       rd_reg((uintptr_t)(ddr_set_t_p) + count));
				count = count + 4;
			}
		}
	}
...
int do_ddr_fastboot_config(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (p_ddr_base && p_ddr_base->ddr_dmc_lpdd4_retraining_address) {
	...
	//if (p_ddr_base->chip_id == MESON_CPU_MAJOR_ID_S5) {
	ddr_set_add = (uint32_t)(uintptr_t)(ddr_set_t_p_s5);
...
//
//			sha_t_chip_id =
//				(unsigned char *)((uintptr_t)(&ddr_sha_s5.sha_chip_id) - (44 * 2));
...
//#endif
	...
	{
		//printf("&ddr_sha_s5.ddrs : 0x%x\n", (uint32_t)(uintptr_t)&ddr_sha_s5.ddrs);
		//printf("&ddr_sha_s5.sha2 : 0x%x\n", (uint32_t)(uintptr_t)out_sha2);
		printf("ddr_set_add : 0x%x   sizeof(ddr_set_t):0x%x\n ",
		       (uint32_t)(uintptr_t)ddr_set_add, (uint32_t)(uintptr_t)ddr_set_size);
		printf("ddr_set_add_chip_id : 0x%x\n",
		       (uint32_t)(uintptr_t)(ddr_set_add + ddr_set_size));
		sha256_csum_wd_internal((unsigned char *)(uintptr_t)ddr_set_add,
...
//#if 1
	...
	sha_t_chip_id = (unsigned char *)((uintptr_t)(&ddr_sha_s5.sha_chip_id) -
					  (44 * 2));
	for (loop = 0; loop < MESON_CPU_CHIP_ID_SIZE; loop++)                 //update chip id
				  ...
				  uint32_t skip_window_test_enable)
{
	...
	ddr_set_add = (uint32_t)(uintptr_t)(ddr_set_t_p);
	ddr_set_size = sizeof(ddr_set_ps0_only_t);
	printf("\nddr_set_t_p=0x%8x\n", ddr_set_add);
...
//#if 1
	...
	sha_t_chip_id = (unsigned char *)((uintptr_t)(&ddr_sha_s5.sha_chip_id) - (44 * 2));
	for (loop = 0; loop < MESON_CPU_CHIP_ID_SIZE; loop++)         //update chip id
		sha_t_chip_id[loop] = global_chip_id[loop];
	//}
//#endif
	...
	if (ddr_set_t_p->cfg_board_common_setting.fast_boot[0] < 0xfe) {
		printf("\nuboot  auto fast boot  auto window test begin\n");
		{
			...
			{
				ddr_do_store_ddr_parameter_ops((uint8_t *)(uintptr_t)(ddr_set_add -
										     SHA_SUM_LEN),
							       write_size);
			}

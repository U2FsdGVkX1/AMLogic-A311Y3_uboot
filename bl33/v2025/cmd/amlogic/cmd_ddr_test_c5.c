...
#define MESON_CPU_MAJOR_ID_TXHD2			0x44
#endif
#ifdef MESON_CPU_MAJOR_ID_S1A
#undef MESON_CPU_MAJOR_ID_S1A
#define MESON_CPU_MAJOR_ID_S1A			0x45
#else
#define MESON_CPU_MAJOR_ID_S1A			0x45
#endif
#ifdef MESON_CPU_MAJOR_ID_S7
#undef MESON_CPU_MAJOR_ID_S7
#define MESON_CPU_MAJOR_ID_S7		0x46
#else
#define MESON_CPU_MAJOR_ID_S7			0x46
#endif
#ifdef MESON_CPU_MAJOR_ID_S7D
...
#define MESON_CPU_MAJOR_ID_S6			0x48
#endif
#ifdef MESON_CPU_MAJOR_ID_T6D
#undef MESON_CPU_MAJOR_ID_T6D
#define MESON_CPU_MAJOR_ID_T6D		0x49
#else
#define MESON_CPU_MAJOR_ID_T6D			0x49
#endif
#ifdef MESON_CPU_MAJOR_ID_A9
#undef MESON_CPU_MAJOR_ID_A9
#define MESON_CPU_MAJOR_ID_A9		0x4d
#else
#define MESON_CPU_MAJOR_ID_A9			0x4d
#endif
#ifdef MESON_CPU_MAJOR_ID_C5
#undef MESON_CPU_MAJOR_ID_C5
#define MESON_CPU_MAJOR_ID_C5		0x4F
#else
#define MESON_CPU_MAJOR_ID_C5		0x4F
#endif
...
//s1a
	{
		...
		.ddr_pctl_timing_end_address = ((0x00bb << 2) + 0xfe036400),    //DMC_DRAM_DFI
		.ddr_dmc_sticky0 = ((0x0000 << 2) + 0xfe036800),
		.ddr_pll_base_address = ((0x0000 << 2) + 0xfc00a000),
		//AM_DDR_PLL_CNTL0//
		//.ddr_boot_reason_address = ((0x00e1 << 2) + 0xfe010000),
		//SYSCTRL_SEC_STICKY_REG1,20210204,0xfe010384, confirm
		.ddr_dmc_lpdd4_retraining_address = ((0x0097 << 2) + 0xfe036400),
	...
	{
		.soc_family_name = "T6D",
		.chip_id = MESON_CPU_MAJOR_ID_T6D,
		.preg_sticky_reg0 = ((0x0000 << 2) + 0xfe036800),
		.ddr_phy_base_address = 0xfc000000,
		.ddr_dmc_base_address = 0xfe036000,
		.ddr_pctl_timing_base_address = ((0x0000 << 2) + 0xfe036400),   //DMC_DRAM_TRFC
	...
	{
		.soc_family_name = "A9",
		.chip_id = MESON_CPU_MAJOR_ID_A9,
		.preg_sticky_reg0 = ((0x0300  << 2) + 0xfe036000),
		.ddr_phy_base_address = 0xfc000000,
		.ddr_dmc_base_address = ((0x0000  << 2) + 0xfe036000),
		.ddr_pctl_timing_base_address = ((0x0101  << 2) + 0xfe036000),   //DMC_DRAM_TRFC
		.ddr_pctl_timing_end_address = ((0x01c0  << 2) + 0xfe036000),    //DMC_DRAM_DFI
		.ddr_dmc_sticky0 = ((0x0300  << 2) + 0xfe036000),
		.ddr_pll_base_address = ((0x0000 << 2) + 0xfe036c00),
		//AM_DDR_PLL_CNTL0//
		//.ddr_boot_reason_address = ((0x00e1 << 2) + 0xfe010000),
		...
		.ee_timer_base_address = ((0x003b << 2) + 0xfe010000),
		//SYSCTRL_TIMERE                             ((0x003b  << 2) + 0xfe010000)
		.ee_pwm_base_address = ((0x0001  << 2) + 0xffa10000),     //AO_PWM_PWM_B
		.ddr_dmc_apd_address = ((0x018c  << 2) + 0xfe036000),    //DMC_DRAM_APD_CTRL
		.ddr_dmc_asr_address = ((0x018d  << 2) + 0xfe036000),    //DMC_DRAM_ASR_CTRL
		.ddr_dmc_refresh_ctrl_address = ((0x0192  << 2) + 0xfe036000),
		// DMC_DRAM_REFR_CTRL ((0x0092 << 2) + 0xff638400)
...
#define DMC_TEST_WINDOW_INDEX_WCK_CK_RO         (0x20 + DMC_TEST_WINDOW_INDEX_WCK_CK)
...
#define DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY_RO    (0x20 + DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY)
#define DMC_TEST_WINDOW_INDEX_RX_CLK_C_DLY_RO    (0x20 + DMC_TEST_WINDOW_INDEX_RX_CLK_C_DLY)
...
//#define DMC_TEST_WINDOW_INDEX_SOC_VREF_RO         (0x20 + DMC_TEST_WINDOW_INDEX_SOC_VREF)
...
#define DMC_TEST_INDEX_TXDQDLY_WINDOW_RO   (0x20 + DMC_TEST_WINDOW_INDEX_TXDQDLY_WINDOW)
...
uint32_t ddr_cacl_phy_delay_all_step(char test_index, uint32_t value)
{
	...
	result = ((coarse_value << 6) + fine_value);
	return result;
...
uint32_t ddr_cacl_phy_over_ride_back_reg(char test_index, uint32_t value)
{
	...
	if (test_index == DMC_TEST_WINDOW_INDEX_ATXDLY) {
		...
		if (value > ((7 << 6) + 63))
			value = ((7 << 6) + 63);
		result = (value & 0x3f) + ((value >> 6) << 16);
	} else if (test_index == DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY ||
		...
		//	result = (value & 0x7f);
		//} else {
		//	result = 64 + (value & 0x3f) + (((value >> 6) - 1) << 16);
		//}
		// result = (value & 0x7f) + ((value >> 7) << 17);
	} else if (test_index == DMC_TEST_WINDOW_INDEX_RXENDLY) {
		if (value > ((31 << 6) + 63))
			value = ((31 << 6) + 63);
		result = (value & 0x3f) + ((value >> 6) << 16);
	} else if (test_index >= WINDOW_TEST_SOC_VREF_DAC0 &&
		   test_index <= WINDOW_TEST_SOC_VREF_DAC3) {
		if (value > 127)
			value = 127;
		result = value;
	}
...
int do_ddr_test_copy(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	size_count = (uint64_t)counter * test_size;
	time_start = get_us_time(); //us
	do {
		loop = 1;
		m_len = test_size / 4;
...
void ddr_memcpy(void *dst, const void *src, uint32_t len)
{
	...
	//debug for test L1 cache ,if only read write small area
#endif
	if (pre_fetch_enable) {
		while (len) {
			//for best test efficiency  not include much more code in the while loop
			ddr_pld_cache(s);
...
unsigned int cpu_ddr_test_init_pattern_generator(unsigned int add_offset)
{
	unsigned int pattern_select = 0;
	unsigned int pattern_value = 0;
	uint32_t matrix_x_select = 0;
	uint32_t matrix_y_select = 0;
	unsigned int pattern_value_temp_16 = 0;

	{
		{
			pattern_select = ((add_offset) % ((PATTERN_MATRIX_Y) * (PATTERN_MATRIX_X)));
			matrix_x_select = pattern_select / (PATTERN_MATRIX_Y);
			matrix_y_select = pattern_select % (PATTERN_MATRIX_Y);
			{
				if (matrix_x_select == 0)
					pattern_value = 0xaaaa5555;  //for 16 bit bus pattern

				if (matrix_x_select == 1)
					pattern_value = 0x0000ffff;  //for 16 bit bus pattern

				if (matrix_x_select == 2)
					pattern_value = 0;

				if (matrix_x_select > 2 && (matrix_x_select < (3 + 32)))
					pattern_value = 1 << (matrix_x_select - 3);
				if ((matrix_x_select > (2 + 32)) &&
				    (matrix_x_select < (3 + 32 + 16))) {
					//for 16 bit bus pattern
					pattern_value_temp_16 = (1 << (matrix_x_select - 3 - 32));
					pattern_value = pattern_value_temp_16 |
							((~pattern_value_temp_16) << 16);
				}
				if ((matrix_x_select > (2 + 32 + 16)) &&
				    (matrix_x_select < (3 + 32 + 16 + 17))) {
					//for dbi bus pattern  17 group
					pattern_value_temp_16 = (0x0f0f +
								 0xf0f * (matrix_x_select - 3 -
									  32 - 16));
					pattern_value = pattern_value_temp_16 |
							((~pattern_value_temp_16) << 16);
				}
			}
			if (matrix_y_select % 2)
...
void cpu_ddr_test_init_pattern_area(unsigned int test_init_start, unsigned int test_size,
				    unsigned int pattern_frequency_setting)
{
	if (pattern_frequency_setting == 0)
		pattern_frequency_setting = 1;  //for different  frequency pattern
	test_size = (test_size > ((PATTERN_MATRIX_LOOP_SIZE) * (pattern_frequency_setting))) ?
		    test_size : ((PATTERN_MATRIX_LOOP_SIZE) * (pattern_frequency_setting));
	unsigned int write_add = test_init_start;
	unsigned int size_loop = 0;
	unsigned int size_loop_max = 0;

	for (; (size_loop < ((PATTERN_MATRIX_LOOP_SIZE) * (pattern_frequency_setting))); ) {
		{
			write_add = (uint32_t)(size_loop + test_init_start);
			wr_reg((unsigned long)write_add,
			       cpu_ddr_test_init_pattern_generator((size_loop >> 2) /
								   pattern_frequency_setting));
			size_loop = size_loop + 4;
		}
	}
	size_loop = 1;
	size_loop_max = ((test_size /
			  (((PATTERN_MATRIX_LOOP_SIZE) * (pattern_frequency_setting)))) + 1);
	for (; (size_loop < size_loop_max); ) {
		ddr_memcpy((void *)(uint64_t)(test_init_start +
					      ((PATTERN_MATRIX_LOOP_SIZE) *
					       ((uint64_t)pattern_frequency_setting)) *
					      (size_loop)),
			   (void *)(uint64_t)test_init_start,
			   (uint32_t)((PATTERN_MATRIX_LOOP_SIZE) * (pattern_frequency_setting)));
		size_loop++;
	}
}
			  ...
			  unsigned int pattern_frequency_setting)
{
	...
	for (; size_loop < (test_size); ) {
		...
		pattern_value = (cpu_ddr_test_init_pattern_generator((size_loop >> 2) /
								     pattern_frequency_setting));
...
int do_cpu_ddr_test(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	unsigned int pattern_frequency_setting = 1;
	char *endp;
	...
	if (argc == 1) {
		...
		if (argc >= 7) {
			pattern_frequency_setting = simple_strtoull_ddr(argv[6], &endp, 0);
			if (pattern_frequency_setting == 0)
				pattern_frequency_setting = 1;
		}
		if (argc >= 8)
	...
	cpu_ddr_test_init_pattern_area(init_start_add, test_size, pattern_frequency_setting);
...
uint32_t get_bdlr_100step(uint32_t ddr_frequency)
{
	...
	if (p_ddr_base->chip_id == MESON_CPU_MAJOR_ID_S6 ||
		p_ddr_base->chip_id == MESON_CPU_MAJOR_ID_A9)
		dll_counter = ((rd_reg_ddr_phy(phy_base_add, 0x6434)) & 0x1ff);
	else
...
int do_ddr_test_pwm_bdlr(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	while (argc_count < argc) {
		para_meter[argc_count - 1] = simple_strtoull_ddr(argv[argc_count], &endp, 0);
			     ...
			     char read_write_enable, char ps)
{
	...
	if (index == DMC_TEST_WINDOW_INDEX_ATXDLY ||
	    index == DMC_TEST_WINDOW_INDEX_ATXDLY_RO) {
		if (sub_index >= 12)
			ac_reg_add_offset = 1; //s6
		reg_add_coarse = (add_base + ((ps << 16) | (0x5 << 12) |
					      (0x0 + (((0 +
							ac_reg_add_offset) >> 0) << 17))));
		reg_add_fine = add_base + ((ps << 16) | (0x5 << 12) |
					    (0x4 + (((((sub_index % 12) >> 2) << 2) +
						      ((ac_reg_add_offset >> 0) << 17)))));
		if (index == DMC_TEST_WINDOW_INDEX_ATXDLY_RO) {
			reg_add_coarse = (add_base + ((0 << 16) | (0x1 << 12) |
			((ac_reg_add_offset >> 0) << 17) | (0x4a0 + 0)));
			reg_add_fine = add_base + ((0 << 16) | (0x1 << 12) |
			((ac_reg_add_offset >> 0) << 17) |
			(0x4a4 + (((sub_index % 12) >> 2) << 2)));
		}
		reg_add_coarse_bit_mask = (~(3 << ((sub_index % 12) << 1)));
		reg_add_fine_bit_mask = (~(0x7f << (((sub_index % 12) & 0x3) << 3)));
	} else if (index == DMC_TEST_WINDOW_INDEX_TXDQSDLY ||
		   index == DMC_TEST_WINDOW_INDEX_TXDQSDLY_RO) {
		db = sub_index & 1;
		db_add_offset =	(sub_index >> 2);
		db_add_offset = db_add_offset ? 0x70 : 0;
		reg_add_coarse = (add_base +
				  ((ps << 16) | ((sub_index & 2) << 16) |
				   ((0x6 + db) << 12) |
				   (0x60 + (db_add_offset))));
		reg_add_fine = reg_add_coarse;
			//	(add_base +
			//	((ps << 16) |
			//	 ((0xa +
			//	   (sub_index & 0x3)) << 12) |
		...
		if (index == DMC_TEST_WINDOW_INDEX_TXDQSDLY_RO) {
			{
				db_add_offset = db_add_offset ? 0x50 : 0;
				reg_add_coarse =
					(add_base + ((0 << 16) | ((sub_index & 2) << 16) |
				   ((0x1 + (0 & 0x1)) << 12) |
				   (db << 8) |
				   (0x540 + (db_add_offset))));
				reg_add_fine = reg_add_coarse;
	...
	} else if (index == DMC_TEST_WINDOW_INDEX_WCK_CK ||
		   index == DMC_TEST_WINDOW_INDEX_WCK_CK_RO) {
		db = sub_index & 1;
		db_add_offset =	(sub_index >> 2);
		db_add_offset = db_add_offset ? 0x70 : 0;
		reg_add_coarse = (add_base +
				  ((ps << 16) | ((sub_index & 2) << 16) |
				   ...
				   (0x64 + (db_add_offset))));
		reg_add_fine = reg_add_coarse;
		if (index == DMC_TEST_WINDOW_INDEX_WCK_CK_RO) {
			db_add_offset = db_add_offset ? 0x50 : 0;
			reg_add_coarse =
				(add_base + ((0 << 16) | ((sub_index & 2) << 16) |
				((0x1 + (0 & 0x1)) << 12) |
		...
		reg_add_fine = (add_base +
				((0 << 16) | (0x1 << 12) |
				 (((sub_index & 0x3) >> (1 <<  disable_ch_1_3)) << 13) |
				 (0x9d4 + (((sub_index & 0x3) &
				 ((disable_ch_1_3 << 1) | 1)) << 8) +
				  ((sub_index >> 2) * (0x80)))));
		...
		if (index == DMC_TEST_WINDOW_INDEX_WDQDBI_RO) {
			reg_add_coarse = (add_base +
					  ((0 << 16) | (0x1 << 12) |
					  (((sub_index & 0x3) >> (1 <<  disable_ch_1_3)) <<
								       13) |
					   (0x528 +
					    (((sub_index & 0x3) &
					    ((disable_ch_1_3 << 1) | 1)) << 8) +
		...
		db = (sub_index % 36) / 9;
		//db = db & 1;
		reg_add_coarse = (add_base + ((ps << 16) | ((db & 2) << 16) |
					      ((0x6 + (db & 1)) << 12) |
					      (0x40 + ((sub_index / 36) ? (0x70) : 0))) +
				  (((sub_index % 9) >> 2) << 2));
		reg_add_fine = reg_add_coarse + 0x10;
		reg_add_coarse_bit_mask = (~(0x7 << (((sub_index % 9) & 0x3) << 3)));
		reg_add_fine_bit_mask =  (~(0x7f << (((sub_index % 9) & 0x3) << 3)));
		if (index == DMC_TEST_WINDOW_INDEX_TXDQDLY_RO) {
			reg_add_coarse = (add_base + ((0 << 16) | ((db & 2) << 16) |
					      ((0x1 + 0) << 12) |
					      (0x520 + ((sub_index / 36) ? (0x50) : 0))) +
						  ((db & 1) << 8) +
				  (((sub_index % 9) >> 2) << 2));
			reg_add_fine = reg_add_coarse + 0x10;
		}
	} else if (index == DMC_TEST_WINDOW_INDEX_TXDQDLY_WINDOW ||
		   index == DMC_TEST_INDEX_TXDQDLY_WINDOW_RO) {
		db = (sub_index % 36) / 9;
		//db = db & 1;
		reg_add_coarse = 0;
		reg_add_fine = (add_base + ((0 << 16) | ((db & 2) << 16) |
					      ((0x1 + 0) << 12) |
					      (0x9b0 + ((sub_index / 36) ? (0x80) : 0))) +
						  ((db & 1) << 3) +
				  (((sub_index % 9) >> 2) << 2));
		reg_add_coarse_bit_mask = 0;
		reg_add_fine_bit_mask = (~(0xff << (((sub_index % 9) & 0x3) << 3)));
		...
		reg_add_coarse_bit_mask = 0;
		reg_add_fine_bit_mask = (~(0xff << (((sub_index % 9) & 0x3) << 3)));
	} else if (index == DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY ||
		   index == DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY_RO) {
		db = (sub_index % 36) / 9;
		//db = db & 1;
		reg_add_coarse = (add_base + ((ps << 16) | ((db & 2) << 16) |
					      ((0x6 + (db & 1)) << 12) |
					      (0x10 + ((sub_index / 36) ? (0x70) : 0))) +
				  (((sub_index % 9) >> 0) << 2));
		reg_add_fine = reg_add_coarse + 0x0;
		reg_add_coarse_bit_mask = (~(0x7 << (((1 % 9) & 0x3) << 3)));
		reg_add_fine_bit_mask = (~(0xff << (((0 % 9) & 0x3) << 3)));;

		if (index == DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY_RO) {
			reg_add_coarse = (add_base +
					((0 << 16) | ((db & 2) << 16) |
					      ((0x1 + 0) << 12) |
					      (0x51c + ((sub_index / 36) ? (0x50) : 0))) +
						  ((db & 1) << 8) +
				  (((0 % 9) >> 2) << 2));
			reg_add_fine = (add_base +
					((0 << 16) | ((db & 2) << 16) |
					      ((0x1 + 0) << 12) |
					      (0x510 + ((sub_index / 36) ? (0x50) : 0))) +
						  ((db & 1) << 8) +
				  (((sub_index % 9) >> 2) << 2));
			reg_add_coarse_bit_mask = (~(0xf << (((sub_index % 9) & 0x7) << 2)));
			reg_add_fine_bit_mask = (~(0xff << (((sub_index % 9) & 0x3) << 3)));
		}

		//ddr_debug_serial_puts("DDR_debug", __FILE__, __LINE__);
	} else if (index == DMC_TEST_WINDOW_INDEX_RX_CLK_C_DLY ||
		   index == DMC_TEST_WINDOW_INDEX_RX_CLK_C_DLY_RO) {
		db = (sub_index % 36) / 9;
		//db = db & 1;
		reg_add_coarse = (add_base + ((ps << 16) | ((db & 2) << 16) |
					      ((0x6 + (db & 1)) << 12) |
				  ...
				  (((sub_index % 9) >> 0) << 2));
		reg_add_fine = reg_add_coarse + 0x0;
		reg_add_coarse_bit_mask = (~(0x7 << (((3 % 9) & 0x3) << 3)));
		reg_add_fine_bit_mask = (~(0xff << (((2 % 9) & 0x3) << 3)));;

		if (index == DMC_TEST_WINDOW_INDEX_RX_CLK_C_DLY_RO) {
			reg_add_coarse = (add_base +
					((0 << 16) | ((db & 2) << 16) |
					      ((0x1 + 0) << 12) |
					      (0x51c + ((sub_index / 36) ? (0x50) : 0))) +
		...
		db_add_offset =	(sub_index >> 2);
		//db = db & 1;
		reg_add_coarse = (add_base + ((ps << 16) | ((sub_index & 2) << 16) |
					      ((0x6 + (sub_index & 1)) << 12) |
					      (0x0 + (db_add_offset ? (0x70) : 0))) +
				  (((0 % 9) >> 2) << 2));
		reg_add_fine = reg_add_coarse;
		if (index == DMC_TEST_WINDOW_INDEX_RXENDLY_RO) {
			reg_add_coarse = (add_base + ((0 << 16) | ((sub_index & 2) << 16) |
					      ((0x1 + 0) << 12) |
					      (0x500 + (db_add_offset ? (0x50) : 0))) +
						  ((sub_index & 1) << 8) +
				  (((0 % 9) >> 2) << 2));
			reg_add_fine = reg_add_coarse;
				//	(add_base +
				//	((0 << 16) |
				//	 (0x1 << 12) |
				//	  ((disable_ch_1_3 ? 0 : (db & 2)) << 12) |
				//	 (0x500 + ((disable_ch_1_3 ? db : (db & 1)) << 8) +
				//	  (db_add_offset))));
		}
		reg_add_coarse_bit_mask = (~(0x1f << 0));
		reg_add_fine_bit_mask = (~(0x7f << 8));
		...
		if (index == DMC_TEST_WINDOW_INDEX_RXEN_WINDOW_DLY_RO) {
			reg_add_coarse = 0;
			reg_add_fine = (add_base + ((0 << 16) | ((sub_index & 2) << 16) |
					      ((0x1 + 0) << 12) |
					      (0x980 + (db_add_offset ? (0x80) : 0))) +
						  (0 << 8) +
				  (((0 % 9) >> 2) << 2));
		}
		reg_add_coarse_bit_mask = 0; // (~(0x1f << 0));
		reg_add_fine_bit_mask = (~(0xfff << (((sub_index & 1)) << 4)));
		...
		reg_add_coarse = 0;
		reg_add_fine =
		(add_base + ((ps << 16) | ((db & 2) << 16) |
					      ((0x6 + (db & 1)) << 12) |
					      (0x144)) +
				  (((sub_index % 9) >> 0) << 2));
		reg_add_fine_bit_mask = (~(0x7f << (((0 % 9) & 0x3) << 3)));
		if (index == WINDOW_TEST_SOC_VREF_DAC0_RO) {
			reg_add_coarse = 0;
			reg_add_fine = (add_base + ((ps << 16) | ((db & 2) << 16) |
					      ((0x1 + 0) << 12) |
					      (0x5a0)) +
						  ((db & 1) << 8) +
				  ((((sub_index % 9) >> 2) >> 0) << 2));
		...
		(add_base + ((ps << 16) | ((db & 2) << 16) |
					      ...
					      (0x144)) +
				  (((sub_index % 9) >> 0) << 2));
		reg_add_fine_bit_mask = (~(0x7f << (((1 % 9) & 0x3) << 3)));
		...
		reg_add_coarse_bit_mask = 0;
		   ...
		   WINDOW_TEST_SOC_VREF_DAC2 ||
		   index == WINDOW_TEST_SOC_VREF_DAC2_RO) {
		db = (sub_index % 36) / 9;
		//if (sub_index < 36) {
		//vref dq and dbi
		reg_add_coarse = 0;
		...
		(add_base + ((ps << 16) | ((db & 2) << 16) |
					      ...
					      (0x144)) +
				  (((sub_index % 9) >> 0) << 2));
		reg_add_fine_bit_mask = (~(0x7f << (((2 % 9) & 0x3) << 3)));
		if (index == WINDOW_TEST_SOC_VREF_DAC2_RO) {
			reg_add_coarse = 0;
			reg_add_fine = (add_base + ((ps << 16) | ((db & 2) << 16) |
					      ((0x1 + 0) << 12) |
					      (0x5a0)) +
		...
		reg_add_coarse_bit_mask = 0;
		   ...
		   WINDOW_TEST_SOC_VREF_DAC3 ||
		   index == WINDOW_TEST_SOC_VREF_DAC3_RO) {
		db = (sub_index % 36) / 9;
		//if (sub_index < 36) {
		//vref dq and dbi
		reg_add_coarse = 0;
		...
		(add_base + ((ps << 16) | ((db & 2) << 16) |
					      ...
					      (0x144)) +
				  (((sub_index % 9) >> 0) << 2));
		reg_add_fine_bit_mask = (~(0x7f << (((3 % 9) & 0x3) << 3)));
		if (index == WINDOW_TEST_SOC_VREF_DAC3_RO) {
			reg_add_coarse = 0;
			reg_add_fine = (add_base + ((ps << 16) | ((db & 2) << 16) |
					      ((0x1 + 0) << 12) |
					      (0x5a0)) +
	...
	if (reg_add_coarse) {
		delay_reg_coarse_value =
			(((rd_reg_ddr_phy(phy_base_add,
				   reg_add_coarse)) &
...
#if 0
	ddr_log_info(LOG_CHL_0, "\nindex sub_index reg_add_coarse fine= %x %x %x %x %x",
	index, sub_index,
	reg_add_coarse,
	reg_add_fine,
	delay_new_value);
	if (reg_add_coarse)
		ddr_log_info(LOG_CHL_0, "coarse value %x", rd_reg_ddr_phy(reg_add_coarse));
	if (reg_add_fine)
		ddr_log_info(LOG_CHL_0, " reg_add_fine value %x", rd_reg_ddr_phy(reg_add_fine));
	ddr_log_info(LOG_CHL_0, "\n");
#endif
	//time[1] = (get_time());
	//serial_puts("\ncost time ");
	//serial_put_dec(time[1] - time[0]);
	//time[0] = time[1];
				   ...
				   char read_write, uint32_t ps, void *input, char print)
{
	...
	char p_size = 1;
	char read_skip = 0;
	char over_ride_index_t = (over_ride_index & 0x1f);
...
//ddr_log_info(LOG_CHL_0, "efuse sipinfo[0]=%x\n", sipinfo[0]);
	if (over_ride_index_t == DMC_TEST_WINDOW_INDEX_ATXDLY) {
		...
		p_size = 2;
		count_max = 8;
	} else if (over_ride_index_t == DMC_TEST_WINDOW_INDEX_WCK_CK) {
		p_size = 2;
		count_max = 8;
	} else if (over_ride_index_t == DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY) {
		p_size = 2;
		count_max = 72;
	} else if (over_ride_index_t == DMC_TEST_WINDOW_INDEX_RX_CLK_C_DLY) {
		p_size = 2;
		count_max = 72;
	...
	//	count_max = 8;
		...
		p_size = 2;
		count_max = 72;
	} else if (over_ride_index_t ==
		   DMC_TEST_WINDOW_INDEX_TXDQDLY_WINDOW ||
		   over_ride_index_t ==
		   DMC_TEST_WINDOW_INDEX_RXDQDLY_WINDOW) {
		p_size = 1;
		count_max = 72;
		...
		p_size = 1;
		count_max = 36;
	} else if (over_ride_index_t == WINDOW_TEST_SOC_VREF_DAC2) {
		p_size = 1;
		count_max = 36;
	} else if (over_ride_index_t == WINDOW_TEST_SOC_VREF_DAC3) {
		p_size = 1;
		count_max = 36;
	} else if (over_ride_index_t ==
		   DMC_TEST_WINDOW_INDEX_RXEN_WINDOW_DLY) {
		p_size = 2;
		count_max = 8;
		...
		count_max = 36;
		   ...
		   (over_ride_index_t <=
		    DMC_TEST_WINDOW_INDEX_WDQDBI_WINDOW)) {
		p_size = 2;
		count_max = 72;         //8;
	} else if ((over_ride_index_t >=
		    DMC_TEST_WINDOW_INDEX_RXDQDBI_VREF0) &&
		   (over_ride_index_t <=
		    DMC_TEST_WINDOW_INDEX_RXDQDBI_VREF1)) {
		p_size = 1;
		count_max = 36;        //8;
	}
	for (t_count = 0; t_count < count_max; t_count++) {
		if (read_write == REGISTER_READ) {
			...
			if (over_ride_index_t ==
					WINDOW_TEST_SOC_VREF_DAC0_RO ||
					over_ride_index_t ==
					WINDOW_TEST_SOC_VREF_DAC1_RO ||
					over_ride_index_t ==
					WINDOW_TEST_SOC_VREF_DAC2_RO ||
					over_ride_index_t ==
					WINDOW_TEST_SOC_VREF_DAC3_RO) {
				if (t_count > 35)
					read_skip = 1;
				//else
				if ((t_count % 9) == 8)
			...
			if (over_ride_index == DMC_TEST_WINDOW_INDEX_RXDQDBI_RO) {
				over_ride_index_t = DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY_RO;
				if ((t_count % 9) == 8) {
					t_count_value =
...
{
	uint32_t ps = 0;
	board_phase_setting_ps_t *board_phase_setting_ps_cur;

	board_phase_setting_ps_cur = &p_ddrs->cfg_ddr_training_delay_ps;
	...
	//	&(board_phase_setting_ps_cur->write_wck_delay, print);

	ddr_read_write_training_value(p_ddrs, DMC_TEST_WINDOW_INDEX_RXCLKDLY, read_write, ps,
				      &board_phase_setting_ps_cur->read_dqs_delay[0], print);

	ddr_read_write_training_value(p_ddrs, DMC_TEST_WINDOW_INDEX_DRAM_VREF, read_write, ps,
				      &board_phase_setting_ps_cur->dram_vref[0], print);

	ddr_read_write_training_value(p_ddrs,
		DMC_TEST_WINDOW_INDEX_ATXDLY, read_write, ps,
		&board_phase_setting_ps_cur->ac_trace_delay[0], print);
	ddr_read_write_training_value(p_ddrs,
	DMC_TEST_WINDOW_INDEX_TXDQSDLY, read_write, ps,
		&board_phase_setting_ps_cur->write_dqs_delay[0], print);
	ddr_read_write_training_value(p_ddrs,
	DMC_TEST_WINDOW_INDEX_WCK_CK, read_write, ps,
		&board_phase_setting_ps_cur->write_wck_delay[0], print);
	ddr_read_write_training_value(p_ddrs,
		DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY, read_write, ps,
		&board_phase_setting_ps_cur->read_dq_delay_t[0], print);
	ddr_read_write_training_value(p_ddrs,
	 DMC_TEST_WINDOW_INDEX_RX_CLK_C_DLY, read_write, ps,
		&board_phase_setting_ps_cur->read_dq_delay_c[0], print);
	//ddr_read_write_training_value(p_ddrs,
	//	DMC_TEST_WINDOW_INDEX_RXCLKDLY, read_write, ps,
	//	&board_phase_setting_ps_cur->read_dqs_delay[0], print);
	ddr_read_write_training_value(p_ddrs,
		DMC_TEST_WINDOW_INDEX_TXDQDLY, read_write, ps,
		&board_phase_setting_ps_cur->wdq_delay[0], print);
	ddr_read_write_training_value(p_ddrs,
		DMC_TEST_WINDOW_INDEX_RXENDLY, read_write, ps,
		&board_phase_setting_ps_cur->read_dqs_gate_delay[0], print);
	ddr_read_write_training_value(p_ddrs,
		WINDOW_TEST_SOC_VREF_DAC0, read_write, ps,
		&board_phase_setting_ps_cur->soc_bit_vref0[0], print);
	ddr_read_write_training_value(p_ddrs,
		WINDOW_TEST_SOC_VREF_DAC1, read_write, ps,
		&board_phase_setting_ps_cur->soc_bit_vref1[0], print);
	ddr_read_write_training_value(p_ddrs,
		WINDOW_TEST_SOC_VREF_DAC2, read_write, ps,
		&board_phase_setting_ps_cur->soc_bit_vref2[0], print);
	ddr_read_write_training_value(p_ddrs,
		WINDOW_TEST_SOC_VREF_DAC3, read_write,ps,
		&board_phase_setting_ps_cur->soc_bit_vref3[0], print);
	//	ps = ps + 1;
	//}
...
int do_ddr2pll_g12_cmd(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	pll = simple_strtoull(argv[1], &endp, 0);
	if (*argv[1] == 0 || *endp != 0) {
	...
	while (argc_count < argc) {
		para_meter[argc_count - 1] = simple_strtoull(argv[argc_count], &endp, 0);
		if (*argv[argc_count] == 0 || *endp != 0)
			para_meter[argc_count - 1] = 0;
		argc_count++;
	}
		      ...
		      uint32_t	offset_value)
{
	...
	//if (over_ride_index == DMC_TEST_WINDOW_INDEX_ATXDLY)
#if 1 //def ENABLE_DDR_WINDOW_TEST_READ_TC_SAME_TIME
	char	over_ride_index2 = 0;

	if (over_ride_index == DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY ||
		over_ride_index == DMC_TEST_WINDOW_INDEX_RX_CLK_C_DLY) {
		over_ride_index2 =
		(over_ride_index == DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY) ?
		DMC_TEST_WINDOW_INDEX_RX_CLK_C_DLY :
		DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY;
		for (temp_count_3 = 0; temp_count_3 < offset_value; temp_count_3++)
			ddr_reg_increase_sub(over_ride_index2,
					((over_ride_sub_index)),
					over_ride_increase_decrease, 1);
		}
#endif
...
int do_ddr_s5_offset_data(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	uint32_t dmc_retraining_ctrl = 0;
	uint32_t dmc_refresh_ctrl_vt = 0;

	if (p_ddr_base->ddr_dmc_refresh_ctrl_address) {
		dmc_refresh_ctrl_vt = rd_reg((p_ddr_base->ddr_dmc_refresh_ctrl_address));
		wr_reg((p_ddr_base->ddr_dmc_refresh_ctrl_address),
		dmc_refresh_ctrl_vt & (~(1 << 3)));
	}
	if (p_ddr_base->ddr_dmc_lpdd4_retraining_address) {
		dmc_retraining_ctrl = rd_reg((p_ddr_base->ddr_dmc_lpdd4_retraining_address));
		wr_reg((p_ddr_base->ddr_dmc_lpdd4_retraining_address),
...
#define  G12_DATA_WRITE_OFFSET_MAX   (0X3F + 7 * 32)
	...
	if (test_index == DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY) //DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY
		count_max = 36 * 2;
	//	lcdlr_max = 255;
	if (test_index == DMC_TEST_WINDOW_INDEX_RX_CLK_C_DLY) //DMC_TEST_WINDOW_INDEX_RX_CLK_T_DLY
		count_max = 36 * 2;
	...
	if (test_index == WINDOW_TEST_SOC_VREF_DAC2) {
		count_max = 36 * 1;
		//	lcdlr_max = 0x3f;
		//printf(" soc vref rank0 and rank1 share vref dac\n");
	}
	if (test_index == WINDOW_TEST_SOC_VREF_DAC3) {
		count_max = 36 * 1;
		//	lcdlr_max = 0x3f;
		//printf(" soc vref rank0 and rank1 share vref dac\n");
	}
	...
	for (; count < count_max; count++) {
		...
		if (count > 63)
			if (test_dq_mask_3 & (1 << (count % 32)))
				continue;
		for (int sub_offset = 0;sub_offset < offset_value; sub_offset++) {
			if (p_ddr_base->ddr_dmc_refresh_ctrl_address) {
				wr_reg((p_ddr_base->ddr_dmc_refresh_ctrl_address),
				dmc_refresh_ctrl_vt & (~(1 << 3)));
			}
	...
	if (p_ddr_base->ddr_dmc_lpdd4_retraining_address)
		wr_reg(p_ddr_base->ddr_dmc_lpdd4_retraining_address, dmc_retraining_ctrl);
	if (p_ddr_base->ddr_dmc_refresh_ctrl_address)
		wr_reg(p_ddr_base->ddr_dmc_refresh_ctrl_address, dmc_refresh_ctrl_vt);
	//if ((p_ddr_base->chip_id == MESON_CPU_MAJOR_ID_T3)) {
	dmc_ddr_config_channel_id = 0;
	dmc_change_channel(dmc_ddr_config_channel_id);
	//}
...
//#endif
	...
	ret = store_rsv_read("ddr-parameter", ((sizeof(ddr_sha_t_s5) + 511) / 512) * 512,
		       (uint8_t *)(temp_buf));
...
int do_ddr_dfs_test(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	while (count < ddr_dfs_test_times) {
		...
		error_cur = do_ddr_test((cmd_tbl_t *)cmdtp, 1, (int)argc, (argv));
...
int do_ddr_asr_test(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	unsigned int asr_random_test_enable1 = 0;
	unsigned int asr_random_test_enable2 = 0;
	...
	if (argc > 4) {
		count = 3;
		asr_random_test_enable1	= simple_strtoull_ddr(argv[count + 1], &endp, 0);
		if (*argv[count + 1] == 0 || *endp != 0)
			asr_random_test_enable1 = 0;
	}
	if (argc > 5) {
		count = 4;
		asr_random_test_enable2 = simple_strtoull_ddr(argv[count + 1], &endp, 0);
		if (*argv[count + 1] == 0 || *endp != 0)
			asr_random_test_enable2 = 0;
	}
	...
	while (count < ddr_asr_test_times) {
		...
		if (asr_cur_fre_sel == 0)
			...
			asr_new_value = 0;
		if (asr_random_test_enable1) {
			//ddr_udelay(10);
			// coverity[DC.WEAK_CRYPTO]
			val = (rand() % 1500000000) + 1;
			ddr_udelay(val);
		}
		wr_reg(asr_address, asr_new_value);
		wr_reg(asr_address1, asr_new_value);
		if (asr_random_test_enable2) {
			// coverity[DC.WEAK_CRYPTO]
			val = (rand() % 1500000000) + 1;
			ddr_udelay(val);
		}
		//run_command("ddrtest 3", 0);
		argc2 = argc - 1;
		error_cur = do_ddr_test((cmd_tbl_t *)cmdtp, 1, (int)argc2, (argv));
		error_total = error_total + error_cur;

...
int ReadBackupGeometry(char *superbuf, LpMetadataGeometry* geometry) {
	...
	memcpy(buffer,
		superbuf + LP_PARTITION_RESERVED_BYTES + LP_METADATA_GEOMETRY_SIZE,
				   ...
				   LpMetadataGeometry *geometry, int slot_number)
{
	...
	memcpy(header, superbuf + GetPrimaryMetadataOffset(geometry, slot_number),
	...
	u32 valid_attributes = LP_PARTITION_ATTRIBUTE_MASK_V0;
	...
	for (i = 0; i < header->partitions.num_entries; i++) {
		...
		if (partition.first_extent_index + partition.num_extents <
		...
		strlcpy(node->name, partition.name,
			min(sizeof(PartitionList), sizeof(partition.name)));
		if (ishead == 0) {
			part_list = node;
			part_list->next = NULL;
			tail = node;

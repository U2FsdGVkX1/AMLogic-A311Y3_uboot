...
/* for t5m,t6d,t6w */
#define TOP_EDID_RAM_OVR0_DATA	(0xfe398000 + (0x016 << 2))
...
{
	int ret = 0;
	char *hdmi_5v_wakeup_val_str = NULL;
	unsigned int hdmi_5v_wakeup_val;

	//Get hdmirx 5v wakeup status from dtb
	ret = run_command("fdt get value rx_5v_wake_up_value /hdmirx rx_5v_wake_up_en", 0);
	if (ret) {
		printf("get cmd buf error\n");
		return 0;
	}
	hdmi_5v_wakeup_val_str = env_get("rx_5v_wake_up_value");
	if (!hdmi_5v_wakeup_val_str) {
		printf("get 5v wakeup value fail\n");
	} else {
		hdmi_5v_wakeup_val = ustrtoul(hdmi_5v_wakeup_val_str, NULL, 16);
		printf("5v wakeup value = %d\n", hdmi_5v_wakeup_val);
	}
	if (hdmi_5v_wakeup_val) {
		if (IS_ENABLED(CONFIG_MESON_T6D))
			writel(0x1, TOP_EDID_RAM_OVR0_DATA);
	}
	return 0;
}
...
int hdmirx_read_edid(const char *filename,
					...
					int *pedid_size)
{
	/* compile error,use driver edid,dont need this function */
	return 0;
	...
	//if (!filename || !pedid_buf || !pedid_size)
		//return -1;
	//if (!ini_is_file_exist(filename)) {
		//printf("%s, file \"%s\" is not exist!\n", __func__, filename);
		//return -1;
	//}

	//tmp_size = ini_get_file_size(filename);
	//if (tmp_size <= 0) {
	...
	//if (tmp_buf) {
		//rd_cnt = ini_read_file_to_buffer(filename, 0,  tmp_size, tmp_buf);
		//if (rd_cnt <= 0) {
			//free(tmp_buf);
				//return -1;
		//}

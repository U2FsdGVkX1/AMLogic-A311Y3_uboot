...
static void qms_scene_pre_process(struct hdmitx_dev *hdev)
{
	...
	if (!reboot_mode) {
		...
		if ((!strcmp(reboot_mode, "factory_reset")) || (!strcmp(reboot_mode, "update")))
			recovery_mode = true;
	}
	...
	/* recovery mode not support qms */
	if (env_get("qms_en") && (env_get_ulong("qms_en", 10, 0) == 1) && !recovery_mode)
		env_qms_en = 1;
	else
		return;
...
static int _do_output(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (strcmp(argv[1], "list") == 0) {
		...
		if (!hdev->pxp_mode) {
			if (!hdmitx_edid_check_data_valid(hdev->RXCap.edid_check, hdev->rawedid)) {
...
static int do_output(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	ret = _do_output(cmdtp, flag, argc, argv);
	hdmi_mode_frac_postprocess();
	return ret;
}
...
static int do_clkmsr(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (hdev->chip_type == MESON_CPU_ID_S5) {
		...
		clk_msr(95);
		return CMD_RET_SUCCESS;
	} else if (hdev->chip_type == MESON_CPU_ID_A9) {
		clk_msr(49);
		clk_msr(50);
		clk_msr(51);
		clk_msr(52);
...
static int do_debug(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	unsigned char enc_index;
	unsigned char clk_source_idx;
	int pixel_ivctx, line_ivctx;
	struct hdmitx_dev *hdev = get_hdmitx21_device();
	...
	} else if (strncmp(argv[1], "vpfdet", 6) == 0 ) {
		hdmitx_dump_vpfdet();
	} else if (strncmp(argv[1], "clk_source_idx", 14) == 0 ) {
		clk_source_idx = strtoul(argv[2], NULL, 10);
		hdev->clk_source_idx = clk_source_idx;
		printf("select clk_source_idx: %d", hdev->clk_source_idx);
	} else if (strncmp(argv[1], "venc", 4) == 0 ) {
		enc_index = strtoul(argv[1] + 4, NULL, 16);
		hdev->enc_idx = enc_index;
		printf("select enc index: %d\n", hdev->enc_idx);
	} else if (strncmp(argv[1], "pix_capture", 11) == 0 ) {
		pixel_ivctx = strtoul(argv[2], NULL, 10);
		line_ivctx = strtoul(argv[3], NULL, 10);
		pr_info("pix_capture x:%d y:%d\n", pixel_ivctx, line_ivctx);
		pix_capture_dump(hdev, pixel_ivctx, line_ivctx);
	}
 ...
 */
	...
	if (prxcap->video_capability_data)
		printf("Video_Capability_Data: 0x%x\n", prxcap->video_capability_data);
...
static void get_parse_edid_data(struct hdmitx_dev *hdev)
{
	...
	hdmitx_config_i2c_rate(DDC_I2C_50K);
	hdev->hwop.read_edid(hdev->rawedid);
	if (hdmitx_edid_is_all_zeros(hdev->rawedid)) {
		pr_info("First read edid all 0 data\n");
		hdev->hwop.read_edid(hdev->rawedid);
	}

	/* If EDID is not correct at first time, then retry */
	if (hdmitx_edid_check_data_valid(hdev->RXCap.edid_check, hdev->rawedid) == false) {
		hdmitx_config_i2c_rate(DDC_I2C_25K);
		pr_info("config i2c 25k and read edid again\n");
		msleep(20);
...
static int _do_get_parse_edid(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	struct hdmitx_dev *hdev = get_hdmitx21_device();
	...
	/*
	 * The hdmitx driver gets the value of env too early in hdmitx21_init.
	 * The value of emmc's env partition has not been loaded yet.
	 * Therefore, the value obtained at this time is the data saved at compile time,
	 * which is the default value defined in board.h.
	 * To prevent this case, need to get the value of env later
	 */
	char *dongle_mode = env_get("dongle_mode");
	char *dsc_policy = env_get("dsc_policy");
	char *edid_check = env_get("edid_check");
	char tmp_sel_hdmimode[MESON_MODE_LEN] = {0};
	...
	printf("[%s][%d] dongle_mode=[%d], dsc_policy=[%d], edid_check=[%d]\n", __func__, __LINE__,
	       hdev->dongle_mode, txcap->dsc_policy, hdev->RXCap.edid_check);
	...
	if (!no_manual_output) {
		...
		if (is_mode_name_frac(hdmimode)) {
			convert_name_frac2int(hdmimode, tmp_sel_hdmimode);
		} else {
			strncpy(tmp_sel_hdmimode, hdmimode, MESON_MODE_LEN - 1);
			tmp_sel_hdmimode[MESON_MODE_LEN - 1] = '\0';
		}
		para = hdmitx21_get_fmtpara(tmp_sel_hdmimode, colorattribute);
		 ...
		 * that the user selected format is over-written by policy(for example:
	 ...
	 * with uboot policy.
	 * 4.user selected mode is over written by system policy
	 */
	...
	hdmi_mode_frac_preprocess();
	memset(tmp_sel_hdmimode, 0, ARRAY_SIZE(tmp_sel_hdmimode));
	if (is_mode_name_frac(sel_hdmimode))
		convert_name_frac2int(sel_hdmimode, tmp_sel_hdmimode);
	else
		memcpy(tmp_sel_hdmimode, sel_hdmimode, sizeof(tmp_sel_hdmimode));
	hdev->para = hdmitx21_get_fmtpara(tmp_sel_hdmimode, env_get("colorattribute"));
	hdev->vic = hdev->para->timing.vic;
	/*
...
{
	return _do_get_parse_edid(cmdtp, flag, argc, argv);
}
...
static void vout_hdmi_hpd(struct hdmitx_dev *hdev, int hpd_st)
{
	...
	if (!hpd_st) {
		...
		if (env_get("outputmode")) {
			...
			printf("ERROR %s: outputmode env is NULL\n", __func__);
			env_set("outputmode", "1080p60hz");
		}
		switch (hdev->enc_idx) {
		case 0:
...
int do_hpd_detect(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (env_get_yesno("hdmitx_hpd_bypass") == 1) {
...
#ifdef	CONFIG_MAX_BOOT_TIME
	if (!hpd_st)
		env_set_ulong("hpd_detect", i * 100 * 1000);
#endif

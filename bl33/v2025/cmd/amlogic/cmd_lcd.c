...
static int do_lcd_power(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	unsigned int val = 0;
	...
	aml_lcd_driver_power_ctrl(0, val);
...
static int do_lcd_dump(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
...
static int do_lcd_tcon(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (strcmp(argv[1], "reg") == 0) {
		...
		if (pdrv->tcon_global_reset)
			pdrv->tcon_global_reset(pdrv);
		else
			printf("no lcd tcon_global_reset\n");
	} else {
		ret = -1;
	}
	return ret;
...
static int do_lcd_window(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	window_attr.pos.start_x =
			(unsigned short)simple_strtoul(argv[1], NULL, 10);
	window_attr.pos.start_y =
			(unsigned short)simple_strtoul(argv[2], NULL, 10);
	window_attr.pos.end_x =
			(unsigned short)simple_strtoul(argv[3], NULL, 10);
	window_attr.pos.end_y =
			(unsigned short)simple_strtoul(argv[4], NULL, 10);
	window_attr.color_in.red =
			(unsigned short)simple_strtoul(argv[5], NULL, 10);
	window_attr.color_in.green =
			(unsigned short)simple_strtoul(argv[6], NULL, 10);
	window_attr.color_in.blue =
			(unsigned short)simple_strtoul(argv[7], NULL, 10);
	window_attr.color_out.red =
			(unsigned short)simple_strtoul(argv[8], NULL, 10);
	window_attr.color_out.green =
			(unsigned short)simple_strtoul(argv[9], NULL, 10);
	window_attr.color_out.blue =
			(unsigned short)simple_strtoul(argv[10], NULL, 10);
	aml_lcd_driver_window(0, &window_attr);
	return 0;
}

static int do_lcd_cursor(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	struct lcd_cursor_attr_s cursor_attr;

	if (argc != 8 && argc != 2)
		return -1;
	cursor_attr.status = (unsigned char)simple_strtoul(argv[1], NULL, 10);
	if (cursor_attr.status && argc != 8)
		return -1;
	if (argc == 8) {
		cursor_attr.mode = (unsigned char)simple_strtoul(argv[2], NULL, 10);
		cursor_attr.x = (unsigned short)simple_strtoul(argv[3], NULL, 10);
		cursor_attr.y = (unsigned short)simple_strtoul(argv[4], NULL, 10);

		cursor_attr.red = (unsigned short)simple_strtoul(argv[5], NULL, 10);
		cursor_attr.green = (unsigned short)simple_strtoul(argv[6], NULL, 10);
		cursor_attr.blue = (unsigned short)simple_strtoul(argv[7], NULL, 10);
	}
	aml_lcd_driver_cursor(0, &cursor_attr);
	return 0;
}
...
static int do_lcd_mem(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (strcmp(argv[1], "add") == 0) {
		...
		if (argc < 3)
			goto do_lcd_mem_err;
		name = argv[2];
		ret = lrm_get_by_name(name, &pa, &size);
		if (ret) {
			printf("error: %s: not find\n", name);
			return 0;
		}
		lrm_free(NULL, pa);
	} else if (strcmp(argv[1], "get") == 0) {
		...
		vaddr = (unsigned char *)lrm_phys_to_virt(pa, size);
		printf("data addr: 0x%x:", (unsigned int)pa);
		for (i = 0; i < size; i += 16) {
			printf("%04x:", i);
			for (j = 0; j < 16; j++) {
				if ((i + j) >= size)
					break;
				printf(" %02x", vaddr[i + j]);
			}
			printf("\n");
		}
		...
		if (argc < 3)
			goto do_lcd_mem_err;
		if (strcmp(argv[2], "show") == 0) { //read from param_mem
			aml_lcd_panel_mem_debug(NULL);
		} else if (strcmp(argv[2], "get") == 0) { //read from param_mem
			if (argc < 4)
				goto do_lcd_mem_err;
			name = argv[3];
			aml_lcd_panel_mem_debug(name);
		} else if (strcmp(argv[2], "test") == 0) { //read from lrm
			if (argc < 4) {
				aml_lcd_panel_param_test(NULL);
			} else {
				name = argv[3];
				aml_lcd_panel_param_test(name);
			}
		}
	} else if (strcmp(argv[1], "bootargs") == 0) {
		if (argc >= 3) {
			name = argv[2];
			vaddr = lrm_bootargs_get_data(name, &size);
			if (vaddr) {
				for (i = 0; i < size; i += 16) {
					printf("%04x:", i);
					for (j = 0; j < 16; j++) {
...
do_lcd_mem_err:
	printf("format: lcd mem [cmd] [param...]\n"
		"help:\n"
		"lcd mem update: save panel mem data to lrm\n"
		"lcd mem show: dump lrm data info\n"
		"lcd mem add size name: alloc lrm mem for param 'name'\n"
		"lcd mem add_tail: alloc lrm tail_mem for param 'name'\n"
		"lcd mem rm name: remove lrm mem for param 'name'\n"
		"lcd mem get name: dump data of param 'name' in lrm\n"
		"lcd mem param show: list param in panel param_mem\n"
		"lcd mem param get name: dump data of param 'name' in panel param_mem\n"
		"lcd mem bootargs: list bootargs in panel bootargs_mem\n"
		"lcd mem bootargs name: dump data of bootargs 'name' in panel bootargs_mem\n");
...
static int do_lcd_bl(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (strcmp(argv[1], "on") == 0) {
		...
		if (argc == 3) {
			level = (int)simple_strtol(argv[2], NULL, 10);
...
static int do_lcd1_power(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	unsigned int val = 0;
	...
	aml_lcd_driver_power_ctrl(1, val);
...
static int do_lcd1_dump(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
...
static int do_lcd1_window(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	struct lcd_window_attr_s window_attr;
	...
	window_attr.color_out.blue =
			(unsigned short)simple_strtoul(argv[10], NULL, 10);
	aml_lcd_driver_window(1, &window_attr);
	return 0;
}

static int do_lcd1_cursor(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	struct lcd_cursor_attr_s cursor_attr;
	...
	aml_lcd_driver_cursor(1, &cursor_attr);
	return 0;
}
...
static int do_lcd2_power(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	unsigned int val = 0;
	...
	aml_lcd_driver_power_ctrl(2, val);
...
static int do_lcd2_dump(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
...
static int do_mipi_dsi_cmd(int index, cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (!strcmp(argv[1], "LPCD") || !strcmp(argv[1], "LPRX")) {
		aml_lcd_mipi_dsi_dphy_test(index, 0x20);
		return 0;
	}
...
static cmd_tbl_t cmd_lcd_sub[] = {
	...
	U_BOOT_CMD_MKENT(window,  3, 0, do_lcd_window,   "", ""),
	U_BOOT_CMD_MKENT(check,   2, 0, do_lcd_check,    "", ""),
	U_BOOT_CMD_MKENT(cursor,  4, 0, do_lcd_cursor,   "", ""),
...
static cmd_tbl_t cmd_lcd1_sub[] = {
	...
	U_BOOT_CMD_MKENT(window,  3, 0, do_lcd1_window,   "", ""),
	U_BOOT_CMD_MKENT(check,   2, 0, do_lcd1_check,   "", ""),
	U_BOOT_CMD_MKENT(cursor,  4, 0, do_lcd1_cursor,   "", ""),
...
U_BOOT_CMD(
	...
	"lcd dump         - show lcd parameters and registers\n"
...
U_BOOT_CMD(
	...
	"lcd1 dump         - show lcd parameters and registers\n"
...
U_BOOT_CMD(
	...
	"lcd2 dump         - show lcd parameters and registers\n"

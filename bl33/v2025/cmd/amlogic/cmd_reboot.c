...
int do_get_rebootmode (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	u32 bit_mask;
#ifdef CONFIG_REBOOT_VIA_RESET_PIN_STICKY
	...
	stick_reboot_flag = get_stick_reboot_flag_mbx();
...
#endif
		case AMLOIGC_AOCPU_WATCHDOG_REBOOT:
		{
			printf("aocpu trigger wdt\n");
			env_set("reboot_mode", "aocpu_watchdog");
			break;
		}
#ifdef AMLOIGC_RTC_QUIESCENT_REBOOT
		case AMLOIGC_RTC_QUIESCENT_REBOOT:
		{
			env_set("reboot_mode", "rtc_quiescent");
			break;
		}
#endif
		default:
...
int do_reboot (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (argc <= 1) {
		...
		} else if (strcmp(mode, "quiescent") == 0) {
			reboot_mode_val = AMLOGIC_QUIESCENT_REBOOT;

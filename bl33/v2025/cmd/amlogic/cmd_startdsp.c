 ...
 */
#define HIFI_CLK_MUXA_B_SEL_BIT		BIT(31)
#define HIFI_CLK_SEL_MASK		(0x7)
#define HIFI_CLK_DIV_MASK		(0x7F)
#define HIFI_CLK_MUXB_CLK_SEL_SHIFT	(25)
#define HIFI_CLK_MUXB_CLK_SEL_MASK	(HIFI_CLK_SEL_MASK << HIFI_CLK_MUXB_CLK_SEL_SHIFT)
#define HIFI_CLK_MUXB_GATE_BIT		BIT(24)
#define HIFI_CLK_MUXB_CLK_DIV_SHIFT	(16)
#define HIFI_CLK_MUXB_CLK_DIV_MASK	(HIFI_CLK_DIV_MASK << HIFI_CLK_MUXB_CLK_DIV_SHIFT)
#define HIFI_CLK_MUXA_CLK_SEL_SHIFT	(9)
#define HIFI_CLK_MUXA_CLK_SEL_MASK	(HIFI_CLK_SEL_MASK << HIFI_CLK_MUXA_CLK_SEL_SHIFT)
#define HIFI_CLK_MUXA_GATE_BIT		BIT(8)
#define HIFI_CLK_MUXA_CLK_DIV_SHIFT	(0)
#define HIFI_CLK_MUXA_CLK_DIV_MASK	(HIFI_CLK_DIV_MASK << HIFI_CLK_MUXA_CLK_DIV_SHIFT)
...
int set_dsp_clk(uint32_t id, uint32_t freq_sel)
{
	...
	case 8:
		clk_sel = 3;
		clk_div = 1;
		dsp_clk_tag = "fclk_div5/2:200MHz";
		break;
	case 9:
		clk_sel = 3;
		clk_div = 3;
	...
	control |= (HIFI_CLK_MUXA_GATE_BIT | HIFI_CLK_MUXB_GATE_BIT);
	writel(control, addr);

	if (control & HIFI_CLK_MUXA_B_SEL_BIT) {
		control &= ~((HIFI_CLK_DIV_MASK << HIFI_CLK_MUXA_CLK_DIV_SHIFT) |
				(HIFI_CLK_SEL_MASK << HIFI_CLK_MUXA_CLK_SEL_SHIFT));
		control |= (clk_div << HIFI_CLK_MUXA_CLK_DIV_SHIFT) |
				(clk_sel << HIFI_CLK_MUXA_CLK_SEL_SHIFT);
		writel(control, addr);
		control &= ~HIFI_CLK_MUXA_B_SEL_BIT;
		writel(control, addr);
	} else {
		control &= ~((HIFI_CLK_DIV_MASK << HIFI_CLK_MUXB_CLK_DIV_SHIFT) |
				(HIFI_CLK_SEL_MASK << HIFI_CLK_MUXB_CLK_SEL_SHIFT));
		control |= (clk_div << HIFI_CLK_MUXB_CLK_DIV_SHIFT) |
				(clk_sel << HIFI_CLK_MUXB_CLK_SEL_SHIFT);
		writel(control, addr);
		control |= HIFI_CLK_MUXA_B_SEL_BIT;
		writel(control, addr);
	}
...
U_BOOT_CMD(
	...
	"arg[3]: dsp clk set\n 1:800M 2:667M 3:24M 4:500M 5:400M 6:333M 7:250M 8:200M 9:100M 10:245.76M\n"
);

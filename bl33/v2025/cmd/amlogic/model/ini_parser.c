...
static void ini_memnd_alloc_recode(const char *fun_name, const char *var_name, int size, int type)
{
	...
	ALOGD("[mem]: size: %s%d, total size: %d: %s: %s%s%s\n",
...
{
	ALOGD("ini parse mem alloc size: %d\n\n", ini_mem_alloc_size);
}
...
static int get_section_total_size(struct ini_s *ini_buf, struct ini_section_s *psec)
{
	int pos, size;
			...
			struct ini_line_s *pline, const char *new_value, unsigned int set_mode)
{
	...
	tmp_mem = malloc(ini_buf->total_size);
	if (!tmp_mem) {
		ALOGE("%s: tmp_mem error\n", __func__);
		return -1;
	}
	memset(tmp_mem, 0, ini_buf->total_size);
	...
	if (set_mode == INI_SET_KEY_VAL_MODE_OVERWRITE) { //overwrite new_value
		...
		strlcpy(tmp_value, new_value, new_len);
		//update line attr
		...
		strlcpy((tmp_value + pre_len - 1), new_value, new_len);
		//update line attr
	...
	//restore cur_mem
	memcpy(local_ini_mem + ini_buf->mem_start_pos, tmp_mem + tmp_inip->mem_start_pos, ini_buf->mem_size);
			...
			const char *new_line, const char *new_value, int line_cnt)
{
	...
	memset(tmp_mem, 0, ini_buf->total_size);
	tmp_inip = (struct ini_s *)tmp_mem;
	memcpy(tmp_mem, local_ini_mem, sizeof(struct ini_s));
	tmp_inip->mem_cur_pos = 0;
	...
	while (pre_pline) {
		...
		for (i = 0; i < line_cnt; i++) {
			if (strcmp(pre_name, token_line) == 0) { //target line, overwrite value
				...
				strlcpy(tmp_value, token_value, tmp_size);
...
{
	struct ini_s *tmp_inip;
	struct ini_section_s *tmp_psec, *target_psec = NULL;
	struct ini_line_s *tmp_pline, *cur_pline, *new_pline;
	unsigned char *tmp_mem;
	const char *cur_sec_name, *new_sec_name;
	char *tmp_str, *target_str;
	int offset, cur_size, new_size, tmp_pos, str_size;
	int ret = 0, err_no = 0;
	unsigned char *local_ini_mem = (unsigned char *)cur_ini_buf;

	if (!cur_ini_buf || !new_ini_buf || !new_psec)
		return -1;

	tmp_mem = malloc(cur_ini_buf->total_size);
	if (!tmp_mem) {
		ALOGE("%s: tmp_mem error\n", __func__);
		return -1;
	}
	memset(tmp_mem, 0, cur_ini_buf->total_size);
	tmp_inip = (struct ini_s *)tmp_mem;
	memcpy(tmp_mem, local_ini_mem, sizeof(struct ini_s));
	tmp_inip->mem_cur_pos = 0;

	new_sec_name = pos_to_str(new_ini_buf, new_psec->name_pos);
	if (!cur_psec) {
		ALOGI("%s: new_section %s append at last\n", __func__, new_sec_name);
		//save cur all sections
		tmp_pos = cur_ini_buf->mem_cur_pos; //real mem size
		memcpy(tmp_mem + tmp_inip->mem_start_pos,
		       local_ini_mem + cur_ini_buf->mem_start_pos, tmp_pos);
		tmp_inip->mem_cur_pos = tmp_pos;
	} else {
		cur_sec_name = pos_to_str(cur_ini_buf, cur_psec->name_pos);
		if (strcmp(cur_sec_name, new_sec_name)) {
			ALOGI("%s: new_section %s append after cur_section %s\n",
			      __func__, new_sec_name, cur_sec_name);

			tmp_pos = section_to_pos(cur_ini_buf, cur_psec) +
				get_section_total_size(cur_ini_buf, cur_psec);
			memcpy(tmp_mem + tmp_inip->mem_start_pos,
			       local_ini_mem + cur_ini_buf->mem_start_pos, tmp_pos);
			tmp_inip->mem_cur_pos = tmp_pos;
		} else {
			ALOGI("%s: new_section %s overwrite\n", __func__, new_sec_name);
			tmp_pos = section_to_pos(cur_ini_buf, cur_psec);
			//save front sections
			memcpy(tmp_mem + tmp_inip->mem_start_pos,
			       local_ini_mem + cur_ini_buf->mem_start_pos, tmp_pos);
		...
		target_psec = pos_to_section(cur_ini_buf, cur_psec->next);
	}

	tmp_psec = pos_to_section(tmp_inip, tmp_pos);
	if (!tmp_psec) {
		err_no = __LINE__;
		ret = -1;
		goto ini_write_new_section_end;
	}

	//save new section, update global offset for section and each line
	new_size = new_psec->local_size;
	if ((tmp_pos + new_size) > tmp_inip->mem_size) {
		err_no = __LINE__;
		ret = -1;
		goto ini_write_new_section_end;
	}
	memcpy((void *)tmp_psec, new_psec, new_size);
	tmp_pos += new_size;
	tmp_inip->mem_cur_pos = tmp_pos;
	//save new section name
	str_size = strlen(new_sec_name) + 1; //include '\0'
	tmp_psec->name_pos = tmp_pos;
	tmp_str = pos_to_str(tmp_inip, tmp_psec->name_pos);
	strlcpy(tmp_str, new_sec_name, str_size);
	tmp_pos += str_size;
	tmp_inip->mem_cur_pos = tmp_pos;

	tmp_pline = NULL;
	new_pline = pos_to_line(new_ini_buf, new_psec->first_line);
	while (new_pline) {
		if (!tmp_pline)
			tmp_psec->first_line = tmp_pos;
		else
			tmp_pline->next = tmp_pos;
		tmp_pline = pos_to_line(tmp_inip, tmp_pos);
		if (!tmp_pline) {
			err_no = __LINE__;
			ret = -1;
			goto ini_write_new_section_end;
		}
		new_size = new_pline->local_size;
		if ((tmp_pos + new_size) > tmp_inip->mem_size) {
			err_no = __LINE__;
			ret = -1;
			goto ini_write_new_section_end;
		}
		memcpy(tmp_pline, new_pline, new_size);
		offset = line_to_pos(new_ini_buf, new_pline) - line_to_pos(tmp_inip, tmp_pline);
		tmp_pline->name_pos -= offset;
		tmp_pline->value_pos -= offset;
		tmp_pos += new_size;
		tmp_inip->mem_cur_pos = tmp_pos;

		new_pline = pos_to_line(new_ini_buf, new_pline->next);
	}

	if (!target_psec) //cur_section is the last section
		goto ini_write_new_section_ok;

	//save behind sections
	//need trave lines to update each name_pos and value_pos
	target_psec = pos_to_section(cur_ini_buf, cur_psec->next);
	while (target_psec) {
		tmp_psec->next = tmp_pos;
		tmp_psec = pos_to_section(tmp_inip, tmp_psec->next);
		if (!tmp_psec) {
			err_no = __LINE__;
			ret = -1;
		...
		cur_size = target_psec->local_size;
		if ((tmp_pos + cur_size) > tmp_inip->mem_size) {
			err_no = __LINE__;
			ret = -1;
			goto ini_write_new_section_end;
		}
		memcpy((void *)tmp_psec, target_psec, cur_size);
		tmp_pos += cur_size;
		tmp_inip->mem_cur_pos = tmp_pos;
		//save section name
		target_str = pos_to_str(cur_ini_buf, target_psec->name_pos);
		str_size = strlen(target_str) + 1; //include '\0'
		tmp_psec->name_pos = tmp_pos;
		tmp_str = pos_to_str(tmp_inip, tmp_psec->name_pos);
		strlcpy(tmp_str, target_str, str_size);
		tmp_pos += str_size;
		tmp_inip->mem_cur_pos = tmp_pos;

		tmp_pline = NULL;
		cur_pline = pos_to_line(cur_ini_buf, target_psec->first_line);
		while (cur_pline) {
			if (!tmp_pline)
				tmp_psec->first_line = tmp_pos;
			else
			...
			cur_size = cur_pline->local_size;
			if ((tmp_pos + cur_size) > tmp_inip->mem_size) {
				err_no = __LINE__;
				ret = -1;
				goto ini_write_new_section_end;
			}
			memcpy((void *)tmp_pline, cur_pline, cur_size);
			offset = line_to_pos(cur_ini_buf, cur_pline) - line_to_pos(tmp_inip, tmp_pline);
			tmp_pline->name_pos -= offset;
			tmp_pline->value_pos -= offset;
			...
			cur_pline = pos_to_line(cur_ini_buf, cur_pline->next);
		}

		target_psec = pos_to_section(cur_ini_buf, target_psec->next);
	}

ini_write_new_section_ok:
	offset = tmp_inip->mem_cur_pos - cur_ini_buf->mem_start_pos;
	ini_memnd_alloc_recode(__func__, new_sec_name, offset, INI_MEM_SECTION);
	ALOGI("%s: new_section %s write ok\n", __func__, new_sec_name);

	//restore cur_mem
	memcpy(local_ini_mem + cur_ini_buf->mem_start_pos, tmp_mem + tmp_inip->mem_start_pos,
	       cur_ini_buf->mem_size);
	cur_ini_buf->mem_cur_pos = tmp_inip->mem_cur_pos;

ini_write_new_section_end:
	memset(tmp_mem, 0, cur_ini_buf->total_size);
	free(tmp_mem);
	if (ret)
		ALOGE("%s: %d\n", __func__, err_no);
			...
			const char *name, const char *value)
{
	...
	if (!str) {
		ALOGE("%s: %s: name_pos error\n", __func__, name);
		return NULL;
	}
	strlcpy(str, name, str_size);
	tmp_pos += str_size;
...
static struct ini_section_s *ini_new_section(struct ini_s *ini_buf, const char *section)
{
...
static int ini_multi_line_str_store(char *buf, char *str, int max_len)
{
	...
	if (strlen(str) >= max_len) {
		ALOGE("%s: out of max_len %d: %s\n", __func__, max_len, str);
		return 0;
	}
	len = snprintf(buf, max_len, "%s", str);
...
int ini_parse_mem(struct ini_s *ini_buf, const char *tmp_buf)
{
	...
	line = (char *)malloc(INI_LINE_LEN_MAX);
	if (!line)
		return -1;
	memset(line, 0, INI_LINE_LEN_MAX);

	multi_line.max_len = INI_MULTI_LINE_LEN_MAX;
	multi_line.buf = (char *)malloc(multi_line.max_len);
	if (!multi_line.buf) {
		free(line);
		return -1;
	}
	memset(multi_line.buf, 0, multi_line.max_len);
	multi_line.valid = 0;
	multi_line.cur_pos = 0;
	...
	while (1) {
		...
		if (char_cnt > INI_LINE_LEN_MAX) {
			ALOGE("%s: line %d char_cnt %d out of support\n",
	...
	while (psec) {
		...
		while (pline) {
			...
			if (value_size >= 510) {
				...
				for (i = 0; i < n; i++) {
					j = i * 510;
					strncpy(str, value + j, 510);
					str[510] = '\0';
					printf("%s\n", str);
				}
				if (m) {
					j = n * 510;
					strncpy(str, value + j, m);
					str[m] = '\0';
					printf("%s\n", str);
				}
			} else {

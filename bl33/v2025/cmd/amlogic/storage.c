...
#ifdef BL2_CONFIG
int store_is_fastboot(void)
{
	if (BL2_CONFIG & BL2_FASTBOOT)
		return 1;
	else
		return 0;
}
...
int store_ddr_save_fastboot_para(size_t size, void *buf)
{
	...
	return store->ddr_save_para ? store->ddr_save_para(size, buf) : 0;
}
...
int store_send_param(u32 mem_dtb)
{
	struct storage_t *store = store_get_current();
	...
	return store->param_ops ? store->param_ops(mem_dtb) : 0;
}
			  ...
			  int flag, int argc, char * const argv[])
{
	...
	if (s != NULL && strcmp(s, ".chip") == 0) {
		...
		size = (u32)simple_strtoul(argv[argc - 1], NULL, 16);
...
#endif
	time = get_timer(0);
	ret = store->read(name, offset, size, (u_char *)(uintptr_t)addr);
	time = get_timer(time);

	if (size != 0)
		printf("%u %s ", size, (argc == 6) ? "bytes" : "blocks");

	printf("read in %u ms", time);
	if ((time > 0) && (size != 0)) {
		puts(" (");
		print_size(div_u64(size, time) * 1000, "/s");
		puts(")");
...
#endif
	time = get_timer(0);
	ret = store->write(name, offset, size, (u_char *)(uintptr_t)addr);
	time = get_timer(time);

	if (size != 0)
		printf("%u %s ", size, (argc == 6) ? "bytes" : "blocks");

	printf("write in %u ms", time);
	if ((time > 0) && (size != 0)) {
		puts(" (");
		print_size(div_u64(size, time) * 1000, "/s");
		puts(")");

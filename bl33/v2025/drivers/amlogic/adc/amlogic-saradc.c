...
#define SARADC_REG4_ADC_RESET			BIT(25)		/* reset analog (a9 only) */
...
#define WORKAROUND_ID_NONE			0
#define WORKAROUND_ID_S7D			1
#define WORKAROUND_ID_A9			2
...
static int amlogic_saradc_parse_dt(struct udevice *dev)
{
	...
	if (device_is_compatible(dev, "amlogic,s7d-saradc"))
		priv->workaround = WORKAROUND_ID_S7D;
	else if (device_is_compatible(dev, "amlogic,a9-saradc"))
		priv->workaround = WORKAROUND_ID_A9;
...
static int amlogic_saradc_start_sample(struct amlogic_saradc *priv)
{
	...
	if (priv->workaround == WORKAROUND_ID_S7D) {
		udelay(200);
		...
		if (ret)
			goto failed;
	} else if (priv->workaround == WORKAROUND_ID_A9) {
		udelay(200);
		clrsetbits_le32(priv->base + SARADC_REG4, SARADC_REG4_ADC_RESET, 0);
		udelay(10);
		clrsetbits_le32(priv->base + SARADC_REG4, SARADC_REG4_ADC_RESET,
				SARADC_REG4_ADC_RESET);
	}

...
bool self;
int table_test_mask;
static struct amfc_cmd_list test_acl1  __attribute__((aligned(64)));
static struct amfc_cmd_list test_acl2  __attribute__((aligned(64)));
#endif

#define HW_VERSION_10		0x100
#define HW_VERSION_11		0x101
#define HW_VERSION_12		0x102
...
int amfc_init(void)
{
	...
	case MESON_CPU_MAJOR_ID_C5:
		// 666MHz
		writel(0 | (1 << 8) | (4 << 9), CLKCTRL_AMFC_CLK_CTRL);
		break;
...
int amfc_decompress(void *src, void *dst, ssize_t src_size, ssize_t dst_size, int stream)
{
	...
	if (log_en)
		printf("%s, acl:%p, src:%p, dst:%p, src size:%ld, dst size:%ld\n",
			__func__, &acl, src, dst, (long)src_size, (long)dst_size);
	/* setup command list */
	...
	if (!(status & AMFC_ERROR_MASK)) {
		...
		if (ret <= 0) {
			...
			dump_addr((void *)(long)acl.src_addr, 32);
			dump_addr((void *)(long)acl.dst_addr, 32);
			writel(0x03, AMFC_GL_CMD1_IRQCLR);
		}
	} else {
		printf("decompress failed:%x %x, time:%d us, acl:%p, src:%p, dst:%p, src_size:%ld, dst_size:%ld, timeout:%d\n",
			...
			&acl, src, dst, (long)src_size, (long)dst_size, timeout);
...
/*
 * pack file test for performance and data consistent.
...
static void amfc_reset_test(unsigned long cnt)
{
	char *p_enc_src, *p_enc_dst, *p_dec_src, *p_dec_dst;
	int enc_src_size, enc_dst_size, dec_src_size, dec_dst_size;
	int i = 0;
	int cmd_time;
	int timeout, old_log;
	...
	old_log = log_en;
	p_enc_src = (char *)0x01000000;
	p_enc_dst = (char *)0x02000000;
	p_dec_src = (char *)0x03000000;
	p_dec_dst = (char *)0x04000000;

	enc_src_size = dec_dst_size = enc_dst_size = 0x01000000;
	timeout = ((enc_src_size + PAGE_SIZE) / PAGE_SIZE) * 100 + 5000;

	for (i = 0; i < p_enc_dst - p_enc_src; i++) {
		p_enc_src[i] = i & 0xff;
	}
	log_en = 0;
	enc_time = get_time();
	dec_src_size = amfc_compress(p_enc_src, p_dec_src, enc_src_size, enc_dst_size);
	enc_time = get_time() - enc_time;
	if (dec_src_size < 0)
		printf("build test buffer failed\n");
	else
		printf("source compressed to size:%d\n", dec_src_size);

	dec_time = get_time();
	amfc_decompress(p_dec_src, p_dec_dst, dec_src_size, dec_dst_size, 0);
	dec_time = get_time() - dec_time;
	log_en = old_log;
	printf("enc time:%ld, dec_time:%ld, loop:%ld\n", enc_time, dec_time, cnt);

	i = 0;
	while (i < cnt) {
		/* build 2 descriptors 0 for enc, 1 for dec */
		// case 1, cmd0 work and reset cmd 1 with random delay
		memset(&test_acl1, 0, sizeof(test_acl1));
		memset(&test_acl2, 0, sizeof(test_acl2));
		test_acl1.src_size = enc_src_size;
		test_acl1.dst_size = enc_dst_size;
		set_up_addr(&test_acl1, (unsigned long)p_enc_src, ADDR_SRC);
		set_up_addr(&test_acl1, (unsigned long)p_enc_dst, ADDR_DST);
		test_acl1.algorithm = ALGORITHM_ZSTD;
		test_acl1.end       = 1;
		test_acl1.compress  = CMD_COMPRESS;
		test_acl1.owner     = 1;
		test_acl1.status    = 0xffffffff;

		flush_dcache_range((unsigned long)p_enc_src, (unsigned long)p_enc_src + enc_src_size);
		flush_dcache_range((unsigned long)&test_acl1, (unsigned long)&test_acl1 + sizeof(acl));
		invalidate_dcache_range((unsigned long)p_enc_dst, enc_dst_size);

		test_acl2.src_size = dec_src_size;
		test_acl2.dst_size = dec_dst_size;
		set_up_addr(&test_acl2, (unsigned long)p_dec_src, ADDR_SRC);
		set_up_addr(&test_acl2, (unsigned long)p_dec_dst, ADDR_DST);
		test_acl2.algorithm = ALGORITHM_ZSTD;
		test_acl2.end       = 1;
		test_acl2.compress  = CMD_DECOMPRESS;
		test_acl2.owner     = 1;
		test_acl2.status    = 0xffffffff;

		flush_dcache_range((unsigned long)p_dec_src, (unsigned long)p_dec_src + dec_src_size);
		flush_dcache_range((unsigned long)&test_acl2, (unsigned long)&test_acl2 + sizeof(acl));
		invalidate_dcache_range((unsigned long)p_dec_dst, dec_dst_size);

		/* sw reset */
		writel(0x80000000, AMFC_GL_CMD1_CONTROL);
		writel(0x03, AMFC_GL_CMD1_IRQCLR);
		writel((unsigned long)&test_acl2 >> ADDR_SHIFT, AMFC_GL_CMD1_DESC_BASE0_ADDR);

		/* SW reset */
		writel(0x80000000, AMFC_GL_CMD0_CONTROL);
		writel(0x03, AMFC_GL_CMD0_IRQCLR);
		writel((unsigned long)&test_acl1 >> ADDR_SHIFT, AMFC_GL_CMD0_DESC_BASE0_ADDR);

		/* trigger 2 cmd */
		writel(1, AMFC_GL_CMD0_CONTROL);
		writel(1, AMFC_GL_CMD1_CONTROL);

		delay = rand() % (enc_time + 1);
		udelay(delay);

		/* reset cmd 1 and check if cmd0 will timeout? */
		writel(0x80000000, AMFC_GL_CMD1_CONTROL);
		...
		while (1) {
			status = readl(AMFC_GL_CMD0_STATUS);
			if ((status & IRQ_DONE)) {
				printf("%s, cmd 1 reset while cmd 0 is working, test pass, delay:%d, loop:%d\n", __func__, delay, i);
				break;
			}
			if ((status & IRQ_ERR)) {
				printf("%s, cmd 1 reset while cmd 0 is working, error occur, delay:%d, loop:%d\n", __func__, delay, i);
				dump_addr((void *)&test_acl1, 32);
				fail0++;
				break;
			}
			if (get_time() - cmd_time > timeout) {
				printf("%s, time out, cmd1 reset while cmd 0 working test failed, delay:%d, loop:%d\n", __func__, delay, i);
				dump_addr((void *)&test_acl1, 32);
				fail0++;
				break;
			}
		}
		invalidate_dcache_range((unsigned long)&test_acl1, (unsigned long)&test_acl1+ sizeof(acl));

		// case 2, cmd1 work and reset cmd 0 with random delay
		memset(&test_acl1, 0, sizeof(test_acl1));
		memset(&test_acl2, 0, sizeof(test_acl2));
		test_acl1.src_size = enc_src_size;
		test_acl1.dst_size = enc_dst_size;
		...
		delay = rand() % (dec_time + 1);
		udelay(delay);

		/* reset cmd 0 and check if cmd1 will timeout? */
		writel(0x80000000, AMFC_GL_CMD0_CONTROL);
		...
		while (1) {
			status = readl(AMFC_GL_CMD1_STATUS);
			if ((status & IRQ_DONE)) {
				printf("%s, cmd 0 reset while cmd 1 is working, test pass, delay:%d, loop:%d\n", __func__, delay, i);
				break;
			}
			if ((status & IRQ_ERR)) {
				printf("%s, cmd 0 reset while cmd 1 is working, error occur, delay:%d, loop:%d\n", __func__, delay, i);
				dump_addr((void *)&test_acl2, 32);
				dump_addr((void *)AMFC_GL_VERSION, AMFC_SECURE_TOP_REG - AMFC_GL_VERSION + 4);
				fail1++;
				break;
			}
			if (get_time() - cmd_time > timeout) {
				printf("%s, time out, cmd 0 reset while cmd 1 working test failed, delay:%d, loop:%d\n", __func__, delay, i);
				dump_addr((void *)&test_acl2, 32);
				fail1++;
				break;
			}
		}
		invalidate_dcache_range((unsigned long)&test_acl2, (unsigned long)&test_acl2+ sizeof(acl));
		i++;
	}
	printf("%s, faile:%d %d, loop:%d\n", __func__, fail0, fail1, i);
}

static void amfc_stream_auto_reset_test(void)
{
	char *p_enc_src, *p_enc_dst, *p_dec_src, *p_dec_dst;
	int enc_src_size, enc_dst_size, dec_src_size, dec_dst_size;
	int i = 0;
...
//	test_acl1.stream    = 1;
	...
	test_acl2.src_size = dec_src_size;
	test_acl2.dst_size = 10000;
	set_up_addr(&test_acl2, (unsigned long)p_dec_src, ADDR_SRC);
	set_up_addr(&test_acl2, (unsigned long)p_dec_dst, ADDR_DST);
	test_acl2.algorithm = ALGORITHM_ZSTD;
	test_acl2.end       = 1;
	test_acl2.stream    = 1;		// setup stream
	test_acl2.compress  = CMD_DECOMPRESS;
	test_acl2.owner     = 1;
	test_acl2.status    = 0xffffffff;
	...
	cmd_time = get_time();
	/* wait cmd1 for done or error */
	while (1) {
		status = readl(AMFC_GL_CMD1_STATUS);
		if ((status & IRQ_DONE)) {
			if (hw_version < HW_VERSION_12)
				writel(0x80000000, AMFC_GL_CMD1_CONTROL);
			break;
		}
		if ((status & IRQ_ERR)) {
			printf("%s, cmd 1 stream error occur\n", __func__);
			break;
		}
		if (get_time() - cmd_time > timeout) {
			printf("%s, time out, cmd1 stream timeout\n", __func__);
			break;
		}
	}
	...
	while (1) {
		status = readl(AMFC_GL_CMD0_STATUS);
		if ((status & IRQ_DONE)) {
			printf("%s, cmd 1 stream while cmd 0 is working, test pass\n", __func__);
			break;
		}
		if ((status & IRQ_ERR)) {
			printf("%s, cmd 1 stream while cmd 0 is working, error occur\n", __func__);
			break;
		}
		if (get_time() - cmd_time > timeout) {
			printf("%s, time out, cmd1 stream while cmd 0 working test failed\n", __func__);
			break;
		}
	}
	invalidate_dcache_range((unsigned long)&test_acl1, (unsigned long)&test_acl1+ sizeof(acl));
...
const unsigned test_data[]={
	...
	0x00000000 , 0x00000000 , 0x00000000 , 0x00000000,
	0x00000000 , 0x00000000 , 0x00000000 , 0x00000000,
	0xb5280000 , 0x1040fd2f , 0x006cff00 , 0x30302000,
	0x0002300a , 0x40f167f4 , 0x003c4400 , 0x00010000,
	0x805fa7fd , 0x0000003c , 0xf7fd0001 , 0x003c1001,
	0x00010000 , 0x1001f7fd , 0x0000003c , 0xf7fd0001,
	0x003c1001 , 0x00010000 , 0x1001f7fd , 0x0000003c,
	0xf7fd0001 , 0x003c1001 , 0x00010000 , 0x1001f7fd,
	0x0000003c , 0xf7fd0001 , 0x003c1001 , 0x00010000,
	0x1001f7fd , 0x0000003c , 0xf7fd0001 , 0x003c1001,
	0x00010000 , 0x1001f7fd , 0x0000003c , 0xf7fd0001,
	0x003c1001 , 0x00010000 , 0x1001f7fd , 0x0000003c,
	0xf7fd0001 , 0x003d1001 , 0x00010000 , 0x1001f7fd,
	0x00000000 , 0x00000000 , 0x00000000 , 0x00000000
};

static void amfc_page_boundary_test(void)
{
	unsigned char *p_src, *p_dst;
	int src_size, dst_size, ret;

	p_src = (unsigned char *)0x01000000;
	p_dst = (unsigned char *)0x02000000;
	src_size = 4096;
	dst_size = 65600;
	memcpy(p_src, test_data, sizeof(test_data));

	page_table_mode = 1;
	ret = amfc_decompress(p_src, p_dst, src_size, dst_size, 0);
	if (ret < 0) {
		printf("%s, test failed, ret:%d\n", __func__, ret);
	} else {
		printf("%s, test pass, ret:%d\n", __func__, ret);
	}
}
#endif
...
static int do_amfc(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (!strcmp(argv[1], "stream_auto_reset_test")) {
		amfc_stream_auto_reset_test();
		return 0;
	}
	if (!strcmp(argv[1], "page_bound_test")) {
		amfc_page_boundary_test();
		return 0;
	}
#endif

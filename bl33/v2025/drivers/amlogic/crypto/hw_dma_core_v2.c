...
#endif
#ifdef AES_TEST_CASE
	aes_dma_time = get_time();
#endif
	while (*P_DMA_STS0 == 0)
		;
#ifdef AES_TEST_CASE
	aes_dma_time = get_time() - aes_dma_time;
#endif
...
#ifdef CRYPTO_DEBUG
	printf("*P_DMA_STS0 = %x\n", *P_DMA_STS0);
	for (i = 0; i < sizeof(dsc) / sizeof(struct dma_dsc_v2); i++) {

...
enum _KmUsrKeyType{
    KM_USER_KEY_TYPE_MAC        = 0 ,//key format is all ascii, and split by :
...
static int _read_key_in_type_mac(const char* keyname, char* databuf, const unsigned bufLen, char* decryptBuf)
{
    ...
    if (_UsrMacKeyLen > bufLen) {
        KM_ERR("mac len %u is invalid, must be %d\n", bufLen, _UsrMacKeyLen);
        return -__LINE__;
    }
    ...
    for (index = 0; index < keyDevSz; ++index)
    {
        ...
        n += snprintf(&databuf[n], 2, "%02x:", byteSum);
        if (n > bufLen) {
            break;
        }
    }
...
static int _burn_key_in_type_sha1(const char* keyname, void* databuf, const unsigned bufLen, char* decryptBuf)
{
    ...
    sha1_csum_wd((u8*)srcKeyVal, licLen, genSum, 512);
    ...
    if (ret) {
        ...
        for (byteIndex = 0; byteIndex < shaSumLen; ++byteIndex)
        {
            ...
            org_n += snprintf(&org_sha1Str[org_n], 2, "%02x",  orgSum[byteIndex]);
            gen_n += snprintf(&gen_sha1Str[gen_n], 2, "%02x",  genSum[byteIndex]);
...
int key_manage_total_size(u32 *retval)
{
    key_unify_total_size(retval);
    return 0;
}
...
static int do_keyman_total_size(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    u32 totalsize;

    if (argc < 1) return CMD_RET_USAGE;

    key_manage_total_size(&totalsize);
    KM_MSG("key total size is %d\n", totalsize);
...
static cmd_tbl_t cmd_keyman_sub[] = {
    ...
    U_BOOT_CMD_MKENT(totalsize,     1, 0, do_keyman_total_size, "", ""),
};

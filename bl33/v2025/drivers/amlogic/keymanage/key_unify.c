    ...
    else
    {
        ...
        *fmtStr = '\0';
        for (; i < dataLen; ++i, ++pdata)
        {
            ...
            n += snprintf(&fmtStr[n], 2, "%02x",  *pdata);
            if (n > fmtSize) {
...
int key_unify_read(const char *keyname, void* keydata, const unsigned bufLen)
{
    ...
    const u32 keySz = theDevOps->pGetSize(keyname);
    if (keySz > bufLen && bufLen) {
        KM_ERR("keySz[%u] > bufLen[%d]\n", keySz, bufLen);
        return __LINE__;
    }
...
int key_unify_total_size(u32 *retval)
{
	keymanage_secukey_total_size(retval);
	return 0;
}
...
U_BOOT_CMD(
	...
	"keyunify write keyname data <len>  ---- write key data. no len if data is ascii str\n"
...
int keymanage_secukey_total_size(u32 *retval) { return -EINVAL; }

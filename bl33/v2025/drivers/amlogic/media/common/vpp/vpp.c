	...
	int mtx_on)
{
	...
	if (mtx_sel == VPP_OSD1_MTX) {
		...
		vpp_reg_setb(matrix_en_ctrl, mtx_on, 0, 1);
	} else if (mtx_sel == VPP_OSD3_MTX) {
		matrix_coef00_01 = VPP_WRAP_OSD3_MATRIX_COEF00_01;
		matrix_coef02_10 = VPP_WRAP_OSD3_MATRIX_COEF02_10;
		matrix_coef11_12 = VPP_WRAP_OSD3_MATRIX_COEF11_12;
		matrix_coef20_21 = VPP_WRAP_OSD3_MATRIX_COEF20_21;
		matrix_coef22 = VPP_WRAP_OSD3_MATRIX_COEF22;
		matrix_offset0_1 = VPP_WRAP_OSD3_MATRIX_OFFSET0_1;
		matrix_offset2 = VPP_WRAP_OSD3_MATRIX_OFFSET2;
		matrix_pre_offset0_1 = VPP_WRAP_OSD3_MATRIX_PRE_OFFSET0_1;
		matrix_pre_offset2 = VPP_WRAP_OSD3_MATRIX_PRE_OFFSET2;
		matrix_en_ctrl = VPP_WRAP_OSD3_MATRIX_EN_CTRL;
...
static void vpp_top_post2_matrix_yuv2rgb(int vpp_top)
{
	...
	if (vpp_top == 0) {
		...
		vpp_reg_write(VPP1_MATRIX_PRE_OFFSET0_1 + offset_vpp1,
			(((m[0] >> 2) & 0xfff) << 16) | ((m[1] >> 2) & 0xfff));
		vpp_reg_write(VPP1_MATRIX_PRE_OFFSET2 + offset_vpp1,
			(m[2] >> 2) & 0xfff);
		vpp_reg_write(VPP1_MATRIX_COEF00_01 + offset_vpp1,
			(((m[3] >> 2) & 0x1fff) << 16) | ((m[4] >> 2) & 0x1fff));
		vpp_reg_write(VPP1_MATRIX_COEF02_10 + offset_vpp1,
			(((m[5] >> 2) & 0x1fff) << 16) | ((m[6] >> 2) & 0x1fff));
		vpp_reg_write(VPP1_MATRIX_COEF11_12 + offset_vpp1,
			(((m[7] >> 2) & 0x1fff) << 16) | ((m[8] >> 2) & 0x1fff));
		vpp_reg_write(VPP1_MATRIX_COEF20_21 + offset_vpp1,
			(((m[9] >> 2) & 0x1fff) << 16) | ((m[10] >> 2) & 0x1fff));
		vpp_reg_write(VPP1_MATRIX_COEF22 + offset_vpp1,
			(m[11] >> 2) & 0x1fff);

		vpp_reg_write(VPP1_MATRIX_OFFSET0_1 + offset_vpp1,
			(((m[18] >> 2) & 0xfff) << 16) | ((m[19] >> 2) & 0xfff));
		vpp_reg_write(VPP1_MATRIX_OFFSET2 + offset_vpp1,
			(m[20] >> 2) & 0xfff);

		vpp_reg_setb(VPP1_MATRIX_EN_CTRL + offset_vpp1, 1, 0, 1);
	} else if (vpp_top == 2) {
...
static void amvecm_saturation_hue_post(int sat, int hue)
{
	...
	vpp_reg_write(VPP_VADJ2_MA_MB, (int)mab);
	mc = 0 - (mb & 0x3ff); /* mc = -mb */
	md = (s16)((mab << 6) >> 22);  /* md =	ma; */
	mab = ((mc & 0x3ff) << 16) | (md & 0x3ff);

	vpp_reg_write(VPP_VADJ2_MC_MD, (int)mab);
#ifndef AML_S5_DISPLAY
	vpp_reg_setb(VPP_VADJ_CTRL, 1, 2, 1);
#endif
}
...
void vpp_viu2_matrix_update(int type)
{
	...
	switch (type) {
	case VPP_CM_RGB:
		if (chip_id == MESON_CPU_MAJOR_ID_A9) {
			/* vpp_top1: yuv2rgb */
...
static void vpp_osd_path_init(void)
{
	int chip_id = vpp_get_chip_type();
	...
	vpp_reg_setb(VIU_OSD1_PATH_CTRL, 0x2, 15, 2);//Osd1 dolby bypass 0/1: no bypass/bypass
	vpp_reg_setb(VIU_OSD2_PATH_CTRL, 0x7, 18, 3);
	vpp_reg_setb(VIU_OSD3_PATH_CTRL, 0x2, 15, 2);
}

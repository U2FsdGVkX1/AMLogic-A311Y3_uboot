...
int load_dv_fw(char *fw_dir, void *cp_info)
{
	...
	tmp_size = model_get_file_size(fw_dir);
	...
	if (tmp_buf) {
		rd_cnt = model_read_file(fw_dir, tmp_buf, tmp_size);
		if (rd_cnt <= 0 || rd_cnt > tmp_size) {
	...
	if (check_fw_valid(tmp_buf, tmp_size) == 1) {
		...
		exec_size = ALIGN(DV_FW_MAX_SIZE, SZ_2M);
		if (addr &  ~SZ_2M) {
			addr_align = (ALIGN(addr, SZ_2M) - SZ_2M);
			exec_size += SZ_2M;
		}

		extern void mmu_set_efi_attr(uint64_t addr, uint64_t size);
		mmu_set_efi_attr((uint64_t)addr_align, exec_size);
...
#else
	if (fw_dir_odm_ext && model_file_is_exist(fw_dir_odm_ext)) {
		ret = load_dv_fw(fw_dir_odm_ext, &cp_info);
	} else if (fw_dir_vendor && model_file_is_exist(fw_dir_vendor)) {
		ret = load_dv_fw(fw_dir_vendor, &cp_info);
	} else if (fw_dir && model_file_is_exist(fw_dir)) {

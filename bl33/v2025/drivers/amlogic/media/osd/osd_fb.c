...
int get_osd_layer(void)
{
	...
	if (layer_str && strcmp(layer_str, "osd0") == 0) {
		osd_index = OSD1;
		vpp_index = VPU_VPP0;
	} else if (layer_str && strcmp(layer_str, "osd1") == 0) {
		osd_index = OSD2;
		vpp_index = VPU_VPP0;
	} else if (layer_str && strcmp(layer_str, "osd2") == 0) {
		osd_index = OSD3;
		vpp_index = VPU_VPP0;
	} else if (layer_str && strcmp(layer_str, "osd3") == 0) {
		osd_index = OSD4;
		vpp_index = VPU_VPP0;
	} else if (layer_str && strcmp(layer_str, "viu2_osd0") == 0) {
		osd_index = OSD3;
		vpp_index = VPU_VPP1;
	} else if (layer_str && strcmp(layer_str, "viu3_osd0") == 0) {
		osd_index = OSD4;
		vpp_index = VPU_VPP2;
	} else {
...
int video_display_bitmap(ulong bmp_image, int x, int y)
{
	...
	osd_logd("Display-bmp: (x:%d y:%d) %d x %d  with %d colors\n",
		 x, y, (int)width, (int)height, (int)colors);
...
no_scale:
	...
	if (osd_get_chip_type() == MESON_CPU_MAJOR_ID_A9 && is_vpp1(osd_index))
		vpp1_post_blend_update_a9(osd_index);
#endif

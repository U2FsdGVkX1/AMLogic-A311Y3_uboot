...
void osd_setting_default_hwc(u32 index, struct pandata_s *disp_data)
{
	...
	if (osd_get_chip_type() == MESON_CPU_MAJOR_ID_S5 ||
	    ...
	    osd_get_chip_type() == MESON_CPU_MAJOR_ID_A9)
		postbld_src3_sel = 4;
...
void vpp1_post_blend_update_a9(u32 index)
{
	osd_reg_write(VPP1_VD1_BLEND_SRC_CTRL, 0);
	osd_reg_write(VPP1_VD2_BLEND_SRC_CTRL, 0);
	osd_reg_write(VPP1_OSD1_BLEND_SRC_CTRL, 2);
	osd_reg_write(VPP1_OSD2_BLEND_SRC_CTRL, 0);
	osd_reg_write(OSD3_FIELD_SEL, 1);
}
...
static void osd_update_mif_linear_addr(u32 index)
{
	...
	if (index >= OSD_MAX) {
		osd_loge("%s: invalid index=%u\n", __func__, index);
		return;
	}

	osd_blk1_cfg_w4 = hw_osd_reg_array[index].osd_blk1_cfg_w4;
	osd_blk2_cfg_w4 = hw_osd_reg_array[index].osd_blk2_cfg_w4;
	osd_ctrl_stat = hw_osd_reg_array[index].osd_ctrl_stat;
...
#if defined(AML_T7_DISPLAY) || defined(AML_S5_DISPLAY) || defined(AML_S6_DISPLAY)
static void osdx_update_disp_freescale_enable(u32 index)
{
	...
	if (is_vpp1(index)) {
		...
		VSYNCOSD_WR_MPEG_REG(VPP1_BLD_OUT_SIZE, vinfo->height << 16 | vinfo->width);
#endif
#ifdef AML_S5_DISPLAY
		/* vpp1 scope */
		VSYNCOSD_WR_MPEG_REG
...
static void osd_update_color_mode(u32 index)
{
	...
	if (osd_hw.color_info[index]) {
		...
		if (osd_get_chip_type() == MESON_CPU_MAJOR_ID_A9) {
			switch (osd_hw.color_info[index]->color_index) {
			case 8:
			case 16:
				data32 = osd_hw.color_info[index]->color_index / 8;
				break;
			case 32:
				data32 = 3;
				break;
			...
			//	break;
			default:
				osd_loge("%s, color_index:%d is not supported\n", __func__,
					 osd_hw.color_info[index]->color_index);
				break;
			}
			VSYNCOSD_WR_MPEG_REG_BITS(mif_ctrl_reg[index],
				data32, 9, 4);
		}
	}
...
void osd_init_hw_viux(u32 index)
{
	...
	if (!logo_loaded[index]) {
		...
		/* a9 osd23_hdr_mux need to select osd3 go filed */
		if (osd_get_chip_type() == MESON_CPU_MAJOR_ID_A9)
			osd_reg_set_bits(VIU_OSD2_PATH_CTRL, 0x1, 20, 1);
		...
		if (osd_get_chip_type() == MESON_CPU_MAJOR_ID_T3 ||
			osd_get_chip_type() == MESON_CPU_MAJOR_ID_T5M ||
			osd_get_chip_type() == MESON_CPU_MAJOR_ID_T6W ||
			osd_get_chip_type() == MESON_CPU_MAJOR_ID_A9)
	...
	if (osd_get_chip_type() == MESON_CPU_MAJOR_ID_T6W ||
	    osd_get_chip_type() == MESON_CPU_MAJOR_ID_A9)
		osd_hw.mif_linear = 1;
}
#else
...
#ifdef CONFIG_AML_DOLBY
		if (dolby_vision_on)
			osd_reg_set_bits(VIU_OSD1_PATH_CTRL, 0x1, 15, 2);/*bit16 0: enable dv; bit15 1: disable hdr*/
		else
#endif
			osd_reg_set_bits(VIU_OSD1_PATH_CTRL, 0x2, 15, 2);
...
void osd_init_hw(u32 index)
{
	...
	if (!logo_loaded[index]) {
		...
		if (osd_hw.osd_ver == OSD_HIGH_ONE) {
			...
			/* a9 vpp1 ofifo size */
			if (osd_get_chip_type() == MESON_CPU_MAJOR_ID_A9)
				osd_reg_write(VPP1_LFIFO_CTRL, data32);
		}
#endif
		/* init osd fifo control register */
		/* set DDR request priority to be urgent */

...
static void vdac_enable_cvbs_out(bool on)
{
	...
	if (on) {
		if (vdac_data->cpu_id >= VDAC_CPU_S1A) {
			vdac_write_reg(reg_cntl0, vdac_data->cvbsout_cfg_cntl0);
			vdac_ctrl_config(1, reg_cntl1, 7);
		} else if (vdac_data->cpu_id >= VDAC_CPU_S4) {
...
static void vdac_gsw_init(void)
{
	...
	/* printf("reg_cnt0: %x, reg_cnt1: %x\n", */
	/* vdac_read_reg(vdac_data->reg_ctrl0), */
	/* vdac_read_reg(vdac_data->reg_ctrl1)); */
...
static struct meson_vdac_ctrl_s vdac_ctrl_enable_s1a[] = {
	{ANACTRL_VDAC_CTRL1, 0, 0, 7},  /*gsw */
	{ANACTRL_VDAC_CTRL1, 1, 7, 1}, /* cdac_pwd */
	{VDAC_REG_MAX, 0, 0, 0},
};
...
struct vdac_data_s vdac_data_s1a = {
	.cpu_id = VDAC_CPU_S1A,
	.reg_ctrl0 = ANACTRL_VDAC_CTRL0,
	.reg_ctrl1 = ANACTRL_VDAC_CTRL1,
	.vdac_ctrl = vdac_ctrl_enable_s1a,
	.cvbsout_cfg_cntl0 = 0x00418982, //vlsi suggestion value
};

struct vdac_data_s vdac_data_s7 = {
	.cpu_id = VDAC_CPU_S7,
	.reg_ctrl0 = ANACTRL_VDAC_CTRL0,
	.reg_ctrl1 = ANACTRL_VDAC_CTRL1,
	.vdac_ctrl = vdac_ctrl_enable_s7,

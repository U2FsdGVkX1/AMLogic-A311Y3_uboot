...
struct venc_test_pattern_s {
	char *name;
	uint8_t mode, tst_en, vfifo_en, rgbin;
	uint16_t Y, Cb, Cr;
};
...
static void dptx_venc_set_timing(struct dptx_drv_s *dptx)
{
	...
	case eDPTX_CHIP_T7:
		dptx_vcbus_write(ENCL_INBUF_CNTL1 + offset, (5 << 13) |
					(dptx->act_timing.h_act - 1));
...
static void dptx_dual_set_t7(struct dptx_drv_s *dptx,
				uint8_t en, uint8_t to_port, uint8_t dual_mode)
{
...
struct edptx_venc_op_s edptx_venc_t7 = {
	.wait_vsync        = dptx_venc_wait_vsync,
	.get_max_lcnt      = dptx_venc_get_max_lint_cnt,
	.venc_debug_test   = dptx_venc_debug_test,
	.venc_set_timing   = dptx_venc_set_timing,
	.venc_set          = dptx_venc_set,
	.venc_switch       = dptx_venc_enable_ctrl,
	.mute_set          = dptx_venc_mute_set,
	.get_encl_line_cnt = dptx_venc_get_encl_line_cnt,
};

struct edptx_venc_op_s *dptx_venc_op_init_t7(struct dptx_drv_s *dptx)
{
	return &edptx_venc_t7;
};

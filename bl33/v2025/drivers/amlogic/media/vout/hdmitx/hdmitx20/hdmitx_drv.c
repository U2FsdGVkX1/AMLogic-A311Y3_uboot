...
/* TODO */
static const char *vend_name = "CUS_NAME"; /* Max 8 bytes */
static const char *prod_desc = "PRODUCT_NAME"; /* Max 16 bytes */
...
static void hdmi_tvenc_set(enum hdmi_vic vic)
{
	...
	case HDMI_720x576i50_4x3:
		...
		hd_write_reg(P_VPU_HDMI_SETTING, (0 << 0) | /* [    0] src_sel_enci*/
			...
			(0 << 4) | /* [    4] inv_dvi_clk. 1=Invert clock to external DVI, (clock inversion exists at internal HDMI).*/
...
void get_hdmi_efuse(struct hdmitx_dev *hdev)
{
	...
	for (; efuse_name_table[i]; i++) {
		...
		if (rc == EFUSE_OBJ_SUCCESS) {
			strlcpy(efuse_field.name, efuse_field_name, sizeof(efuse_field.name) - 1);

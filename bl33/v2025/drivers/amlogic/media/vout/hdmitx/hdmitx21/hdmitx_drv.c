...
//static void hdmitx_power_init(void);
...
static void hdmi_hwp_init(void)
{
	...
	hdmitx21_mux_ddc();
	/* set MPEG, audio and default video */
...
static struct hdmi_support_mode gxbb_modes[] = {
	...
	{HDMI_63_1920x1080p120_16x9, "1920x1080p120hz", 0},
	{HDMI_64_1920x1080p100_16x9, "1920x1080p100hz", 0},
...
static void hdmitx21_test_bist(unsigned int mode)
{
	...
	if (hdev->para->timing.pi_mode == 0 &&
		...
		hdev->chip_type != MESON_CPU_ID_A9)
		enci_en = true;
	...
	case 3:
		...
		break;
	case 4:
		hd21_set_reg_bits(ENCP_VIDEO_MODE_ADV, 0, 3, 1);//select pattern_gen output
		hd21_set_reg_bits(ENCP_VIDEO_RGBIN_CTRL, 1, 1, 1);//rgb src
		hd21_write_reg(VENC_VIDEO_TST_EN, 1);
		hd21_write_reg(VENC_VIDEO_TST_MDSEL, 4);
		hd21_write_reg(VENC_VIDEO_TST_Y, 256);
		hd21_write_reg(VENC_VIDEO_TST_CB, 12);
		hd21_write_reg(VENC_VIDEO_TST_CR, 3);
		hd21_write_reg(VENC_VIDEO_TST_CLRBAR_STRT , 1280);
		hd21_write_reg(VENC_VIDEO_TST_CLRBAR_WIDTH, 720);
		hd21_write_reg(VENC_VIDEO_TST_VDCNT_STSET , 2);
		break;
	case 'x':
	case 'X':
		if (enci_en)
		...
		if (hdev->para->vic == HDMI_6_720x480i60_4x3 ||
			hdev->para->vic == HDMI_7_720x480i60_16x9 ||
			hdev->para->vic == HDMI_21_720x576i50_4x3 ||
			hdev->para->vic == HDMI_22_720x576i50_16x9)
			height /= 2;
		hd21_write_reg(VENC_VIDEO_TST_Y, 0x3ff);
		hd21_write_reg(VENC_VIDEO_TST_CB, 0x16);
		hd21_write_reg(VENC_VIDEO_TST_CR, 0x9);
	...
	case 'g':
	case 'G':
		width = hdev->para->timing.h_active / block_num;
		step_dat = 1024 / block_num;
		ofst_x = hd21_read_reg(ENCP_VIDEO_HAVON_BEGIN) - 2;
		pr_info("Generate GRAY pattern\n");
		hd21_set_reg_bits(ENCP_VIDEO_MODE_ADV, 0, 3, 1);//select pattern_gen output
		hd21_set_reg_bits(ENCP_VIDEO_RGBIN_CTRL, 1, 1, 1);//rgb src
		hd21_write_reg(VENC_VIDEO_TST_EN, 1);// test bist en
		hd21_write_reg(VENC_VIDEO_TST_MDSEL, 5);// mode gray
		hd21_set_reg_bits(VENC_VIDEO_TST_Y, step_dat, 0, 10);//gray step dat
		hd21_set_reg_bits(VENC_VIDEO_TST_CB, 0, 0, 3);//gray init sel
		hd21_set_reg_bits(VENC_VIDEO_TST_CR, 7, 0, 3);//gray step sel
		hd21_set_reg_bits(VENC_VIDEO_TST_CR, width , 3, 7);//gray step wid
		hd21_set_reg_bits(VENC_VIDEO_TST_CLRBAR_STRT, ofst_x, 0, 13);//gray ofst x
		break;
	case 'w':
	case 'W':
		hd21_write_reg(VENC_VIDEO_TST_EN, 1);
		hd21_write_reg(VENC_VIDEO_TST_MDSEL, 6);
		hd21_set_reg_bits(ENCP_VIDEO_MODE_ADV, 0, 3, 1);
		hd21_set_reg_bits(ENCP_VIDEO_RGBIN_CTRL, 1, 1, 1);
		hd21_write_reg(VENC_VIDEO_TST_VDCNT_STSET, 2);
		hd21_write_reg(VENC_VIDEO_TST_CLRBAR_STRT, 960);
		hd21_write_reg(VENC_VIDEO_TST_CLRBAR_WIDTH, 120);
		hd21_write_reg(VENC_VIDEO_TST_Y, 0x3ff);
		hd21_write_reg(VENC_VIDEO_TST_CB, 0x3ff);
		hd21_write_reg(VENC_VIDEO_TST_CR, 0);
		hd21_write_reg(VENC_VIDEO_TST_VDCNT_STSET, 3);
		hd21_write_reg(VENC_VIDEO_TST_CLRBAR_STRT, 128);
		hd21_write_reg(VENC_VIDEO_TST_CLRBAR_WIDTH, 600);
		hd21_write_reg(VENC_VIDEO_TST_Y, 0);
		hd21_write_reg(VENC_VIDEO_TST_CB, 0x3ff);
		hd21_write_reg(VENC_VIDEO_TST_CR, 0x3ff);
		hd21_write_reg(VENC_VIDEO_TST_VDCNT_STSET, 0);
		hd21_write_reg(VENC_VIDEO_TST_CLRBAR_STRT, hdev->para->timing.h_active);
		hd21_write_reg(VENC_VIDEO_TST_CLRBAR_WIDTH, hdev->para->timing.v_active);
		break;
	case 0:
	default:
		if (enci_en) {
...
static void get_kernel_enc_idx_clk_source(struct hdmitx_dev *hdev)
{
	char *dt_addr = NULL;
	int nodeoffset;
	char *propdata;
	...
	propdata = (char *)fdt_getprop((const void *)dt_addr, nodeoffset, "enc_idx", NULL);
	if (propdata) {
		if (be32_to_cpup((u32 *)propdata) == 2)
			hdev->enc_idx = 2;
		else if (be32_to_cpup((u32 *)propdata) == 1)
			hdev->enc_idx = 1;
		else
			hdev->enc_idx = 0;
	}
	...
	propdata = (char *)fdt_getprop((const void *)dt_addr, nodeoffset, "clk_source_idx", NULL);
	if (propdata) {
		if (be32_to_cpup((u32 *)propdata) == 1)
			hdev->clk_source_idx = 1;
		else
			hdev->clk_source_idx = 0;
	}
	printf("hdmitx clk source: %d\n", hdev->clk_source_idx);
...
static void hdmitx_load_dts_config(struct hdmitx_dev *hdev)
{
	...
	/*
	 * bq201-s905d5 viu2 only has 250M
	 * Maximum supported resolution: 1080p60hz
	 */
	if (env_get("connector1_type") && is_valid_hdmi(env_get("connector1_type"))) {
		/*
		 * 1080p_limit
		 * 1: limit to 1080p60hz
		 * 0: unlimited
		 */
		propdata = (char *)fdt_getprop(dt_blob, node, "1080p_limit", NULL);
		if (propdata) {
			if (be32_to_cpup((u32 *)propdata) == 1)
				/*
				 * If 1080p_limit is equal to 1, it means 4k
				 * is not supported, and res_1080p needs to be written as 1
				 */
				hdev->tx_common.res_1080p = 1;
		}
		printf("1080p_limit: %d\n", hdev->tx_common.res_1080p);
	}
	 ...
	 */
	if (hdev->tx_common.res_1080p)
		txcap->tx_max_tmds_clk = 225;
	else
		txcap->tx_max_tmds_clk = 600;

	if (hdev->chip_type == MESON_CPU_ID_S5)
		txcap->dsc_capable = true;
	else
		txcap->dsc_capable = false;
	...
	if (propdata) {
		...
		if (max_refreshrate > 120)
			printf("wrong max_refreshrate %d\n", max_refreshrate);
		else
			hdev->tx_common.max_refreshrate = max_refreshrate;
	}
	printf("max_refreshrate: %d\n", hdev->tx_common.max_refreshrate);

	hdev->vend_name = (char *)fdt_getprop(dt_blob, node, "vendor_name", NULL);
	hdev->prod_desc = (char *)fdt_getprop(dt_blob, node, "product_desc", NULL);
}
...
static void amhdmitx_infoframe_init(struct hdmitx_dev *hdev)
{
	...
	if (hdev->vend_name && hdev->prod_desc)
		hdmi_spd_infoframe_init(&hdev->infoframes.spd.spd, hdev->vend_name,
					hdev->prod_desc);
...
void hdmitx21_init(void)
{
	...
	/*
	 * After the S7 chip, uboot introduced a standby solution,
	 * and power consumption was reduced after standby.
	 */
	//if (hdev->chip_type >= MESON_CPU_ID_S7)
	//	hdmitx_power_init();
}
...
{
	/* default clk source : 0 */
	if (hdev->chip_type == MESON_CPU_ID_A9) {
		if (hdev->clk_source_idx == 1) {
			/* select vid_pll_clk_pre */
			hd21_set_reg_bits(CLKCTRL_VIID_CLK0_CTRL_A9, 0, 16, 3);
			/* for clk div */
			hd21_set_reg_bits(CLKCTRL_VIID_CLK0_DIV_A9, div - 1, 0, 8);
			/* clk div gate */
			hd21_set_reg_bits(CLKCTRL_VIID_CLK0_CTRL_A9, 1, 0, 1);
		} else {
			/* select vid_pll_clk_pre */
			hd21_set_reg_bits(CLKCTRL_VID_CLK0_CTRL_A9, 0, 16, 3);
			/* for clk div */
			hd21_set_reg_bits(CLKCTRL_VID_CLK0_DIV_A9, div - 1, 0, 8);
			/* clk div gate */
			hd21_set_reg_bits(CLKCTRL_VID_CLK0_CTRL_A9, 7, 0, 3);
		}
		if (hdev->clk_analog_path) {
			 ...
			 */
			hd21_set_reg_bits(CLKCTRL_HDMI_CLK_CTRL_A9, 1, 15, 1);
...
static void set_hdmi_tx_pixel_div(u32 div)
{
	...
	if (hdev->chip_type == MESON_CPU_ID_A9) {
		if (hdev->clk_source_idx == 1)
			vid_clk_ctrl0 = CLKCTRL_VIID_CLK0_CTRL_A9;
		else
			vid_clk_ctrl0 = CLKCTRL_VID_CLK0_CTRL_A9;
		vid_clk_ctrl2 = CLKCTRL_VID_CLK0_CTRL2_A9;
		hdmi_clk_ctrl0 = CLKCTRL_HDMI_CLK_CTRL_A9;
	} else {
		vid_clk_ctrl0 = CLKCTRL_VID_CLK0_CTRL;
		vid_clk_ctrl2 = CLKCTRL_VID_CLK0_CTRL2;
		hdmi_clk_ctrl0 = CLKCTRL_HDMI_CLK_CTRL;
	}

	/* enable gate */
	hd21_set_reg_bits(vid_clk_ctrl2, 1, 5, 1);
	/* for s7 s7d s6 */
	if (hdev->chip_type == MESON_CPU_ID_S7 || hdev->chip_type == MESON_CPU_ID_S7D ||
	    hdev->chip_type == MESON_CPU_ID_S6 || hdev->chip_type == MESON_CPU_ID_A9) {
		if (hdev->para->cs == HDMI_COLORSPACE_YUV420) {
			hd21_set_reg_bits(vid_clk_ctrl0, 1, 1, 1);
			// div 2
			hd21_set_reg_bits(hdmi_clk_ctrl0, hdev->clk_source_idx == 1 ? 9 : 1, 16, 4);
		} else {
			// div 1
			hd21_set_reg_bits(hdmi_clk_ctrl0, hdev->clk_source_idx == 1 ? 8 : 0, 16, 4);
		}
	}
}
 ...
 * todo
 */
static void set_encp_div(void)
{
	struct hdmitx_dev *hdev = get_hdmitx21_device();
	switch (hdev->chip_type) {
	case MESON_CPU_ID_A9:
		if (hdev->clk_source_idx == 1)
			hd21_set_reg_bits(CLKCTRL_VIID_CLK0_CTRL_A9, 1, 19, 1);
		else
			hd21_set_reg_bits(CLKCTRL_VID_CLK0_CTRL_A9, 1, 19, 1);
		break;
	case MESON_CPU_ID_S5:
	case MESON_CPU_ID_T7:
		hd21_set_reg_bits(CLKCTRL_VID_CLK0_DIV, 1, 24, 4);
		hd21_set_reg_bits(CLKCTRL_VID_CLK0_CTRL, 1, 19, 1);
		break;
	default:
		hd21_set_reg_bits(CLKCTRL_VID_CLK0_DIV, 0, 24, 4);
		hd21_set_reg_bits(CLKCTRL_VID_CLK0_CTRL, 1, 19, 1);
		break;
	}
}
...
static void hdmitx_enable_encp_clk(void)
{
	...
	/* default config
	 * enc_idx = 0, clk_source_idx = 0
	 */
	if (hdev->chip_type == MESON_CPU_ID_A9) {
		if (hdev->enc_idx == 1 && hdev->clk_source_idx == 1) {
			/* enc1 clk sel div1, use v2 clk config */
			hd21_set_reg_bits(CLKCTRL_VIID_CLK0_DIV_A9, 8, 8, 4);
			/* enc1 clk gate */
			hd21_set_reg_bits(CLKCTRL_VID_CLK0_CTRL2_A9, 1, 11, 1);
		} else if (hdev->enc_idx == 1 && hdev->clk_source_idx == 0) {
			/* enc1 clk sel div1, use v0 clk config */
			hd21_set_reg_bits(CLKCTRL_VIID_CLK0_DIV_A9, 0, 8, 4);
			hd21_set_reg_bits(CLKCTRL_VID_CLK0_CTRL2_A9, 1, 11, 1);
		} else if (hdev->enc_idx == 0 && hdev->clk_source_idx == 1) {
			/* enc0 clk sel div1, use v2 clk config*/
			hd21_set_reg_bits(CLKCTRL_VIID_CLK0_DIV_A9, 8, 12, 4);
			/* enc0 clk gate */
			hd21_set_reg_bits(CLKCTRL_VID_CLK0_CTRL2_A9, 1, 10, 1);
		} else {
			/* enc0 clk sel div1 */
			hd21_set_reg_bits(CLKCTRL_VIID_CLK0_DIV_A9, 0, 12, 4);
			/* enc0 clk gate */
			hd21_set_reg_bits(CLKCTRL_VID_CLK0_CTRL2_A9, 1, 10, 1);
		}
	} else {
...
static void set_hdmitx_fe_clk(void)
{
	...
	if (hdev->chip_type == MESON_CPU_ID_A9) {
		vid_clk_cntl2 = CLKCTRL_VID_CLK0_CTRL2_A9;
		vid_clk_div = CLKCTRL_VID_CLK0_DIV_A9;
		hdmi_clk_cntl = CLKCTRL_HDMI_CLK_CTRL_A9;
	...
	if (hdev->chip_type == MESON_CPU_ID_A9) {
		hd21_set_reg_bits(hdmi_clk_cntl, hdev->clk_source_idx == 1 ? 8 : 0, 20, 4);
	} else {
...
static void _hdmitx21_set_clk(struct hdmitx_dev *hdev)
{
...
void enable_crt_video_hdmi(u32 enable, u32 in_sel, u8 enc_sel)
{
	...
	data32 = (enc_sel << 21) |
		 ...
		 (0 << 0);
	if (hdev->chip_type == MESON_CPU_ID_S5) {
		if (para->cs == HDMI_COLORSPACE_YUV420 && !hdev->para->frl_rate) {
...
static void construct_spd_packet(struct hdmitx_dev *hdev)
{
...
{
	struct hdmitx_dev *hdev = get_hdmitx21_device();
	u32 MATRIX_COEF00_01;
	u32 MATRIX_COEF02_10;
	u32 MATRIX_COEF11_12;
	u32 MATRIX_COEF20_21;
	u32 MATRIX_COEF22;
	u32 MATRIX_PRE_OFFSET0_1;
	u32 MATRIX_PRE_OFFSET2;
	u32 MATRIX_OFFSET0_1;
	u32 MATRIX_OFFSET2;
	u32 MATRIX_EN_CTRL;
	u32 vpu_hdmi_fmt_ctrl;

	if (hdev->chip_type == MESON_CPU_ID_A9) {
		MATRIX_COEF00_01 = VPU_HDMI_MATRIX_COEF00_01_A9;
		MATRIX_COEF02_10 = VPU_HDMI_MATRIX_COEF02_10_A9;
		MATRIX_COEF11_12 = VPU_HDMI_MATRIX_COEF11_12_A9;
		MATRIX_COEF20_21 = VPU_HDMI_MATRIX_COEF20_21_A9;
		MATRIX_COEF22 = VPU_HDMI_MATRIX_COEF22_A9;
		MATRIX_PRE_OFFSET0_1 = VPU_HDMI_MATRIX_PRE_OFFSET0_1_A9;
		MATRIX_PRE_OFFSET2 = VPU_HDMI_MATRIX_PRE_OFFSET2_A9;
		MATRIX_OFFSET0_1 = VPU_HDMI_MATRIX_OFFSET0_1_A9;
		MATRIX_OFFSET2 = VPU_HDMI_MATRIX_OFFSET2_A9;
		MATRIX_EN_CTRL = VPU_HDMI_MATRIX_EN_CTRL_A9;
		vpu_hdmi_fmt_ctrl = VPU_HDMI_FMT_CTRL_A9;
	} else {
		MATRIX_COEF00_01 = VPU_HDMI_MATRIX_COEF00_01;
		MATRIX_COEF02_10 = VPU_HDMI_MATRIX_COEF02_10;
		MATRIX_COEF11_12 = VPU_HDMI_MATRIX_COEF11_12;
		MATRIX_COEF20_21 = VPU_HDMI_MATRIX_COEF20_21;
		MATRIX_COEF22 = VPU_HDMI_MATRIX_COEF22;
		MATRIX_PRE_OFFSET0_1 = VPU_HDMI_MATRIX_PRE_OFFSET0_1;
		MATRIX_PRE_OFFSET2 = VPU_HDMI_MATRIX_PRE_OFFSET2;
		MATRIX_OFFSET0_1 = VPU_HDMI_MATRIX_OFFSET0_1;
		MATRIX_OFFSET2 = VPU_HDMI_MATRIX_OFFSET2;
		MATRIX_EN_CTRL = VPU_HDMI_MATRIX_EN_CTRL;
		vpu_hdmi_fmt_ctrl = VPU_HDMI_FMT_CTRL;
	}

	pr_info("ycbcr2rgb matrix\n");
	/* ycbcr not full range, 601 conversion */
	hd21_write_reg(MATRIX_PRE_OFFSET0_1,  ((0x0) << 16) | (0xe00)); /* 0xfc00e00); */
	hd21_write_reg(MATRIX_PRE_OFFSET2, (0xe00)); /* 0x0e00); */
	 ...
	 * 1   1.772     0
	 */
	hd21_write_reg(MATRIX_COEF00_01, (0x400 << 16) | 0);
	hd21_write_reg(MATRIX_COEF02_10, (0x59c << 16) | 0x400);
	hd21_write_reg(MATRIX_COEF11_12, (0x1ea0 << 16) | 0x1d25);
	hd21_write_reg(MATRIX_COEF20_21, (0x400 << 16) | 0x717);
	/*
	 * S6 TODO
	 * 1.164     0       1.596
	 * 1.164   -0.392    -0.813
	 ...
	 */
	hd21_write_reg(MATRIX_COEF22, 0x0);
	hd21_write_reg(MATRIX_OFFSET0_1, 0x0);
	hd21_write_reg(MATRIX_OFFSET2, 0x0);

	/* ycbcr full range, 601 conversion */
	if (0) {
		hd21_write_reg(MATRIX_PRE_OFFSET0_1,
		 ...
		 * 1.164   2.017     0
		 */
		hd21_write_reg(MATRIX_COEF00_01, (0x4a8 << 16) | 0);
		hd21_write_reg(MATRIX_COEF02_10, (0x662 << 16) | 0x4a8);
		hd21_write_reg(MATRIX_COEF11_12, (0x1e6f << 16) | 0x1cbf);
		hd21_write_reg(MATRIX_COEF20_21, (0x4a8 << 16) | 0x811);
		hd21_write_reg(MATRIX_COEF22, 0x0);
		hd21_write_reg(MATRIX_OFFSET0_1, 0x0);
		hd21_write_reg(MATRIX_OFFSET2, 0x0);
	}
...
static void hdmitx_set_phy_todig(struct hdmitx_dev *hdev)
{
	...
	case MESON_CPU_ID_S6:
		...
		break;
	case MESON_CPU_ID_A9:
		hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL3_A9, 0, 1, 1);
		hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL3_A9, 0, 3, 1);
		usleep_range(100, 110);
		hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL3_A9, 3, 0, 2);
		hd21_set_reg_bits(ANACTRL_HDMIPHY_CTRL3_A9, 1, 3, 1);
		break;
	default:
...
static void hdmitx21_set_reset_reg_bit(u32 addr, u32 bit_nr)
{
	if (bit_nr > 31)
		return;
...
/*
 * not use, maybe use later and enable here
 * static void hdmitx21_nreset_reg_bit(u32 addr, u32 bit_nr)
 * {
 *	if (bit_nr > 31)
 *		return;
 *	hdmitx21_set_reg_bits(addr, 0, bit_nr, 1);
...
void hdmitx21_set(struct hdmitx_dev *hdev)
{
	...
	u32 enc_sel = 0;
	u32 data32;
	u32 vpu_hdmi_setting_reg = VPU_HDMI_SETTING;
	u32 vpu_hdmi_fmt_ctrl_reg = VPU_HDMI_FMT_CTRL;
	...
	get_kernel_enc_idx_clk_source(hdev);

	para->frl_rate = hdmitx_select_frl_rate(&para->dsc_en,
						hdev->tx_common.tx_hw->hdmi_tx_cap.dsc_policy,
						para->vic, para->cs, para->cd);
	if (para->dsc_en && hdev->chip_type == MESON_CPU_ID_S5) {
		/*
	...
	if (hdev->manual_frl_rate) {
		para->frl_rate = hdev->manual_frl_rate;
	...
	} else if (hdev->enc_idx == 1) {
		set_hdmitx_enc_idx(1);
	}
	...
	switch (hdev->chip_type) {
	case MESON_CPU_ID_A9:
		hd21_set_reg_bits(VPU_HDMI_SETTING_A9, 0, (hdev->enc_idx == 0) ? 0 : 1, 1);
		break;
	default:
		break;
	}
	 ...
	 */
	if (para->cs == HDMI_COLORSPACE_YUV422 || para->cd == COLORDEPTH_24B || para->dsc_en) {
	...
	if (para->timing.pi_mode == 0 &&
		...
		hdev->chip_type != MESON_CPU_ID_A9) {
		/* sel enci timming */
		hd21_write_reg(VPU_VENC_CTRL, 0);
		set_tv_enci_new(hdev, enc_sel, vic, 1);
		hdmitx_enable_enci_clk();
	} else {
		/* A9 not have enci, use encp to configure the output interlaced format */
		pr_info("sel encp timming\n");
		/* sel encp timming */
		hd21_write_reg(VPU_VENC_CTRL, 1);
	...
	case MESON_CPU_ID_S6:
		...
		data32 = (((para->cs == HDMI_COLORSPACE_RGB) ? 3 :
			  ...
			  (2 << 22) |
			  (0 << 24) |
			  (((para->cs == HDMI_COLORSPACE_RGB) ? 1U : 0U) << 31);
		break;
	case MESON_CPU_ID_A9:
		vpu_hdmi_fmt_ctrl_reg = VPU_HDMI_FMT_CTRL_A9;
		/* bit[31] = 1 enable ycbcr2rgb or rgb2yuv */
		data32 = (((para->cs == HDMI_COLORSPACE_RGB) ? 3 :
				(para->cs == HDMI_COLORSPACE_YUV420) ? 2 :
				(para->cs == HDMI_COLORSPACE_YUV422) ? 1 : 0) << 0) |
				(((para->cd == COLORDEPTH_24B) ? 1 : 0) << 10) |
				(2 << 22) |
				(para->timing.pixel_repetition_factor << 24) |
	...
	default:
		data32 = (((para->cs == HDMI_COLORSPACE_YUV420) ? 2 :
			  ...
			  (2 << 22) |
			  (0 << 24);
		if (para->frl_rate && para->cs == HDMI_COLORSPACE_YUV420)
			data32 |= 3 << 0; // 3:420 dual port
		break;
	}
	hd21_write_reg(vpu_hdmi_fmt_ctrl_reg, data32);
	/*
	 * [    2] inv_hsync_b
	 * [    3] inv_vsync_b
	...
	data32 = (0 << 2) |
		...
		(0 << 12);
	if (hdev->chip_type == MESON_CPU_ID_A9)
		hd21_write_reg(VPU_HDMI_DITH_CNTL_A9, data32);
	else
	...
	data32 = 0;
	data32 |= (1U << 31); /* [   31] cntl_hdcp14_min_size_v_en */
	...
	data32 = 0;
	data32 |= (1U << 31); /* [   31] cntl_hdcp22_min_size_v_en */
	 ...
	 */
	if (hdev->chip_type == MESON_CPU_ID_S5 && para->dsc_en)
	...
	case MESON_CPU_ID_S6:
		...
		break;
	case MESON_CPU_ID_A9:
		vpu_hdmi_setting_reg = VPU_HDMI_SETTING_A9;
	...
	hd21_write_reg(vpu_hdmi_setting_reg, data32);
	/* after s1a, ycbcr -> rgb */
	if (hdev->chip_type == MESON_CPU_ID_S1A ||
		hdev->chip_type == MESON_CPU_ID_S7 ||
		hdev->chip_type == MESON_CPU_ID_S7D ||
	...
	switch (hdev->chip_type) {
	case MESON_CPU_ID_A9:
		/* bit[1:0]: turn on venc0/venc1
		 * 1: set enc0 to hdmi
		 * 2: set enc1 to hdmi
		 */
		hd21_set_reg_bits(VPU_HDMI_SETTING_A9, 1, (hdev->enc_idx == 0) ? 0 : 1, 1);
		break;
	default:
		break;
	}
		...
		*/
		{
			...
			if (!h_unstable && hdev->chip_type > MESON_CPU_ID_T7) {
				...
				if (loop < 0)
					pr_err("%s[%d] phase is still unstable\n", __func__,
					__LINE__);
			}
		}
	}
	...
	if (hdev->chip_type == MESON_CPU_ID_S5) {
		if (para->frl_rate) {
			...
			if (para->dsc_en) {
				...
				ret = frl_check_full_bw(HDMI_COLORSPACE_YUV444, COLORDEPTH_24B,
							...
							1000 * 2,
					hc_active, para->frl_rate, &tri_bytes_per_line);
#endif
			} else {
				ret = frl_check_full_bw(hdev->para->cs, hdev->para->cd,
							hdev->para->timing.pixel_freq,
							hdev->para->timing.h_active,
					para->frl_rate, &tri_bytes_per_line);
			}
			/* manual mode */
			if (hdev->dfm_type == 1) {
		...
		if (hdev->RXCap.max_frl_rate && para->frl_rate)
			hdev->flt_train_st = hdmitx_frl_training_main(para->frl_rate);
	}
	if (hdev->pxp_mode)
		return; /* skip in pxp */
...
static void hdmi_phy_suspend(void)
{
	...
	case MESON_CPU_ID_S6:
		...
		break;
	case MESON_CPU_ID_A9:
		phy_cntl0 = ANACTRL_HDMIPHY_CTRL0_A9;
		phy_cntl3 = ANACTRL_HDMIPHY_CTRL3_A9;
		phy_cntl5 = ANACTRL_HDMIPHY_CTRL5_A9;
		hd21_write_reg(phy_cntl3, 0x0);
		hd21_write_reg(phy_cntl5, 0x0);
		break;
	default:
...
static void set_aud_acr_pkt(struct hdmitx_dev *hdev)
{
	...
	if (hdmitx_check_frac_rate(hdev) == 1)
		char_rate = char_rate - char_rate / 1001;
	if (para->cs == HDMI_COLORSPACE_YUV422)
		cd = 4;
	else
		cd = para->cd;
	if (para->cs == HDMI_COLORSPACE_YUV420)
		char_rate = char_rate / 2;
	aud_n_para = hdmitx_hw_get_audio_n_paras(FS_48K,
						 cd, char_rate);

	if (hdev->para->frl_rate)
		aud_n_para = hdmi21_get_frl_aud_n_paras(FS_48K, hdev->para->frl_rate);
	hdmitx21_set_reg_bits(ACR_CTS_CLK_DIV_IVCTX, hdev->para->frl_rate ? 1 : 0, 4, 1);

	pr_info("audio: aud_n_para = %d\n", aud_n_para);
...
static int hdmitx_set_audmode(struct hdmitx_dev *hdev)
{
	...
	data32 = 0;
	/*UDP_TXCTRL : [1] layout; [7] aud_mute_en */
	hdmitx21_wr_reg(AUDP_TXCTRL_IVCTX, data32 & 0xff);
	/* R_CTRL  bit[3]:reg_no_mclk_ctsgen_sel_pclk. bit[0]: make hw_cts_hw_sw_sel = 0 */
	hdmitx21_wr_reg(ACR_CTRL_IVCTX, 0x02);

	set_aud_acr_pkt(hdev);

	/* FREQ 00:mclk=128*Fs;01:mclk=256*Fs;10:mclk=384*Fs;11:mclk=512*Fs;...*/
	hdmitx21_wr_reg(FREQ_SVAL_IVCTX, 0);

	/*
	 * [7:6] reg_tpi_spdif_sample_size: 0=Refer to stream header;
	 * 1=16-bit; 2=20-bit; 3=24-bit
	 */
	/* [  4] reg_tpi_aud_mute */
	data32 = 0;
	data32 |= (3 << 6);
	...
	data32 |= (0 << 1);
	data32 |= (0 << 0);
	hdmitx21_wr_reg(AUD_MODE_IVCTX, 0x2);  /* AUD_MOD */
...
static void hdmitx_set_div40(bool div40)
{
	...
	switch (hdev->chip_type) {
	case MESON_CPU_ID_S5:
		set_s5_top_div40(div40, hdev->para->frl_rate);
		break;
	case MESON_CPU_ID_S1A:
...
static void config_hdmi21_tx(struct hdmitx_dev *hdev)
{
	...
	hdmitx_soft_reset(RESET_SOFTWARE_LOGIC);
	data8 = 0;
	/* [1:0]color depth. 00:8bpp;01:10bpp;10:12bpp;11:16bpp */
	data8 |= (dp_color_depth & 0x03);
	/* [7]  deep color enable bit */
	data8 |= (((dp_color_depth != 4) ? 1 : 0) << 7);
	data8 |= (para->frl_rate ? 1 : 0) << 3;
	data8 |= (para->frl_rate ? 1 : 0) << 4;
	hdmitx21_wr_reg(P2T_CTRL_IVCTX, data8);
	if (hdev->chip_type == MESON_CPU_ID_A9) {
		/* reg_null_pkt_en,Null packet Enable. */
		hdmitx21_set_reg_bits(P2T_CTRL_IVCTX, 1, 6, 1);
		/* send packet as fixed slot */
		hdmitx21_set_reg_bits(TMDS_CTRL4_IVCTX, 1, 1, 1);
		pr_info("send packet as fixed slot\n");
	}
	...
	data32 |= (1 << 5);
	/* [ 0] scrambler_en. */
	data32 |= (scrambler_en & 0x01 << 0);
	hdmitx21_wr_reg(SCRCTL_IVCTX, data32 & 0xff);

	hdmitx21_set_reg_bits(FRL_LINK_RATE_CONFIG_IVCTX, para->frl_rate, 0, 4);

	/* default value */
	hdmitx21_wr_reg(SW_RST_IVCTX, 0);
	hdmitx21_wr_reg(HT_DIG_CTL22_PHY_IVCTX, 0);
	hdmitx21_wr_reg(CLK_DIV_CNTRL_IVCTX, para->frl_rate ? 0 : 1);
...
{
	struct hdmi_format_para *para = hdev->para;
	u32 vpu_hdmi_dith_cntl;

	if (hdev->chip_type == MESON_CPU_ID_A9)
		vpu_hdmi_dith_cntl = VPU_HDMI_DITH_CNTL_A9;
	else
		vpu_hdmi_dith_cntl = VPU_HDMI_DITH_CNTL;
	...
	for (; efuse_name_table[i]; i++) {
		...
		if (rc == EFUSE_OBJ_SUCCESS) {
			...
			if (*efuse_field.data == 1) {
				switch (i) {
				case 0:
					hdev->tx_common.efuse_dis_hdmi_4k60 = 1;
					pr_info("get efuse FEAT_DISABLE_HDMI_60HZ = 1\n");
					break;
				case 1:
					hdev->tx_common.efuse_dis_output_4k = 1;
					pr_info("get efuse FEAT_DISABLE_OUTPUT_4K = 1\n");
					break;
				case 2:
					hdev->tx_common.efuse_dis_hdcp_tx22 = 1;
					pr_info("get efuse FEAT_DISABLE_HDCP_TX_22 = 1\n");
					break;
				case 3:
					hdev->tx_common.efuse_dis_hdmi_tx3d = 1;
					pr_info("get efuse FEAT_DISABLE_HDMI_TX_3D = 1\n");
					break;
				case 4:
					hdev->tx_common.efuse_dis_hdcp_tx14 = 1;
...
{
	if (enable_all) {
		hdmitx21_wr_reg(PKT_AUTO_0_IVCTX, 0); /* enable all */
		hdmitx21_wr_reg(PKT_AUTO_1_IVCTX, 0); /* enable all */
		if (enable_all & 0x1) {
			pr_info("enable to change AVI packet send position begin\n");
			hdmitx21_wr_reg(PKT_LOC_AVI_IVCTX, mov_val);
			pr_info("enable to change AVI packet send position end\n");
		}
		if (enable_all & 0x2) {
			pr_info("enable to change GAMUT packet send position begin\n");
			hdmitx21_wr_reg(PKT_LOC_GAMUT_IVCTX, mov_val);
			pr_info("enable to change GAMUT packet send position end\n");
		}
		if (enable_all & 0x4) {
			pr_info("enable to change AUD packet send position begin\n");
			hdmitx21_wr_reg(PKT_LOC_AUD_IVCTX, mov_val);
			pr_info("enable to change AUD packet send position end\n");
		}
		if (enable_all & 0x8) {
			pr_info("enable to change SPD packet send position begin\n");
			hdmitx21_wr_reg(PKT_LOC_SPD_IVCTX, mov_val);
			pr_info("enable to change SPD packet send position end\n");
		}
		if (enable_all & 0x10) {
			pr_info("enable to change MPEG packet send position begin\n");
			hdmitx21_wr_reg(PKT_LOC_MPEG_IVCTX, mov_val);
			pr_info("enable to change MPEG packet send position end\n");
		}
		if (enable_all & 0x20) {
			pr_info("enable to change VSIF packet send position begin\n");
			hdmitx21_wr_reg(PKT_LOC_VSIF_IVCTX, mov_val);
			pr_info("enable to change VSIF packet send position end\n");
		}
		if (enable_all & 0x40) {
			pr_info("enable to change GEN packet send position begin\n");
			hdmitx21_wr_reg(PKT_LOC_GEN_IVCTX, mov_val);
			pr_info("enable to change GEN packet send position end\n");
		}
		if (enable_all & 0x80) {
			pr_info("enable to change GEN2 packet send position begin\n");
			hdmitx21_wr_reg(PKT_LOC_GEN2_IVCTX, mov_val);
			pr_info("enable to change GEN2 packet send position end\n");
		}
		if (enable_all & 0x100) {
			pr_info("enable to change GEN3 packet send position begin\n");
			hdmitx21_wr_reg(PKT_LOC_GEN3_IVCTX, mov_val);
			pr_info("enable to change GEN3 packet send position end\n");
		}
		if (enable_all & 0x200) {
			pr_info("enable to change GEN4 packet send position begin\n");
			hdmitx21_wr_reg(PKT_LOC_GEN4_IVCTX, mov_val);
			pr_info("enable to change GEN4 packet send position end\n");
		}
		if (enable_all & 0x400) {
			pr_info("enable to change GEN5 packet send position begin\n");
			hdmitx21_wr_reg(PKT_LOC_GEN5_IVCTX, mov_val);
			pr_info("enable to change GEN5 packet send position end\n");
		}
		if (enable_all & 0x800) {
			pr_info("enable to change VTEM packet send position begin\n");
			hdmitx21_wr_reg(PKT_LOC_VTEM_IVCTX, mov_val);
			pr_info("enable to change VTEM packet send position end\n");
		}
	} else {
		if (pkt == AVI_PKT) {
			pr_info("enable to change AVI packet send position begin\n");
...
void hdmitx21_write_dhdr_sram(void)
{
	...
	hdmitx21_wr_reg(D_HDR_MEM_READ_EN_IVCTX, 1); /* open xfifo debug path */
	/* write */
	pr_info("write start\n");
...
void hdmitx21_read_dhdr_sram(void) {
	...
	pr_info("read start1\n");
	for (h = 0; h < 64; h++) {
		/* read address, open reg_xfifo_rd, start read xdata; loop for raddr +1 */
		hdmitx21_rd_reg(D_HDR_MEM_XFIFO_IVCTX);
		for (i = 0; i < 28; i++) {
	...
	hdmitx21_wr_reg(D_HDR_MEM_READ_EN_IVCTX, 0);  /* close xfifo debug path */
	/*
	 * release rst
	 * data32 = hdmitx21_rd_reg(HDMITX_TOP_SW_RESET);
...
u32 hdmitx_check_frac_rate(struct hdmitx_dev *hdev)
{
	...
	if (para->timing.sname)
		...
		memcpy(ext_name, para->timing.name, sizeof(ext_name));
	 ...
	 */
	if (user_hdmimode && !strstr(user_hdmimode, ext_name)) {
		if (frac_rate_str) {
			if (frac_rate_str[0] == '1' && hdmitx_likely_frac_rate_mode(ext_name))
				frac_rate = 1;
			else
				frac_rate = 0;
		} else {
		...
		user_frac_rate_str = env_get("user_frac_rate");
		if (user_frac_rate_str) {
			if (user_frac_rate_str[0] == '0')
				frac_rate = 0;
			else if (hdmitx_likely_frac_rate_mode(ext_name))
				frac_rate = 1;
		} else if (frac_rate_str) {
			if (frac_rate_str[0] == '0')
				frac_rate = 0;
			else if (hdmitx_likely_frac_rate_mode(ext_name))
				frac_rate = 1;
		} else {
	 ...
	 * Use new environment variables "actual_frac_rateto" save the real frac_rate,
...
#if 0
static int aml_hdmitx_suspend(void *pm_ops)
{
	struct dev_pm_ops *pm = (struct dev_pm_ops *)pm_ops;

	printf("hdmitx suspend: %s\n", pm->name);
	/* disable phy */
	hdmi_phy_suspend();
	/* disable PLL */
...
/* resume needs to execute the complete uboot bootcmd */
static int aml_hdmitx_resume(void *pm_ops)
{
	struct dev_pm_ops *pm = (struct dev_pm_ops *)pm_ops;

	printf("hdmitx resume: %s\n", pm->name);
	return 0;
}

/* power on will execute the complete boot process */
static int aml_hdmitx_poweroff(void *pm_ops)
{
	struct dev_pm_ops *pm = (struct dev_pm_ops *)pm_ops;

	printf("hdmitx poweroff: %s\n", pm->name);
	/* disable phy */
	hdmi_phy_suspend();
	/* disable PLL */
	hd21_write_reg(ANACTRL_HDMIPLL_CTRL0, 0x0);
...
static void hdmitx_power_init(void)
{
	...
	pm_ops = dev_register_pm("hdmitx_ops", &aml_hdmitx_suspend,
		&aml_hdmitx_resume, &aml_hdmitx_poweroff, 0);
}
#endif

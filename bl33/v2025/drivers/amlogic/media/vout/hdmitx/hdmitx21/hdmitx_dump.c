...
static int dump_hdmivpfdet_show(void)
{
	...
	val = CONNECT2REG(reg);
	if (val)
		pr_info("pixcap_in_y [%x] 0x%x %d\n", reg, val, val);

	reg = VP_CMS_CSC0_PIXCAP_IN_CB_IVCTX;
	val = CONNECT2REG(reg);
	if (val)
		pr_info("pixcap_in_CB [%x] 0x%x %d\n", reg, val, val);

	reg = VP_CMS_CSC0_PIXCAP_IN_CR_IVCTX;
	val = CONNECT2REG(reg);
	if (val)
		pr_info("pixcap_in_CR [%x] 0x%x %d\n", reg, val, val);

	reg = VP_OUTPUT_MAPPING_IVCTX;
	val = CONNECT2REG(reg);
	if (val)
		pr_info("vp__output_mapping [%x] 0x%x %d\n", reg, val, val);
...
void pix_capture_dump(struct hdmitx_dev *hdev, int pixel_ivctx, int line_ivctx)
{
	u32 reg;
	u32 val;
	...
	pr_info("pix_capture x_1:%d x_2:%d y_1:%d y_2:%d\n", pixel_low, pixel_height, line_low, line_high);
	...
	if (val) {
		if (avi_index)
			pr_info("pixcap_in_y [%x] 0x%x %d div16:%d\n", reg, val, val, val / 16);
		else
			pr_info("pixcap_in_r [%x] 0x%x %d\n", reg, val, val);
	}
	...
	if (val) {
		if (avi_index)
			pr_info("pixcap_in_CB [%x] 0x%x %d div16:%d\n", reg, val, val, val / 16);
		else
			pr_info("pixcap_in_g [%x] 0x%x %d\n", reg, val, val);
	}
	...
	if (val) {
		if (avi_index)
			pr_info("pixcap_in_CR [%x] 0x%x %d div16:%d\n", reg, val, val, val / 16);
		else
			pr_info("pixcap_in_b [%x] 0x%x %d\n", reg, val, val);
	}
...
static void hdmitx_dump_frl_status(void)
{
	...
	pr_info("\n--------frl status--------\n");
	pr_info("FRL rate: %s\n", hdev->para->frl_rate < FRL_RATE_MAX ?
		rate_string[hdev->para->frl_rate] : rate_string[FRL_RATE_MAX]);

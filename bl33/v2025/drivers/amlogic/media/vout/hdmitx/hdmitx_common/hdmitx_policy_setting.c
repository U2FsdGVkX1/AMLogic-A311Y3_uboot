 ...
 * so here needs convert such string to hex value
 */
int get_hdr_strategy_priority(void)
{
	u64 hdr_strategy_priority = env_get_ulong("hdr_priority", 10, ~0UL);

	if (hdr_strategy_priority == ~0UL || hdr_strategy_priority > INT_MAX)
		return -1;
...
static bool is_preferred_50hz(void)
{
	char *preferred_50hz = env_get("preferred50hz");

	if (!preferred_50hz)
		return false;
	return strcmp(preferred_50hz, "true") == 0;
}
...
/* for kernel 6.12 59hz/23hz mode */
static bool get_preferred_frac_mode(void)
{
	char *frac_mdoe = NULL;
	bool kernel_6_12 = false;
	bool preferred_frac_mode = false;

	frac_mdoe = env_get("frac_rate_policy");
	kernel_6_12 = get_kernel_dtb_info();
	if (frac_mdoe) {
		if (frac_mdoe[0] == '1' && kernel_6_12)
			preferred_frac_mode = true;
	}
	printf("preferred frac mode: %d\n", preferred_frac_mode);
	return preferred_frac_mode;
}
...
void get_hdmi_input(struct hdmitx_dev *hdev, struct meson_policy_in *input)
{
	...
	input->con_info.is_preferred_frac_mode = get_preferred_frac_mode();
	input->con_info.is_preferred_50hz = is_preferred_50hz();
	...
	printf("ubootenv preferred_frac_mode: %d, preferred_50hz:%d\n",
		input->con_info.is_preferred_frac_mode,
		input->con_info.is_preferred_50hz);

...
int mipi_dsi_check_state(struct aml_lcd_drv_s *pdrv, uint8_t reg, uint8_t cnt)
{
	int ret = 0, i;
	uint8_t *rd_data;
	uint8_t payload[3] = {DT_GEN_RD_1, 1, 0x04};
	...
	rd_data = (uint8_t *)malloc(sizeof(uint8_t) * cnt);
	if (!rd_data) {

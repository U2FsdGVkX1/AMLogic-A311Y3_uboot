...
void tcon_lut_dma_init_t6d(struct aml_lcd_drv_s *pdrv, struct lcd_tcon_dma_ops_s *ops)
{
	ops->status = 0;
	ops->addr_list = NULL;
				  ...
				  struct tcon_core_reg_info_s *core_reg_info)
{
	...
	if (core_reg_info->header->ext_header_size &&
		...
		if (core_reg_info->header->data_byte_width == 4) {
			...
			for (j = 0; j < core_reg_info->ext_header.reg_blk_num; j++) {
				reg_info = &core_reg_info->ext_header.reg_blk_info[j];
				for (i = 0; i < reg_info->len; i++)
					lcd_tcon_write(reg_info->start + i, table32[offset++]);
			}
		} else {
			...
			for (j = 0; j < core_reg_info->ext_header.reg_blk_num; j++) {
				reg_info = &core_reg_info->ext_header.reg_blk_info[j];
				for (i = 0; i < reg_info->len; i++)
					lcd_tcon_write_byte(reg_info->start + i, table8[offset++]);
			}
		}
	} else {
	...
	if (tcon_conf && tcon_conf->tcon_init_table_post_proc)
...
static void lcd_tcon_data_init_set(struct aml_lcd_drv_s *pdrv, unsigned char *data_buf)
{
	...
	timing = pdrv->config.timing.base_timing;
	if (!timing)
		return;
	...
	if (init_header->ext_header_size) {
		...
		ext_core_tbl_len = lcd_tcon_get_core_size(tcon_conf,
...
static void lcd_tcon_axi_rmem_lut_load(unsigned int index, unsigned char *buf, unsigned int size)
{
	...
	unsigned int mem_paddr_min = 0, mem_paddr_max = 0;
	unsigned int sec_paddr_min = 0, sec_paddr_max = 0;
...
static int lcd_tcon_data_set(struct aml_lcd_drv_s *pdrv, struct tcon_mem_map_table_s *mm_table)
{
	...
	for (i = 0; i < mm_table->block_cnt; i++) {
		...
		ret = lcd_tcon_data_common_parse_set(pdrv, data_buf);
		if (ret == 0) {
...
int lcd_tcon_top_set_t3x(struct aml_lcd_drv_s *pdrv)
{
...
void lcd_tcon_global_reset_t6d(struct aml_lcd_drv_s *pdrv)
{
...
int lcd_tcon_enable_dft(struct aml_lcd_drv_s *pdrv)
{
	struct lcd_tcon_config_s *tcon_conf = get_lcd_tcon_config();
	struct tcon_mem_map_table_s *mm_table = get_lcd_tcon_mm_table();
	struct lcd_tcon_local_cfg_s *local_cfg = get_lcd_tcon_local_cfg();
	...
	//lcd_tcon_setb(0x207, 1, 4, 1);//enable pre_proc_clk, move to init_post_proc
...
int lcd_tcon_disable_uhd(struct aml_lcd_drv_s *pdrv)
{
	struct lcd_tcon_config_s *tcon_conf = get_lcd_tcon_config();

	/* demo od */
	lcd_tcon_setb(0x240, 1, 1, 1);
	/* demo demura */
	lcd_tcon_setb(0x190, 1, 30, 1);
	mdelay(20);
...
int lcd_tcon_disable_fhd(struct aml_lcd_drv_s *pdrv)
{
	struct lcd_tcon_config_s *tcon_conf = get_lcd_tcon_config();
...
int lcd_tcon_forbidden_check_uhd(void)
{
			...
			unsigned int reg)
{
	...
	unsigned int *table32 = NULL, i = 0, offset = 0;
	int find = 0;
	...
	if (lcd_debug_print_flag & LCD_DBG_PR_ADV2) {
		LCDPR("%s: reg=%#x, offset=%#x, val=%#x, find=%d\n", __func__,
			reg, offset, find ? table32[offset] : 0, find);
	}
...
int lcd_tcon_setting_check_uhd(struct aml_lcd_drv_s *pdrv, struct lcd_detail_timing_s *ptiming,
			       struct tcon_core_reg_info_s *core_reg_info, char *ferr_str, char *warn_str)
{
	unsigned int val = 0, tri_gate = 0;
...
int lcd_tcon_setting_check_fhd(struct aml_lcd_drv_s *pdrv, struct lcd_detail_timing_s *ptiming,
				 struct tcon_core_reg_info_s *core_reg_info, char *ferr_str, char *warn_str)
{
	unsigned int val = 0, tri_gate = 0;
	int ferr_len = 0, warn_len = 0, ferr_left, warn_left, ret = 0;

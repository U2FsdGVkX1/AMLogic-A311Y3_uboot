...
static void lcd_vbyone_wait_timing_stable(struct aml_lcd_drv_s *pdrv);
static void lcd_vbyone_cdr_training_hold(struct aml_lcd_drv_s *pdrv, int flag);
...
{
	lcd_vcbus_setb(VBO_VIN_CTRL + offset, 0, 4, 1);
	lcd_vcbus_setb(VBO_VIN_CTRL + offset, 0, 5, 1);
...
void lcd_vbyone_enable(struct aml_lcd_drv_s *pdrv)
{
	...
	if (pdrv->data->chip_type == LCD_CHIP_A9)
		lcd_vcbus_setb(VBO_LANE_OUTPUT + offset, 0xff, 0, 8);

...
static void lcd_bit_rate_match_phy(struct aml_lcd_drv_s *pdrv)
{
	...
	for (i = 0; i < phy_cfg->group_num; i++) {
		...
		LCDPR("bit_rate=%d, match phy[%d]=%d\n", bit_rate, i, phy->phy_clk);
		return;
	}
	if (phy_cfg->phys[0]->phy_clk) {
		LCDPR("bit_rate=%d, no phy_clk matched, use default(phy[0]=%d)\n",
		      bit_rate, phy_cfg->phys[0]->phy_clk);
	}
}
...
static void lcd_phy_match_ss(struct aml_lcd_drv_s *pdrv)
{
	...
	cconf->ss_mode = (tim->ss_mode > cconf->data->ss_mode_max) ? 0 : tim->ss_mode;
...
void lcd_get_ss(struct aml_lcd_drv_s *pdrv)
{
	...
	printf("ss_level: %d, %dppm, dep_sel=%d, str_m=%d, stable=%d\n",
	...
	printf("ss_freq: %d, %s\n", cconf->ss_freq, lcd_ss_freq_table_dft[freq_id]);
...
void lcd_update_clk_frac(struct aml_lcd_drv_s *pdrv)
{
	...
	if (cconf->data->pll_frac_set)
		cconf->data->pll_frac_set(pdrv, cconf->pll_config[0].pll_frac);
...
void lcd_clk_config_probe(struct aml_lcd_drv_s *pdrv)
{
	...
	case LCD_CHIP_A9:
		cconf = lcd_clk_config_chip_init_a9(pdrv);
		break;
#endif
	default:

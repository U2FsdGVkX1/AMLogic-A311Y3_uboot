					  ...
					  struct lcd_cus_ctrl_attr_config_s *attr_conf)
{
	...
	if (lcd_debug_print_flag & LCD_DBG_PR_NORMAL) {
		LCDPR("[%d]: %s: freq=%hu, mode=%hu\n",
			pdrv->index, CLK_ADV_PARSE, pdrv->config.timing.ss_freq,
					     ...
					     struct lcd_cus_ctrl_attr_config_s *attr_conf)
{
	...
	for (etmg_idx = 0; etmg_idx < LCD_EXT_TIMING_MAX; etmg_idx++) {
		...
		if (lcd_debug_print_flag & LCD_DBG_PR_NORMAL) {
			...
			LCDPR("[%d]: %s: timing[%d]: %s\n",
				pdrv->index, EXT_TMG_PARSE, etmg_idx, dbg_buf);
		}
		...
		void *inip, void *psec, struct lcd_cus_ctrl_attr_config_s *attr_conf)
{
	...
	for (i = 0;  i < 15; i++) {
		...
		tmg_temp.asf_mode = lcd_ini_get_val(inip, psec, str, 0);
		sprintf(str, "extend_tmg_%d_ufr_mode", i);
		tmg_temp.ufr_mode = lcd_ini_get_val(inip, psec, str, 0);
		...
		void *inip, void *psec, struct lcd_cus_ctrl_attr_config_s *attr_conf)
{
	...
	for (i = 0; i < MAX_NUM_PHY_CFGS; i++) {
		...
		if (lcd_debug_print_flag & LCD_DBG_PR_NORMAL) {
			LCDPR("%s: tuning_param[%d]: phy_clk:%d, lane_cnt:%d\n",
				TUNING_ATTR_PARSE, i,

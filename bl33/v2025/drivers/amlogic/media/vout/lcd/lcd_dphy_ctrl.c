...
static void lcd_lane_map_chip_init(struct aml_lcd_drv_s *pdrv, struct phy_config_s *phy_cfg)
{
	...
	case LCD_CHIP_A9:
		if (pdrv->index == 0) {
			phy_cfg->lane_offset = 0;
			phy_cfg->lane_mask = 0x1ff;
		} else if (pdrv->index == 1) {
			phy_cfg->lane_offset = 5;
			phy_cfg->lane_mask = 0x3e0;
		}
		break;
...
static int lcd_lvds_get_valid_port_num(struct aml_lcd_drv_s *pdrv,
...
static void lcd_lvds_lane_init(struct aml_lcd_drv_s *pdrv, unsigned int *map0, unsigned int *map1)
{
	...
	ret = lcd_lvds_get_valid_port_num(pdrv, &valid_port);
	if (ret)
		return;
	...
	if (lcd_debug_print_flag & LCD_DBG_PR_NORMAL) {
		LCDPR("[%d]: %s: dual_port=%d, valid_port s0=0x%x, e0=0x%x, s1=0x%x, e1=0x%x\n",
		      pdrv->index, __func__, pdrv->config.control.lvds_cfg.dual_port,
...
static void lcd_lvds_lane_swap(struct aml_lcd_drv_s *pdrv, struct phy_config_s *phy_cfg)
{
	struct lvds_valid_port_s valid_port;
	...
	if (pdrv->config.control.lvds_cfg.lane_reverse) {
...
void lcd_lane_map_preset(struct aml_lcd_drv_s *pdrv)
{
	...
	switch (pdrv->config.basic.lcd_type) {
	case LCD_LVDS:
		lcd_lvds_lane_init(pdrv, &lane_map0, &lane_map1);
		break;
	case LCD_VBYONE:
...
void lcd_lane_map_update(struct aml_lcd_drv_s *pdrv)
{
	...
	if (pdrv->config.basic.lcd_type == LCD_LVDS)
		lcd_lvds_lane_swap(pdrv, phy_cfg);
...
void lcd_mipi_dphy_set(struct aml_lcd_drv_s *pdrv, unsigned char on_off)
{
	...
	//			lcd_combo_dphy_setb(COMBO_DPHY_CNTL0, 0x1, 5, 1);
	//			lcd_combo_dphy_setb(COMBO_DPHY_CNTL1, 0x0, 16, 10);
	//		}
	//
	//		lcd_combo_dphy_setb(COMBO_DPHY_CNTL1, 0x0, 0, 10);
	//	} else if (pdrv->index == 1) {
	//		lcd_combo_dphy_setb(COMBO_DPHY_CNTL1, 0x0, 16, 10);
	//	} else {
	...
	case LCD_CHIP_A9:
		if (on_off) {
			if (pdrv->index == 0) {
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 0x4, 0, 3); //sel dsi
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 3, 0, 2); // fifo en, er en
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL2, 0, 0, 10); // ch_dsi
				if (pdrv->config.control.mipi_cfg.multi_port_cfg & BIT(0)) {
					lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL2, 0, 10, 10);
				}
			} else {
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 0x4, 3, 3); //sel dsi
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 3, 2, 2); // fifo en, er en
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL2, 0, 10, 10); // ch_dsi
			}
			lcd_lane_map_set(pdrv);
		} else {
			if (pdrv->index) {
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 0, 5, 1);
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 0, 2, 2);
			} else {
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 0, 2, 1);
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 0, 0, 2);
				if (pdrv->config.control.mipi_cfg.multi_port_cfg & BIT(0)) {
					lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 0, 5, 1);
					lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 0, 2, 2);
				}
...
void lcd_lvds_dphy_set(struct aml_lcd_drv_s *pdrv, unsigned char on_off)
{
	...
	case LCD_CHIP_A9:
		if (on_off) {
			if (pdrv->index == 0) {
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 0x1, 0, 3); //sel lvds_vx1
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 0x1, 0, 1); // phy0_fifo_en
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 0x1, 1, 1); // phy0_wr_en
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL2, 0x155, 0, 10); // ch_phy0
				if (pdrv->config.control.lvds_cfg.dual_port) {
					lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL2, 0x155, 10, 10);
				}
			} else {
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 0x1, 3, 3); //sel lvds_vx1
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 0, 6, 1); //phy1_pll_sel
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 0, 7, 1); //phy1_p2s_sel
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 1, 2, 1); // phy1_wr_en
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 1, 3, 1); // phy1_wr_en
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL2, 0x2aa, 10, 10); // ch_dsi
			}
			lcd_lane_map_set(pdrv);
		} else {
			if (pdrv->index) {
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 0, 3, 1);
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 0, 2, 2);
			} else {
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 1, 0, 1);
				lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 0, 0, 2);
			}
		}
		break;
...
void lcd_vbyone_dphy_set(struct aml_lcd_drv_s *pdrv, unsigned char on_off)
{
	...
	case LCD_CHIP_A9:
		if (on_off) {
			lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 1, 0, 1);
			lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 3, 0, 2);
			lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL2, 0x5555, 0, 16);
			lcd_lane_map_set(pdrv);
		} else {
			lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL0, 0, 0, 1);
			lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 0, 0, 2);
		}
		break;
	default:
...
void lcd_dphy_set_data(struct aml_lcd_drv_s *pdrv, int data)
{
	...
	case LCD_CHIP_T3X:
		...
		case 0:
			...
			lcd_combo_dphy_setb(reg_phy_tx_ctrl0, 0, 10, 2);
			lcd_combo_dphy_setb(reg_phy_tx_ctrl0, 0, 12, 2);
			break;
		case 1:
			lcd_combo_dphy_setb(reg_phy_tx_ctrl0, 0, 8, 2);
			lcd_combo_dphy_setb(reg_phy_tx_ctrl0, 3, 10, 2);
			lcd_combo_dphy_setb(reg_phy_tx_ctrl0, 0, 12, 2);
			break;
		case 2:
			lcd_combo_dphy_setb(reg_phy_tx_ctrl0, 0, 8, 4);
	...
	case LCD_CHIP_T6W:
		...
		case 2:
			lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 0, 26, 4);
			lcd_ana_setb(ANACTRL_LVDS_TX_PHY_CNTL1, 1, 30, 1);
...
void lcd_dphy_reg_print(struct aml_lcd_drv_s *pdrv)
{
	...
	struct reg_name_set_s dphy_reg_t6d[] = {
	...
	struct reg_name_set_s dphy_reg_a9[] = {
		{ANACTRL_LVDS_TX_PHY_CNTL0, "ANACTRL_LVDS_TX_PHY_CNTL0"},
		{ANACTRL_LVDS_TX_PHY_CNTL1, "ANACTRL_LVDS_TX_PHY_CNTL1"},
		{ANACTRL_LVDS_TX_PHY_CNTL2, "ANACTRL_LVDS_TX_PHY_CNTL2"},
	};

...
void lcd_debug_window(struct aml_lcd_drv_s *pdrv, struct lcd_window_attr_s *window_attr)
{
	...
	ret = lcd_venc_op.window_attr_set(pdrv, window_attr);
	if (ret) {
		LCDERR("[%d]: %s: window attr set failed\n", pdrv->index, __func__);
		return;
	}
}
void lcd_probe_cursor(struct aml_lcd_drv_s *pdrv, struct lcd_cursor_attr_s *cursor_attr)
{
	if (!pdrv)
		return;
	if (!lcd_venc_op.venc_probe_cursor) {
		LCDERR("[%d]: %s: invalid\n", pdrv->index, __func__);
		return;
	}

	lcd_venc_op.venc_probe_cursor(pdrv, cursor_attr);
...
static void lcd_venc_mute_ctrl(struct aml_lcd_drv_s *pdrv,  unsigned char flag)
{
	if (lcd_venc_op.mute_set) {
		lcd_venc_op.mute_set(pdrv, flag);
		if (lcd_debug_print_flag & LCD_DBG_PR_NORMAL)
			LCDPR("[%d]: %s, venc mute %d\n", pdrv->index, __func__, flag);
	} else {
#ifdef CONFIG_AML_VPP
		LCDPR("[%d]: %s, vpp mute todo!!\n", pdrv->index, __func__);
		//if (flag)
		//	set_vpp_mute();
		//else
		//	set_vpp_unmute();
		//if (lcd_debug_print_flag & LCD_DBG_PR_NORMAL)
		//	LCDPR("[%d]: %s, vpp mute %d\n", pdrv->index, __func__, flag);
#endif
	}
	pdrv->boot_ctrl.mute_flag = flag ?  1 : 0;
}
...
void lcd_on_off_mute_ctrl(struct aml_lcd_drv_s *pdrv,  unsigned char flag)
{
	if (!pdrv)
		return;
	if (pdrv->boot_ctrl.init_level == LCD_INIT_LEVEL_PREBOOT && flag == 0) {
		if (lcd_debug_print_flag & LCD_DBG_PR_NORMAL)
			LCDPR("[%d]: preboot bypass unmute\n", pdrv->index);
		return;
	}
	lcd_venc_mute_ctrl(pdrv, flag);
}
...
int lcd_venc_probe(struct aml_lcd_data_s *pdata)
{
	...
	case LCD_CHIP_T6D:
	case LCD_CHIP_T6W:
		ret = lcd_venc_op_init_t6d(&lcd_venc_op);
		break;
#endif
	...
	case LCD_CHIP_A9:
		ret = lcd_venc_op_init_a9(&lcd_venc_op);
		break;
#endif
	case LCD_CHIP_S6:
	default:

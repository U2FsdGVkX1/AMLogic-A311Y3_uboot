...
#include <irq.h> //#include <linux/irqreturn.h>
#include <interrupt.h>
...
static enum dptx_aux_status_t dptx_aux_wait_response(struct dptx_hw_common *tx_comm)
{
	...
	unsigned long timeout = get_timer(0) + tx_comm->tx_aux->aux_timeout_ms;
...
ssize_t dptx_aux_xfer(struct dptx_hw_common *tx_comm, struct dptx_aux_msg *msg)
{
	...
	do {
		tmp_cnt = 0;
		remaining = msg->size - accessed_bytes;

		if (remaining > DP_AUX_MAX_PAYLOAD_BYTES)
			to_access = DP_AUX_MAX_PAYLOAD_BYTES;
		else
			to_access = (u8)(remaining & 0xff);

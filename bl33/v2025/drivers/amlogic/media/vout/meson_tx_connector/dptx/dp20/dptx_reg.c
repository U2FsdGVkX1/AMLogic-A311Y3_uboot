...
int dptx20_reg_write(struct dptx_hw_common *tx_comm, u32 io_type, u32 offset, u32 value)
{
	...
	writel(value, tx_comm->hw_base.regs_region[DPTX_COR_REG_IDX] + offset);
	return 0;
}
...
u32 dptx20_reg_read(struct dptx_hw_common *tx_comm, u32 io_type, u32 offset)
{
	...
	return readl(tx_comm->hw_base.regs_region[DPTX_COR_REG_IDX] + offset);
}
	...
	u32 addr, u32 value, u32 offset, u32 len)
{
	...
	data32 = dptx20_reg_read(tx_comm, io_type, addr);
	data32 &= ~(((1 << len) - 1) << offset);
	data32 |= (value & ((1 << len) - 1)) << offset;
	dptx20_reg_write(tx_comm, io_type, addr, data32);
}

static inline u32 get_reg_addr(struct dptx_hw_common *tx_comm, u32 idx)
{
	WARN_ON(!tx_comm);

	if (idx >= REG_IDX_MAX) {
		DPTX_ERROR("%s check reg type index 0x%x not supported\n", idx);
		return 0;
	}
	return tx_comm->hw_base.regs_region[idx];
}
...
{
	u32 val = 0;
	u32 reg = get_reg_addr(tx_comm, vaddr >> BASE_REG_OFFSET) +

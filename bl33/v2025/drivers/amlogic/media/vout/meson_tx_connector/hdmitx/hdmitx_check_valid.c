...
{
	const struct tx_timing *timing = meson_tx_mode_vic_to_timing(vic);

	if (!tx_comm || !timing || !tx_comm->tx_hw) {
	...
	if (tx_comm->res_1080p) {
		 ...
		 * On chips restricted to 1080p via efuse, the venc stops
		 * timing output if h_total exceeds 3400. Therefore, hdmitx
		 * must filter out any modes where h_total >= 3400 for these
		 * 1080p-limited chips.
		 */
	...
	/* filter max refresh_rate. */
	if (timing->v_freq > (tx_comm->max_refresh_rate * 1000)) {
		/* HDMITX_INFO("validate refresh_rate (%s)-(%d) fail\n", */
...
static bool hdmitx_check_dsc_support(struct tx_cap *hdmi_tx_cap,
				     struct rx_cap *rxcap, struct meson_tx_format_para *para)
{
		...
		if (!must_frl_flag) {
			...
			if (prxcap->dsc_1p2 == 0) {
				...
				if (tx_frl_bandwidth > tx_frl_bandwidth_cap) {
					HDMITX_DEBUG_EDID("frl bandwidth:%d exceed tx_cap:%d\n",
							  tx_frl_bandwidth, tx_frl_bandwidth_cap);
					return -EPERM;
				}
				if (tx_frl_bandwidth > rx_frl_bandwidth_cap) {
					HDMITX_DEBUG_EDID("frl bandwidth:%d exceed rx_cap:%d\n",

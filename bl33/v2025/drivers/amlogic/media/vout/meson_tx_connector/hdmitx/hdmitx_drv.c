...
static void amhdmitx_clktree_probe(struct device *hdmitx_dev, struct hdmitx_common *tx_comm)
{
	...
	if (!tx_comm) {
		HDMITX_ERROR("[%s]: hdmitx common is NULL\n", __func__);
		return;
	}
...
static int amhdmitx_get_dt_info(struct platform_device *pdev, struct hdmitx_common *tx_comm)
{
	...
	if (pdev->dev.of_node) {
		...
		if (!ret)
			tx_comm->hdr_state->hdr_8bit_en = !!val;
...
static void hdmitx_early_suspend(struct early_suspend *h)
{
	...
	need_rst_ratio = hdmitx_find_vendor_ratio(tx_comm->base.edid_buf);
	...
	tx_comm->hdcptx_comm.suspend_flag = true;
...
struct meson_tx_helper_ops hdmitx_common_helper_ops = {
	.create_sysfs	= NULL,
	.remove_sysfs	= NULL,
...
static int amhdmitx_probe(struct platform_device *pdev)
{
	...
	tx_comm->base.tx_hw_base = &hw_comm->tx_hw_base;
	dev_set_drvdata(device, tx_comm);

	/* init tx_common */
	tx_comm->base.pdev = device;
	meson_tx_dev_init(&tx_comm->base, &hw_comm->tx_hw_base, &hdmitx_common_helper_ops);
...
void hdmitx_common_sw_debug_func(struct hdmitx_common *tx_comm, const char *buf)
{
	...
	if (strncmp(tmpbuf, "edid_check", 10) == 0) {
		if (strncmp(tmpbuf + 10, "=0", 2) == 0)
			tx_comm->base.rxcap.edid_check = 0;
		else if (strncmp(tmpbuf + 10, "=1", 2) == 0)
			tx_comm->base.rxcap.edid_check = 1;
		else if (strncmp(tmpbuf + 10, "=2", 2) == 0)
			tx_comm->base.rxcap.edid_check = 2;
		else if (strncmp(tmpbuf + 10, "=3", 2) == 0)
			tx_comm->base.rxcap.edid_check = 3;
		HDMITX_INFO("edid_check = %d\n", tx_comm->base.rxcap.edid_check);
		return;
	} else if (strncmp(tmpbuf, "chkfmt", 6) == 0) {
		meson_tx_mode_print_all_mode_table();
		return;
		...
		if (kstrtoul(tmpbuf + 14, 10, &value) == 0)
			tx_comm->hdr_state->hdr_mute_frame = value;
		HDMITX_INFO("hdr_mute_frame: %d\n", tx_comm->hdr_state->hdr_mute_frame);
		return;
	} else if (strncmp(tmpbuf, "is_hdcp_cts_te", 14) == 0) {
		/* is hdcp cts test equipment */
		HDMITX_INFO("%d\n", hdmitx_edid_only_support_sd(&tx_comm->base.rxcap));
...
static void amhdmitx_remove(struct platform_device *pdev)
{
	...
	hw_comm = tx_comm->tx_hw;
	dev = tx_comm->hdtx_dev;
	hdmitx_common_debugfs_destroy(tx_comm);
	hdmitx_common_profs_destroy(tx_comm);

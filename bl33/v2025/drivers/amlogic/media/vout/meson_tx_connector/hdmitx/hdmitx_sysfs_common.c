			    ...
			    struct device_attribute *attr, char *buf, int size)
{
	...
	block_num = meson_tx_edid_valid_block_num(tx_comm->base.edid_buf);
	if (block_num <= 8)
		num = block_num * 128;
	else
		num = 8 * 128;
...
int meson_tx_edid_print_rx_cap(const struct rx_cap *prxcap,
			 ...
			 char *buf)
{
	...
	return meson_tx_edid_print_rx_cap(&tx_comm->base.rxcap, buf, PAGE_SIZE);
}

/* type: 1 for EDID, 2 for displayID */
static int hdmitx_load_edid_file(struct hdmitx_common *tx_comm, u32 type, char *string)
{
	size_t str_len;
	int i;
	bool valid_len;
	...
	valid_len = 0;
	/* check length for EDID, while not for displayID */
	if (type == 1) {
		for (i = 1; i <= EDID_MAX_BLOCK; i++) {
			if (str_len == (EDID_BLK_SIZE * i + 1)) {
				valid_len = 1;
				break;
			}
		}
	...
	if (edid_len > sizeof(tx_comm->base.edid_buf))
		return 0;
	buf = kzalloc(edid_len, GFP_KERNEL);
	if (!buf)
		return 0;
	...
	memcpy(tx_comm->base.edid_buf, buf, edid_len);

	kfree(buf);
	HDMITX_INFO("%s: %zu bytes loaded from string\n", __func__, str_len);
	return 1;
}
			  ...
			  const char *buf, size_t count)
{
	...
	if (!strncmp(argv[0], "save", strlen("save"))) {
		...
		if (type == 1 || type == 2) {
			...
			hdmitx_save_edid_file(tx_comm->base.edid_buf, argv[2]);
		}
...
{
	struct hdmitx_common *tx_comm = dev_get_drvdata(dev);
	const struct hdr_info *info = &tx_comm->base.rxcap.hdr_info;
...
{
	struct hdmitx_common *tx_comm = dev_get_drvdata(dev);
	const struct dv_info *dv = &tx_comm->base.rxcap.dv_info;
				 ...
				 const char *buf, size_t count)
{
	...
	HDMITX_INFO("%s %s\n", __func__, buf);
	mute_us = meson_tx_mode_get_frame_duration(&tx_comm->fmt_para.timing);
	if (strncmp(buf, "0", 1) == 0) {
		...
		if (hdmitx_find_vendor_phy_delay(tx_comm->base.edid_buf)) {
			     ...
			     char *buf)
{
	int i;
	struct hdmitx_common *tx_comm = dev_get_drvdata(dev);
	enum hdmi_vic *vesa_t = &tx_comm->base.rxcap.vesa_timing[0];
	int pos = 0;

	for (i = 0; vesa_t[i] && i < VESA_MAX_TIMING; i++) {
		const struct tx_timing *timing = meson_tx_mode_vic_to_timing(vesa_t[i]);
				   ...
				   char *buf)
{
	...
	struct rx_cap *prxcap = &tx_comm->base.rxcap;
	const char *modename =
		meson_tx_mode_get_timing_name(prxcap->preferred_mode);
				...
				const char *buf, size_t count)
{
	...
	struct meson_tx_format_para tst_para;
	 ...
	 */
	if (tx_comm->frac_enable && hdmitx_is_mode_name_frac(modename)) {
		hdmitx_convert_name_frac2int(modename, conv_name);
					...
					struct device_attribute *attr, char *buf)
{
	...
	pos += snprintf(buf + pos, PAGE_SIZE, "%d\n\r",
		tx_comm->hdr_state->config_csc_en);
...
#ifdef CONFIG_AMLOGIC_HDMITX21_MODERN
#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
...
#ifdef CONFIG_AMLOGIC_HDMITX_MODERN
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!tx_comm->en_poll_rx_status) {
		if (!hdmitx_find_vendor_hdcp22_non_std(tx_comm->base.edid_buf))
			return count;
	}
	if (strncmp(buf, "1", 1) == 0) {

...
static void hdmitx_disable_encp_clk(struct hdmitx21_dev *hdev)
{
	//hd21_set_reg_bits(CLKCTRL_VID_CLK_CTRL2, 0, 2, 1);
}

static void hdmitx_enable_encp_clk(struct hdmitx21_dev *hdev)
{
	//hd21_set_reg_bits(CLKCTRL_VID_CLK_CTRL2, 1, 2, 1);
}

static void hdmitx_disable_enci_clk(struct hdmitx21_dev *hdev)
{
...
static void hdmitx_disable_tx_pixel_clk(struct hdmitx21_dev *hdev)
{
...
static void set_hpll_clk_out(u32 clk)
{
	...
	case MESON_CPU_ID_S1A:
		hdmitx_set_s1a_hpll_clk_out(frac_rate, clk);
		break;
#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
	case MESON_CPU_ID_T7:
		hdmitx_set_t7_hpll_clk_out(frac_rate, clk);
		break;
#endif
	default:
		break;
...
#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
	case MESON_CPU_ID_T7:
		hdmitx_set_hpll_sspll_t7(vic);
		break;
	case MESON_CPU_ID_S5:
		hdmitx_set_hpll_sspll_s5(vic);
		break;
	case MESON_CPU_ID_S7D:
		hdmitx_set_hpll_sspll_s7d(vic);
		break;
	case MESON_CPU_ID_S7:
		hdmitx_set_hpll_sspll_s7(vic);
		break;
	case MESON_CPU_ID_S6:
		hdmitx_set_hpll_sspll_s6(vic);
		break;
#endif
	case MESON_CPU_ID_S1A:
	default:
...
static void clocks_set_vid_clk_div_for_hdmi(int div_sel)
{
	...
	if (hdev->tx_comm.tx_hw->chip_data->chip_type == MESON_CPU_ID_S1A)
		if (div_sel == VID_PLL_DIV_6p25)
			hdmitx_s1a_reset_div_clk(hdev);

	/* Enable the final output clock */
	hd21_set_reg_bits(reg_vid_pll, 1, 19, 1);
}

static void set_vid_clk_div(struct hdmitx21_dev *hdev, u32 div)
{
...
static void set_hdmitx_enc_div(struct hdmitx21_dev *hdev, u32 div)
{
	div = check_div(div);
	if (div == -1)
		return;
...
static void set_hdmitx_fe_div(struct hdmitx21_dev *hdev, u32 div)
{
	div = check_div(div);
	if (div == -1)
		return;
...
static void set_hdmitx_pnx_div(struct hdmitx21_dev *hdev, u32 div)
{
	div = check_div(div);
	if (div == -1)
		return;
...
static void set_hdmitx_pixel_div(struct hdmitx21_dev *hdev, u32 div)
{
	div = check_div(div);
	if (div == -1)
		return;
...
#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
static void set_hdmitx_s5_htx_pll(struct hdmitx21_dev *hdev)
{
	...
	if (para->tx_hw_para.hdmitx_hw_para.frl_rate != FRL_NONE) {
		...
		case FRL_3G3L:
			...
			break;
		case FRL_6G3L:
			hdmitx_set_s5_htxpll_clk_out(6000000, 4);
			break;
		case FRL_6G4L:
			hdmitx_set_s5_htxpll_clk_out(6000000, 2);
			break;
		case FRL_12G4L:
			hdmitx_set_s5_htxpll_clk_out(6000000, 1);
			break;
		case FRL_8G4L:
			hdmitx_set_s5_htxpll_clk_out(4000000, 1);
			break;
		case FRL_10G4L:
	...
	hdmitx_set_s5_htxpll_clk_out(htx_vco, div);
}
...
void hdmitx_set_s7_htx_pll(struct hdmitx21_dev *hdev)
{
	...
	hdmitx_set_s7_htxpll_clk_out(htx_vco, div);
}
...
void hdmitx_set_s7d_htx_pll(struct hdmitx21_dev *hdev)
{
	...
	hdmitx_set_s7d_htxpll_clk_out(htx_vco, div);
}
#endif
...
void hdmitx_set_s6_htx_pll(struct hdmitx21_dev *hdev)
{
	...
	hdmitx_set_s6_htxpll_clk_out(htx_vco, div);
}

static void set_hdmitx_htx_pll(struct hdmitx21_dev *hdev,
...
#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
static void hdmitx_set_fpll_without_dsc(struct hdmitx21_dev *hdev)
{
...
static void hdmitx_set_fpll_with_dsc(struct hdmitx21_dev *hdev)
{
	/* unit in khz */
	u32 fpll_vco = 2376000;
	u32 div = 1;
 ...
 */
static bool test_pixel_clk(struct hdmitx21_dev *hdev, const struct hw_enc_clk_val_group *t)
{
	u32 idx;
	u32 calc_pixel_clk;
	u32 msr_pixel_clk;
...
void hdmitx21_set_clk(struct hdmitx21_dev *hdev)
{
	...
	case MESON_CPU_ID_T7:
		...
		for (i = 0; i < SET_CLK_MAX_TIMES; i++) {
			...
			//hdmitx_set_t7_hpll_clk_out(frac_rate, 5940000); /* TODO, for t7 */
...
#ifndef CONFIG_AMLOGIC_ZAPPER_CUT
	case MESON_CPU_ID_S7:
		hdmitx_disable_s7_plls(hdev);
		set_hdmitx_htx_pll(hdev, &test_clks);
		break;
	case MESON_CPU_ID_S7D:
		hdmitx_disable_s7d_plls(hdev);
		set_hdmitx_htx_pll(hdev, &test_clks);
		break;
	case MESON_CPU_ID_S6:
		hdmitx_disable_s6_plls(hdev);
		set_hdmitx_htx_pll(hdev, &test_clks);
		break;
#endif
	case MESON_CPU_ID_S5:
...
//in_sel     : 0:vid_pll_clk;  1:fclk_div4; 2:fclk_div3; 3:fclk_div5;

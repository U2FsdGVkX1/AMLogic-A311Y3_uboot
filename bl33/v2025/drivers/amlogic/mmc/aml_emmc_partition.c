...
int fill_ept_by_gpt(struct mmc *mmc)
{
	...
	if (resize_gpt(mmc))
		printf("resize gpt failed\n");

	ALLOC_CACHE_ALIGN_BUFFER_PAD(gpt_header, gpt_head, 1, dev_desc->blksz);

	if (gpt_verify_headers(dev_desc, gpt_head, &gpt_pte)) {
		if (gpt_repair_headers(dev_desc))
			printf("Repaired GPT failed\n");
		if (gpt_verify_headers(dev_desc, gpt_head, &gpt_pte))
			return 1;
	}
...
/*
 * 1. gpt is writted on emmc
 * parse gpt and compose ept and part_table
 *
 */
int get_ept_from_gpt(struct mmc *mmc)
{
	struct partitions *ptbl;

	if (!p_iptbl_ept)
		return -1;

	ptbl = p_iptbl_ept->partitions;

	if (part_table && part_table[0].offset != 0) {
...
static int _construct_ebr_1st_entry(struct _iptbl *p_iptbl,struct dos_partition *p_ebr, int part_num )
{
	...
	* the gap between two partition is a fixed value of PARTITION_RESERVED ,otherwise the emmc partition
...
int aml_gpt_valid(struct mmc *mmc) {
	...
	if (!dev_desc) {
		...
		ALLOC_CACHE_ALIGN_BUFFER_PAD(gpt_header, gpt_head, 1, dev_desc->blksz);
		gpt_entry *gpt_pte = NULL;
		if (gpt_verify_headers(dev_desc, gpt_head, &gpt_pte) != 0) {
...
static int aml_validate_gpt_header(gpt_header *gpt_h, lbaint_t lba, lbaint_t lastlba)
{
	u32 crc32_backup = 0;
	u32 calc_crc32;
	...
	calc_crc32 = crc32(0, (const unsigned char *)gpt_h, le32_to_cpu(gpt_h->header_size));
...
static int aml_validate_gpt_entries(gpt_header *gpt_h, gpt_entry *gpt_e)
{
	u32 calc_crc32;
...
int aml_is_valid_gpt_buf(struct blk_desc *desc, void *buf)
{
	gpt_header *gpt_h;
	gpt_entry *gpt_e;
	...
	if (aml_validate_gpt_header(gpt_h, GPT_PRIMARY_PARTITION_TABLE_LBA, desc->lba))
		return -1;

	/* determine start of GPT Entries in the buffer */
	gpt_e = buf + (le64_to_cpu(gpt_h->partition_entry_lba) * desc->blksz);
	if (aml_validate_gpt_entries(gpt_h, gpt_e))
		return -1;
...
static int update_key_info(struct mmc *mmc, unsigned char *addr, unsigned int size)
{
	...
	if (key_infos[0].stamp > key_infos[1].stamp)
		pdata->key_stamp = key_infos[0].stamp;
	else
		pdata->key_stamp = key_infos[1].stamp;
...
int mmc_key_write_backup(const char *name, unsigned char *addr, unsigned int size)
{
	int ret = 0;
	u64 blk, cnt, key_glb_offset;
	int cpy;
	...
	pdata = mmc->priv;
	key_infos[0].stamp =  pdata->key_stamp + 1;
	key_infos[0].magic = 9;
...
int mmc_key_read_backup(const char *name, unsigned char *addr, unsigned int size)
{
	int valid = 0;
	struct mmc *mmc;

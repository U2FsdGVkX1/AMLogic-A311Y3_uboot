...
int meson_execute_tuning(struct udevice *dev, uint opcode)
{
	...
	u32 old_vcfg = meson_read(mmc, MESON_SD_EMMC_CFG);
	u32 vcfg = old_vcfg;
...
tuning:
	...
	// Turn off auto clk during tuning
	vcfg &= ~CFG_AUTO_CLK;
	...
	if (best_win_start < 0) {
		...
		pr_err("%s: tuning failed, reduce freq and retunning\n",
		       mmc->cfg->name);
		goto tuning;
	...
	meson_write(mmc, old_vcfg, MESON_SD_EMMC_CFG);
	mmc_print_reg(dev);
#ifdef MMC_HS200_MODE
	for (n = 0; n < clk_div; n++) {
		if (n == clk_div - 1) {
...
static int meson_mmc_probe(struct udevice *dev)
{
	...
	val = meson_read(host->mmc, MESON_SD_EMMC_CFG);
	val &= ~CFG_AUTO_CLK;
	meson_write(host->mmc, val, MESON_SD_EMMC_CFG);

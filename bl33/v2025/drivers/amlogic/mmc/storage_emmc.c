...
void mmc_write_cali_mattern(void *addr, struct aml_pattern *table)
{
	...
	mattern[i] = cal_CRC32(0, (u8 *)addr, (vpart->size - 4));
}
...
int mmc_pattern_check(struct mmc *mmc, struct aml_pattern *table)
{
	...
	buf = (u32 *)addr;
	crc32_s = cal_CRC32(0, (u8 *)addr, (vpart->size - 4));
...
int mmc_storage_read(const char *part_name, loff_t off, size_t size, void *dest) {
	...
	} else {//the operation object is partition,the unit of operation is byte.
...
static int amlmmc_write_info_sector(struct mmc *mmc)
{
	...
	struct storage_emmc_boot_info *boot_rsv_info;
	struct boot_info *boot_mode_info;
	...
	u8 *buffer;
	int ret = 0, i;
	unsigned int checksum = 0, reg;

	buffer = malloc(MMC_BLOCK_SIZE);
	if (!buffer)
		return -ENOMEM;

	ret = efuse_obj_get_data("FEAT_ENABLE_HS_EMMC");
	if (ret == EFUSE_OBJ_SUCCESS && *efuse_field.data == 1) {
		memset(buffer, 0, MMC_BLOCK_SIZE);

		boot_mode_info = (struct boot_info *)buffer;
		memcpy(boot_mode_info->magic, BOOTINFO_MAGIC, 8);
		boot_mode_info->version = BOOTINFO_INFO_VER;

		boot_mode_info->emmc.ehost_cfg.src_index = 1;
		boot_mode_info->emmc.ehost_cfg.clk_div = 20;
		boot_mode_info->emmc.ehost_cfg.co_phase = 2;
		reg = readl(pdata->regbase + MESON_SD_EMMC_ADJUST);
		boot_mode_info->emmc.ehost_cfg.rx_adj = 0;

		boot_mode_info->emmc.dev_cfg.speed_mode = 1;
		boot_mode_info->emmc.dev_cfg.width_mode = 2;

		for (i = 0; i < 512; i++) {
			checksum += buffer[i];
		}
		boot_mode_info->checksum = checksum;

		printf("boot_info.version:%04x\n", boot_mode_info->version);
		printf("boot_info.checksum:%04x\n", boot_mode_info->checksum);
	} else {
		memset(buffer, 0, sizeof(*boot_rsv_info));
		boot_rsv_info = (struct storage_emmc_boot_info *)buffer;
		part = aml_get_partition_by_name(MMC_RESERVED_NAME);
		boot_rsv_info->rsv_base_addr = part->offset / MMC_BLOCK_SIZE;
		ddr_part =  aml_get_virtual_partition_by_name(MMC_DDR_PARAMETER_NAME);
		boot_rsv_info->ddr.addr = ddr_part->offset / MMC_BLOCK_SIZE;
		boot_rsv_info->ddr.size = ddr_part->size / MMC_BLOCK_SIZE;

		mask8 = boot_rsv_info->parts;
		mask8_partition_count = fill_mask8_part(mask8);

		boot_rsv_info->version = 1;
		boot_rsv_info->checksum = _calc_boot_info_checksum(boot_rsv_info);

		printf("boot_info.rsv_base_addr:\t%04x\n", boot_rsv_info->rsv_base_addr);
		printf("boot_info.ddr.addr:%04x\n", boot_rsv_info->ddr.addr);
		printf("boot_info.ddr.size:%04x\n", boot_rsv_info->ddr.size);
		printf("boot info: parts %d\n", mask8_partition_count);
		for (i = 0; i < mask8_partition_count; i++) {
			printf("boot_info.part[%d]\n", i);
			printf("\t.name:%s\n", boot_rsv_info->parts[i].name);
			printf("\t.addr:%04x\n", boot_rsv_info->parts[i].addr);
			printf("\t.size:%04x\n", boot_rsv_info->parts[i].size);
		}
		printf("boot_info.version:%04x\n", boot_rsv_info->version);
		printf("boot_info.checksum:%04x\n", boot_rsv_info->checksum);
	}
...
int mmc_gpt_read(void *source)
{
	...
	if (aml_is_valid_gpt_buf(dev_desc, (u_char *)source)) {
...
int mmc_ddr_save_para(size_t size, void *buf)
{
	return mmc_write_rsv("ddr-parameter", size, buf);
}
...
void config_storage_dev_func(struct storage_t *dev, struct mmc* mmc)
{
	...
	dev->ddr_save_para = mmc_ddr_save_para;

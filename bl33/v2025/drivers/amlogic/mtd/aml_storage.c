			  ...
			  loff_t off, size_t size, void *dest)
{
	...
	if (retlen < size) {
		ret = -EIO;
		pr_info("%s %d mtd read err, size(%d), retlen(%d)\n",
			__func__, __LINE__, (int)size, (int)retlen);
		return ret;
	}
	return ret;
}
			   ...
			   loff_t off, size_t size, void *source)
{
	...
	if (retlen < size) {
		ret = -EIO;
		pr_info("%s %d mtd write err, size(%d), retlen(%d)\n",
			__func__, __LINE__, (int)size, (int)retlen);
		return ret;
	}
	return ret;
			   ...
			   loff_t offset, size_t size, int scrub_flag, int bb_flag)
{
	...
	printf("erasing from 0x%llx, length 0x%x\n",
			   offset, (int)size);
...
static u64 mtd_store_boot_copy_size(const char *part_name)
{
	...
	if (store_is_fastboot() && (store_get_type() == BOOT_SNOR)) {
		if (!strcmp(part_name, "bootloader"))
			return mtd_store_size(BOOT_BL2) + mtd_store_size(BOOT_TPL);

		return mtd_store_size(part_name);
	}
				      ...
				      u_char *source, struct mtd_info *mtd)
{
	...
	for (i = 0; i < page_cnt; i++) {
		...
		if (!page_info_is_page(mtd, (int)offset_pos / mtd->writesize) &&
		    read_size < data_len) {
			page_size = min_t(size_t, data_len - read_size, page_size);
			       ...
			       u8 cpy, size_t size, void *dest)
{
	...
	for (; offset < endoff; offset += size_per_copy) {
		...
		#endif
		if (medium_type == BOOT_SNOR &&
		    (!strcmp(part_name, BOOT_LOADER) || !strcmp(part_name, BOOT_BL2)))
				...
				u8 cpy, size_t size, void *source)
{
	...
	for (; offset < endoff; offset += size_per_copy) {
		...
		if (medium_type == BOOT_SNOR &&
			...
			pr_info("write %x bytes page info to %llx\n",
				(int)page_info_len, offset);
		...
		if (store_is_fastboot() &&
			store_get_type() == BOOT_SNOR &&
			strcmp(part_name, BOOT_BL2))
			page_info_len = 0;
		pr_info("write %x bytes to %llx\n",
			(int)len, offset + page_info_len);
		ret = mtd_store_write_skip_bad(mtd,
					       offset + page_info_len,
					       &len,
					       &retlen,
					       limit,
					       (u_char *)source,
...
static int mtd_store_boot_erase(const char *part_name, u8 cpy)
{
	...
	if (!strcmp(part_name, BOOT_LOADER)) {
		if (medium_type == BOOT_SNOR && !store_is_fastboot())
...
static int nor_rsv_read(const char *name, size_t size, void *buf)
{
	...
	if (length > total) {
		pr_info("request 0x%x over the rsv size 0x%x\n",
...
static int mtd_store_get_fdt_node_offset(enum boot_type_e medium_type, u32 mem)
{
	char buf[128];
	char *fdtaddr = NULL;
	u32 mem_dtb;
	...
	if (!working_fdt) {
		pr_err("%s: working_fdt is no set, fdt add to set working_fdt\n",
		       __FILE__);
		if (mem) {
			mem_dtb = mem;
		} else {
...
static int mtd_store_param_rsv_partition(u32 mem_dtb)
{
	int parent_offset;
	printf("mtd_store_param_rsv_partition\n");
	parent_offset = mtd_store_get_fdt_node_offset(store_get_type(), mem_dtb);
	if (parent_offset > 0)
		return meson_rsv_add_dtb(working_fdt, parent_offset);

	pr_err("%s: can not get fdt node for rsv!\n",
		       __FILE__);
	return 0;
}

static int mtd_store_param_bl2_partition(u32 mem)
{
	char buf[128];
	char *fdtaddr = NULL;
	u32 mem_dtb;
...
static int mtd_store_param_boot_layout(u32 mem_dtb)
{
	struct mtd_info *mtd = mtd_store_get(0);
...
add:
	node_offset = mtd_store_get_fdt_node_offset(store_get_type(), mem_dtb);
	if (node_offset > 0) {
...
int mtd_store_param_partition(u32 mem_dtb)
{
	...
	if (medium_type == BOOT_NAND_MTD) {
		mtd_store_param_bl2_partition(mem_dtb);
		i = 1;
	}

	if (strstr(env_get("bootargs"), "mtdparts")) {
		pr_err("mtdparts already exit in bootargs!\n");
		return 0;
	}
...
int mtd_store_param_ops(u32 mem_dtb)
{
	int ret = 0;
	...
	pr_err("do mtd store param send!\n");
	ret = mtd_store_param_partition(mem_dtb);
	if (ret)
		goto error;
	ret = mtd_store_param_rsv_partition(mem_dtb);
	if (ret)
		goto error;
	ret = mtd_store_param_boot_layout(mem_dtb);

error:
	return ret;
}

int mtd_ddr_save_para(size_t size, void *buf)
{
	enum boot_type_e medium_type = store_get_type();
	int ret = 0;

	if (store_is_fastboot() && medium_type == BOOT_SNOR) {
		ret = store_erase("ddr_param", 0, 4096, 0);
		if (ret)
			goto error;

		ret = store_write("ddr_param", 0, 4096, buf);
	}
...
void mtd_store_mount_ops(struct storage_t *store)
{
	...
	store->ddr_save_para = mtd_ddr_save_para;

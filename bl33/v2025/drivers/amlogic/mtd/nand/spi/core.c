...
static int meson_spinand_block_checkbad(struct nand_device *nand, const struct nand_pos *pos)
{
	struct spinand_device *spinand = nand_to_spinand(nand);
	...
	if (spinand->bbt && !spinand->bbt_scan) {
		...
		if (status != NAND_BLOCK_GOOD)
			pr_err("%s bad block at 0x%x\n", (status == NAND_FACTORY_BAD) ?
			       "factory" : "user", (u32)offs);
		return status;
	}
...
static int meson_spinand_markbad(struct nand_device *nand, const struct nand_pos *pos)
{
	struct spinand_device *spinand = nand_to_spinand(nand);
	u8 bad_block;
	u8 *buf = NULL;
...
static int spinand_erase(struct nand_device *nand, const struct nand_pos *pos)
{
	...
	unsigned int block = (int)nanddev_pos_to_offs(nand, pos) /
...
static const struct nand_ops spinand_ops = {
	.erase = spinand_erase,
	.markbad = meson_spinand_markbad,
	.isbad = meson_spinand_block_checkbad,
};
...
static int spinand_init(struct spinand_device *spinand)
{
	...
	/* Must cleanup nanddev bbt, we use rsv bbt */

...
void page_info_set_frequency_index(unsigned char frequency_index)
{
	page_info->host_cfg.frequency_index = frequency_index;
}

void page_info_set_adj_index(unsigned char mode_rx_adj)
{
	unsigned char temp = page_info->host_cfg.mode_rx_adj;

	temp &= 0xC0;
	temp |= mode_rx_adj & 0x3f;
	page_info->host_cfg.mode_rx_adj = mode_rx_adj;
}

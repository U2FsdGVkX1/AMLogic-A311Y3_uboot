...
#include <dt-bindings/amlogic/gpio/meson-s7-gpio.h>
...
static int meson_s7_parse_dt_extra(struct meson_pinctrl *pc)
{
...
static struct meson_pinctrl_data meson_s7_periphs_pinctrl_data = {
	.name		= "periphs-banks",
	...
	.parse_dt	= meson_s7_parse_dt_extra,
};

static const struct udevice_id meson_s7_pinctrl_match[] = {
	{
		.compatible = "amlogic,meson-s7-periphs-pinctrl",
		.data = (ulong)&meson_s7_periphs_pinctrl_data,
	},
	{ }
};

U_BOOT_DRIVER(meson_s7_pinctrl) = {
	.name	= "meson-s7-pinctrl",
	.id	= UCLASS_PINCTRL,
	.of_match = of_match_ptr(meson_s7_pinctrl_match),

...
static void pwm_meson_config(struct udevice *dev, unsigned int channel)
{
	struct meson_pwm_priv *priv = dev_get_priv(dev);
	struct meson_pwm_reg *regs = priv->regs;
	struct meson_pwm_state *pwm_state = priv->pwm_state;
	...
	case MESON_PWM0:
		...
		if (priv->pwm_data->extern_clk)
			clrsetbits_le32(clk_addr,
				(0xff << 0) | (3 << 9), (pwm_state[channel].pre_div << 0 | 1 << 8));
		else
			setbits_le32(&regs->miscr,
				(pwm_state[channel].pre_div << 8 | 1 << 15));
		/*set duty*/
	...
	case MESON_PWM1:
		/*set div and clock enable*/
		if (priv->pwm_data->extern_clk)
			clrsetbits_le32(clk_addr, (0xff << 16) | (3 << 25),
				(pwm_state[channel].pre_div << 16 | 1 << 24));
		else
			setbits_le32(&regs->miscr,
				(pwm_state[channel].pre_div << 16 | 1 << 23));
		/*set duty*/
...
static void pwm_meson_config_ext(struct udevice *dev, unsigned int channel)
{
	struct meson_pwm_priv *priv = dev_get_priv(dev);
	struct meson_pwm_reg *regs = priv->regs;
	struct meson_pwm_state *pwm_state = priv->pwm_state;
...
static void meson_pwm_meson_enable(struct udevice *dev, unsigned int channel)
{
	struct meson_pwm_priv *priv = dev_get_priv(dev);
	struct meson_pwm_reg *regs = priv->regs;
	struct meson_pwm_state *pwm_state = priv->pwm_state;
...
static void meson_pwm_meson_disable(struct udevice *dev, unsigned int channel)
{
	struct meson_pwm_priv *priv = dev_get_priv(dev);
	struct meson_pwm_reg *regs = priv->regs;
	struct meson_pwm_state *pwm_state = priv->pwm_state;
...
U_BOOT_DRIVER(meson_pwm) = {
	...
	.priv_auto = sizeof(struct meson_pwm_priv),
};

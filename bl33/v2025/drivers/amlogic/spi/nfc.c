...
struct nfc_clk_provider clk_provider[MAX_CLK_PROVIDER] = {
	...
	{FIX_PLL_DIV2,	CLK_100MHZ_500,	2, 5, 2, 11},
					...
					enum NFC_STORAGE_TYPE storage_type)
{
	...
	if (init_stage || frequency_idx == 0xFF) {
		...
		if (store_is_fastboot()) {
			clk_info = &clk_provider[CLK_100MHZ_500];
			clk_info->adj = 2;
			page_info_set_frequency_index(clk_info->clk_rate);
			page_info_set_adj_index(clk_info->adj);
		}

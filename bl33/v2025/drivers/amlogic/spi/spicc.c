...
static void meson_spicc_oen_enable(struct meson_spicc_device *spicc)
{
	...
	writel(conf, spicc->base + SPICC_ENH_CTL0);
}
...
static void meson_spicc_auto_io_delay(struct meson_spicc_device *spicc)
{
	...
	writel(conf, spicc->base + SPICC_TESTREG);
}
...
static inline void meson_spicc_post_burst(struct meson_spicc_device *spicc)
{
	...
	writel(0, spicc->base + SPICC_DMAREG);
}
...
static inline void meson_spicc_tx(struct meson_spicc_device *spicc)
{
	...
	while (spicc->tx_remain &&
		...
		writel(meson_spicc_pull_data(spicc),
...
static void meson_spicc_setup_dma_burst(struct meson_spicc_device *spicc)
{
	...
	writel((u32)(unsigned long)spicc->tx_buf, spicc->base + SPICC_DRADDR);
	writel((u32)(unsigned long)spicc->rx_buf, spicc->base + SPICC_DWADDR);
	...
	writel(0x3 << 4, spicc->base + SPICC_LD_CNTL0);
	/* Setup burst length */
	writel((words << 16) | words, spicc->base + SPICC_LD_CNTL1);
#else
	/* Setup burst length */
		...
		const void *dout, void *din, unsigned long flags)
{
	...
	dma_addr_t dma_addr_dout, dma_addr_din;
#endif
	int ret = 0;
...
#ifndef CONFIG_SPICC_DMA_SRAM
	if (spicc->using_dma) {
		dma_addr_dout = dma_map_single((void *)dout, bitlen >> 3, DMA_TO_DEVICE);
		dma_addr_din = dma_map_single(din, bitlen >> 3, DMA_FROM_DEVICE);
	}
#endif
...
#ifndef CONFIG_SPICC_DMA_SRAM
	if (spicc->using_dma) {
		dma_unmap_single(dma_addr_dout, bitlen >> 3, DMA_TO_DEVICE);
		dma_unmap_single(dma_addr_din, bitlen >> 3, DMA_FROM_DEVICE);
	}
#endif
...
static int spicc_probe(struct udevice *bus)
{
	...
	writel(SPICC_ENABLE | SPICC_MODE_MASTER
...
U_BOOT_DRIVER(spicc) = {
	...
	.priv_auto = sizeof(struct meson_spicc_device),

...
union spicc_cfg_spi {
	...
	struct  {
		...
		u32		sclk_out_cnt:2;
		u32		mosi_out_idle:2;
		u32		ss_trailing_gap:15;
		u32		ss_leading_gap_high:4;
		u32		rsv:1;
	} b;
};
...
#define SPI_XFER_SCLK_OUT_CNT	GENMASK(18, 17)
#define SPI_XFER_MOSI_OUT_IDLE	GENMASK(20, 19)
#define SPI_XFER_LEADING_GAP	GENMASK(25, 21)
#define SPI_XFER_TRAILING_GAP	GENMASK(30, 26)

static inline void spicc_flush_dcache(unsigned long addr, int sz)
{
	flush_dcache_range(addr, addr + sz);
}

static inline void spicc_invalidate_dcache(unsigned long addr, int sz)
{
...
static void spicc_dump_sg(struct spicc_sg_link *sg_table, bool is_tx)
{
	...
	do {
		...
		print_buffer((ulong)sg, buf, 4, sizeof(*sg) / sizeof(u32), 0);
		if (sg->eoc)
			break;
		sg++;
	} while (1);
...
static void spicc_dump(struct spicc_device *spicc, struct spicc_descriptor *desc)
{
	...
	do {
		...
		print_buffer((ulong)desc, buf, 4, sizeof(*desc) / sizeof(u32), 0);

		if (!desc->tx_paddr)
			printf("no tx\n");
		else if (desc->cfg_start.b.tx_data_mode == SPICC_DATA_MODE_SG)
			spicc_dump_sg((struct spicc_sg_link *)(unsigned long)desc->tx_paddr, 1);
		else
			printf("tx_mem@%llu\n", desc->tx_paddr);
		...
		if (!desc->rx_paddr)
			...
			spicc_dump_sg((struct spicc_sg_link *)(unsigned long)desc->rx_paddr, 0);
		else
			printf("rx_mem@%llu\n", desc->rx_paddr);
...
static int spicc_wait_complete(struct spicc_device *spicc, u32 flags, int len)
{
	unsigned long time;
	u32 sts;

	time = 8 * len / (spicc->real_speed_hz / 1000); /* unit ms */
...
static void spicc_import_config(struct spicc_device *spicc, unsigned long flags)
{
	...
	spicc->cfg_spi.b.sclk_out_cnt = FIELD_GET(SPI_XFER_SCLK_OUT_CNT, flags);
	spicc->cfg_spi.b.mosi_out_idle = FIELD_GET(SPI_XFER_MOSI_OUT_IDLE, flags);

	time_us = FIELD_GET(SPI_XFER_LEADING_GAP, flags);
	rate = DIV_ROUND_UP(spicc->real_speed_hz, 1000);
	cycles = DIV_ROUND_UP(time_us * rate, 1000);
	spicc->cfg_bus.b.ss_leading_gap = cycles & 0xF;
	spicc->cfg_spi.b.ss_leading_gap_high = (cycles >> 4) & 0xF;

	time_us = FIELD_GET(SPI_XFER_TRAILING_GAP, flags);
	rate = DIV_ROUND_UP(clk_get_rate(&spicc->spi_clk), 1000);
	cycles = DIV_ROUND_UP(time_us * rate, 1000);
	spicc->cfg_spi.b.ss_trailing_gap = cycles & 0x7FFF;
}

static int spicc_xfer_pio(struct spicc_device *spicc, const void *tx_buf,
			  void *rx_buf, int len, bool end)
{
	const u8 *t8 = (const u8 *)tx_buf;
	u8 *r8 = (u8 *)rx_buf;
	const u16 *t16 = (const u16 *)tx_buf;
	u16 *r16 = (u16 *)rx_buf;
	const u32 *t32 = (const u32 *)tx_buf;
	u32 *r32 = (u32 *)rx_buf;
	int ret;
...
static struct spicc_sg_link *spicc_map_buf(unsigned long *addr_table, u32 *len_table, int nents)
{
...
static void spicc_sg_flush_dache(struct spicc_sg_link *sg_table)
{
	...
	spicc_flush_dcache((unsigned long)sg_table,
...
static void spicc_sg_invalidate_dcache(struct spicc_sg_link *sg_table)
{
	...
	spicc_invalidate_dcache((unsigned long)sg_table,
			sizeof(*sg) * spicc_get_sg_num(sg_table));
}
...
static void spicc_desc_flush_dcache(struct spicc_descriptor *desc_table)
{
	...
	spicc_flush_dcache((unsigned long)desc_table,
	...
	while (1) {
		...
		if (desc->cfg_start.b.tx_data_mode == SPICC_DATA_MODE_SG)
			spicc_sg_flush_dache((struct spicc_sg_link *)(unsigned long)desc->tx_paddr);
		else if (desc->cfg_start.b.tx_data_mode == SPICC_DATA_MODE_MEM)
			spicc_flush_dcache((unsigned long)desc->tx_paddr, len);
		if (desc->cfg_start.b.rx_data_mode == SPICC_DATA_MODE_SG)
			spicc_sg_flush_dache((struct spicc_sg_link *)(unsigned long)desc->rx_paddr);
		else if (desc->cfg_start.b.rx_data_mode == SPICC_DATA_MODE_MEM)
			spicc_flush_dcache((unsigned long)desc->rx_paddr, len);
...
static void spicc_desc_invalidate_dcache(struct spicc_descriptor *desc_table)
{
	...
	while (1) {
		...
		len *= desc->cfg_start.b.block_num;
		if (desc->cfg_start.b.tx_data_mode == SPICC_DATA_MODE_SG)
			spicc_sg_invalidate_dcache((struct spicc_sg_link *)(unsigned long)desc->tx_paddr);
		else if (desc->cfg_start.b.tx_data_mode == SPICC_DATA_MODE_MEM)
			spicc_invalidate_dcache((unsigned long)desc->tx_paddr, len);
		if (desc->cfg_start.b.rx_data_mode == SPICC_DATA_MODE_SG)
			spicc_sg_invalidate_dcache((struct spicc_sg_link *)(unsigned long)desc->rx_paddr);
		else if (desc->cfg_start.b.rx_data_mode == SPICC_DATA_MODE_MEM)
			spicc_invalidate_dcache((unsigned long)desc->rx_paddr, len);
		if (desc->cfg_start.b.eoc)
			break;
		desc++;
	}
	spicc_invalidate_dcache((unsigned long)desc_table,
			sizeof(*desc) * spicc_get_desc_num(desc_table));
}
...
{
	unsigned long tx_addr = (unsigned long)xfer->tx_buf;
	unsigned long rx_addr = (unsigned long)xfer->rx_buf;
	unsigned int xfer_blocks = xfer->len / spicc->bytes_per_word;
	unsigned int blocks, len, desc_used_num = 0;
	bool first_desc = 1;

	spicc_import_config(spicc, xfer->flags);
	while (xfer_blocks) {
		blocks = min_t(unsigned int, xfer_blocks, SPICC_BLOCK_MAX);
		...
		desc->cfg_start.b.block_num = blocks & 0xfffff;
		...
		if (tx_addr && !(xfer->flags & SPI_XFER_NO_TX)) {
			...
			if (spicc->config_data_mode == SPICC_DATA_MODE_SG)
				desc->tx_paddr = (u64)(unsigned long)spicc_map_buf(&tx_addr, &len, 1);
			else if (first_desc)
				desc->tx_paddr = tx_addr;
			else
				desc->tx_paddr = 0;
			tx_addr += len;
		}
		...
		if (rx_addr && !(xfer->flags & SPI_XFER_NO_RX)) {
			...
			if (spicc->config_data_mode == SPICC_DATA_MODE_SG)
				desc->rx_paddr = (u64)(unsigned long)spicc_map_buf(&rx_addr, &len, 1);
			else if (first_desc)
				desc->rx_paddr = rx_addr;
			else
				desc->rx_paddr = 0;
			rx_addr += len;
		}
...
static void spicc_destroy_desc_table(struct spicc_descriptor *desc_table)
{
	...
	desc = desc_table;
	while (1) {
		if (desc->tx_paddr && desc->cfg_start.b.tx_data_mode == SPICC_DATA_MODE_SG)
			free((void *)(unsigned long)desc->tx_paddr);
		if (desc->rx_paddr && desc->cfg_start.b.rx_data_mode == SPICC_DATA_MODE_SG)
			free((void *)(unsigned long)desc->rx_paddr);
...
{
	u64 paddr = (u64)(unsigned long)desc_table;
	int ret;
	...
	spicc_writel(spicc, paddr & 0xffffffff, SPICC_REG_DESC_LIST_L);
	spicc_writel(spicc, (paddr >> 32) | SPICC_DESC_PENDING,
...
static int spicc_nor_read_id(struct spi_slave *slave, u8 *id_buf)
{
	u8 cmd = NOR_CMD_RDID;
	int ret;

	ret = spi_xfer(slave, 8, &cmd, 0, SPI_XFER_BEGIN);
	if (ret < 0)
		return ret;
...
static int spicc_nor_rdsr(struct spi_slave *slave, u8 cmd, u8 *sta)
{
	u8 dout[2], din[2];
	int ret;
...
static int spicc_nor_wrsr(struct spi_slave *slave, u8 cmd, u8 sta)
{
	u8 dout[2];
...
static inline int spicc_nor_cmd(struct spi_slave *slave, u8 cmd)
{
...
static void spicc_print_buffer(u8 *buf, int len)
{
	int i;
...
void spicc_nor_test(int bus, int cs, int speed, u32 addr, int len)
{
	...
	if (_spi_get_bus_and_cs(bus, cs, speed, 0, "spi_generic_drv",
			       str, &dev, &slave))
		return;
...
U_BOOT_DRIVER(spicc_v2) = {
	...
	.priv_auto = sizeof(struct spicc_device),

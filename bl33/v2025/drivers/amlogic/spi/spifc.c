...
#if defined(CONFIG_SPIFC_COMPATIBLE_TO_APPOLO)
...
static void spifc_chipselect(struct udevice *dev, bool select)
{
	...
	struct spifc_platdata *plat = dev_get_plat(bus);
...
#endif
	ret = gpio_request_by_name(bus, "cs-gpios", 0, &priv->cs_gpios, 0);
	if (ret) {
...
static int spifc_ofdata_to_platdata(struct udevice *bus)
{
	...
	plat->speed = dev_read_u32_default(bus, "max-frequency", 40000000);
	/* default 2 because some board only have 2 spifc io */
	plat->io_num = dev_read_u32_default(bus, "max-io", 2);
	plat->max_cs = dev_read_u32_default(bus, "max-cs", 2);
	// plat->speed = fdtdec_get_uint(blob, node, "max-frequency", 40000000);
	// /* default 2 because some board only have 2 spifc io */
	// plat->io_num = fdtdec_get_uint(blob, node, "max-io", 2);
	// plat->max_cs = fdtdec_get_uint(blob, node, "max-cs", 2);
	plat->mode = 0;
	pr_debug("spifc freq %d, max io %d, reg %p\n",
		 plat->speed, plat->io_num,
		 (void *)plat->reg);
	return 0;
}
...
U_BOOT_DRIVER(spifc) = {
	...
	.id = UCLASS_SPI,
	.of_match = spifc_ids,
	.of_to_plat = spifc_ofdata_to_platdata,
	.plat_auto = sizeof(struct spifc_platdata),
	.priv_auto = sizeof(struct spifc_priv),

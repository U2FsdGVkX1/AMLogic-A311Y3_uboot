			       ...
			       u8 *buf, int len, unsigned long flags)
{
	...
	val = readl(SPIFC_USER_CTRL1);
	val &= ~((3 << 28) | (0xff << 20) | (1 << 19) | (3 << 17) |
			(3 << 15) | (1 << 13) | (1 << 12) | (0x3FF << 0));
	val |= ((1 << 30) | ((priv->save_cmd << 20)) | ((!!priv->save_addr_len) << 19) |
			(bits << 15) | (1 << 14) | (len << 0));
	writel(val, SPIFC_USER_CTRL1);
...
static int spifc_user_ahb_cmd_din(struct spifc_priv *priv, u8 *buf, int len)
{
	u64 *p;
	int len64, i;
	u64 *spifc_ahb_addr;

...
{
	uint32_t *input = (uint32_t *)(uintptr_t)storage_share_in_base;
	uint8_t *data;
	uint32_t namelen;
#if CONFIG_AML_FLUSH_CACHE
	uint32_t *input_tmp;
...
{
	uint32_t *input = (uint32_t *)(uintptr_t)storage_share_in_base;
	uint32_t *output = (uint32_t *)(uintptr_t)storage_share_out_base;
	uint32_t namelen;
	uint8_t *name, *buf;
	uint64_t ret;
#if CONFIG_AML_FLUSH_CACHE
...
void *secure_storage_getbuffer(uint32_t *size)
{
	...
	if (storage_init_status == 1) {
		...
		*size = (uint32_t)storage_share_block_size;
		return (void *)(uintptr_t)storage_share_block_base;
	}
	else {
		*size = 0;
		return NULL;

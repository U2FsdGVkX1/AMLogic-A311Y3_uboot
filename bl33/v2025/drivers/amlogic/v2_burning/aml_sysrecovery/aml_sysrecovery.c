...
#ifndef CFG_AML_SYS_RECOVERY_CLEAR_USR_DATA
#define CFG_AML_SYS_RECOVERY_CLEAR_USR_DATA  "userdata"
#endif// #ifndef CFG_AML_SYS_RECOVERY_CLEAR_USR_DATA
...
#ifdef CFG_AML_SYS_RECOVERY_CLEAR_USR_DATA
...
{
	const char *const _usrDataParts[] = {CFG_AML_SYS_RECOVERY_CLEAR_USR_DATA,};
	const int   dataPartsNum          = ARRAY_SIZE(_usrDataParts);
	int partIndex = 0;
	int ret = 0;
	char cmdbuf[128];
	...
	for (partIndex = 0; partIndex < dataPartsNum; ++partIndex) {
		...
		if (ret)
			DWN_WRN("Wrn:failed in cmd:%s\n", cmdbuf);
	}

	return ret;
}
#endif//#ifdef CFG_AML_SYS_RECOVERY_CLEAR_USR_DATA
 ...
 */
static int optimus_sysrec_burn_package_from_partition(const char *partName,
						      const unsigned int needVerifyWhileBurn,
						      const unsigned int verifyPackageBeforeBurn)
{
...
{
	int rcode = 0;
	unsigned int needVerify         = (1 < argc) ? simple_strtoul(argv[1], NULL, 0) : 0;
	unsigned int verifyPackageBeforeBurn = (2 < argc) ? simple_strtoul(argv[2], NULL, 0) : 0;

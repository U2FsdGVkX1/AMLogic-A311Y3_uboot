...
static int assert_key_is_pressed_in_a_period(unsigned int nMillSeconds, const char *keyName, const char *keyType)
{
	...
	while (is_sys_recovery_key_pressed(hKey)) {
		const unsigned int pressTime = (unsigned)get_timer(start) ;

...
int optimus_buf_manager_init(const unsigned int mediaAlignSz)
{
	if (OPTIMUS_DOWNLOAD_TRANSFER_BUF_ADDR != (uintptr_t)_bufManager.transferBuf) {
	...
	*(u32 *)(&_bufManager.transferUnitSz) = (OPTIMUS_WORK_MODE_USB_PRODUCE >= optimus_work_mode_get())
	...
	DWN_DBG("transfer=0x%p, transferBufSz=0x%x, transferUnitSz=0x%x, writeBackUnitSz=0x%x,"
		"totalSlotNum=%d\n", _bufManager.transferBuf,
		_bufManager.transferBufSz,  _bufManager.transferUnitSz,
		_bufManager.writeBackUnitSz,        _bufManager.totalSlotNum);
...
int optimus_buf_manager_tplcmd_init(const char *mediaType,  const char *partName,   const u64 partBaseOffset,
				    ...
				    const unsigned int itemSizeNotAligned /*if item offset 3 and bytepercluste 4k, then it's 4k -3 */)
{
	...
	if (!strcmp(imgType, "ubifs"))
		cacheAll2Mem = 1;
#endif // #if OPTIMUS_BURN_TARGET_SUPPORT_UBIFS

	if (!strcmp("sparse", imgType) || itemSizeNotAligned)
	...
	if (!strcmp("bootloader", partName) || !strcmp("_aml_dtb", partName) ||
	    ...
	    !strcmp("gpt", partName) || !strcmp(CONFIG_TPL_PART_NAME, partName)) {
#else
	    !strcmp("gpt", partName)) {
	...
	if (!cacheAll2Mem)
		cacheAll2Mem = !strcmp("mem", mediaType) ;
	if (!cacheAll2Mem)
		cacheAll2Mem = pktSz4BufManager <= _bufManager.transferUnitSz;
	if (cacheAll2Mem) {
		writeBackUnitSz             = pktSz4BufManager + _bufManager.transferUnitSz - 1;
		writeBackUnitSz             >>= OPTIMUS_DOWNLOAD_SLOT_SZ_SHIFT_BITS;
		writeBackUnitSz             <<= OPTIMUS_DOWNLOAD_SLOT_SZ_SHIFT_BITS;
...
int optimus_buf_manager_get_buf_for_bulk_transfer(char **pBuf, const unsigned int wantSz, const unsigned int sequenceNo, char *errInfo)
{
	const unsigned int totalSlotNum      = _bufManager.totalSlotNum;
	...
	const u8 *BufBase = (OPTIMUS_MEDIA_TYPE_MEM != _bufManager.destMediaType)  ? _bufManager.transferBuf :
		(u8 *)(uintptr_t)_bufManager.partBaseOffset ;
...
int optimus_buf_manager_report_transfer_complete(const u32 transferSz, char *errInfo)
{
	...
	const u64 leftPktSz              = (totalTransferSz > _bufManager.tplcmdTotalSz) ? 0 : (_bufManager.tplcmdTotalSz - totalTransferSz);
	const u32 thisWriteBackSz        = (_bufManager.transferUnitSz == transferSz) ?
					   _bufManager.writeBackUnitSz :
					   ((u32)totalTransferSz % _bufManager.writeBackUnitSz);
	const u8 *BufBase = (OPTIMUS_MEDIA_TYPE_MEM != _bufManager.destMediaType)  ? _bufManager.transferBuf :
		(u8 *)(uintptr_t)_bufManager.partBaseOffset ;
	...
	if (_bufManager.totalSlotNum == _bufManager.nextWriteBackSlot) {
		...
		const unsigned int reserveNotAlignSz = leftPktSz ? _bufManager.itemOffsetNotAlignClusterSz_f : 0;//reserve
...
int is_largest_data_transferring(void)
{
	...
	//last packet may <=  64k, var totalSlotNum may > _bufManager.tplcmdTotalSz
	return (PKT_TRANSFER_STA_WORKING == _bufManager.pktTransferSta) &&
		(_bufManager.tplcmdTotalSz > totalTransferSz);
}
...
int optimus_buf_manager_get_command_data_for_upload_transfer(u8 *cmdDataBuf, const unsigned int bufLen)
{
	const unsigned int totalSlotNum      = _bufManager.totalSlotNum;
	const u64 totalTransferSz        = ((u64)totalSlotNum) *_bufManager.transferUnitSz;
	const u64 leftPktSz              = (totalTransferSz > _bufManager.tplcmdTotalSz) ? 0 : (_bufManager.tplcmdTotalSz - totalTransferSz);
	const unsigned int thisTransDataLen = (leftPktSz > _bufManager.transferUnitSz) ? _bufManager.transferUnitSz : ((u32)leftPktSz);
	...
	*(unsigned int *)(cmdDataBuf + 0) = 0xefe8;
	*(unsigned int *)(cmdDataBuf + 4) = thisTransDataLen;//Fill transfer data length of this bulk transfer

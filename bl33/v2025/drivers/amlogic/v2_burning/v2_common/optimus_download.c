...
extern unsigned int get_multi_dt_entry(unsigned int long fdt_addr);
int is_optimus_storage_inited(void);

#if !defined(CONFIG_UNIFY_KEY_MANAGE)
int v2_key_read(const char *keyName, u8 *keyVal, const unsigned int keyValLen, char *errInfo,
		unsigned int *fmtLen)
{
...
unsigned int v2_key_burn(const char *keyName, const u8 *keyVal, const unsigned int keyValLen,
			 char *errInfo)
{
	DWN_ERR("burn key not supported as CONFIG_UNIFY_KEY_MANAGE undef!!");
	return OPT_DOWN_FAIL;
...
static unsigned int long _dtb_is_loaded;
static unsigned int long _gpt_is_loaded;
...
struct ImgBurnInfo{
	...
	//needed private info when verify, for example when we read ext4 to sparse file
	u8  burnInfoPrivate[IMG_BURN_INFO_SZ - 32 - sizeof(void *) - sizeof(u64)  *5];
};
...
u32 optimus_cb_simg_write_media(const unsigned int destAddrInSec, const unsigned int dataSzInBy, const char *data)
{
	int ret = OPT_DOWN_OK;
...
static u32 optimus_download_normal_image(struct ImgBurnInfo *pDownInfo, u32 dataSz, const u8 *data)
{
	...
	if (!strncmp(pDownInfo->partName, "bootloader-boot", strlen("bootloader-boot"))) {
		ret = store_bootloader_ops(_STORE_BOOT_OP_WRITE, pDownInfo->partName, (void *)data, dataSz);
	} else
...
//return value is the data size that actual dealt
static u32 optimus_storage_write(struct ImgBurnInfo *pDownInfo, u64 addrOrOffsetInBy,
				 unsigned int dataSz, const u8 *data, char *errInfo)
{
	u32 burnSz = 0;
	...
	if (pDownInfo->imgBurnSta != OPTIMUS_IMG_STA_BURN_ING) {
	...
	case OPTIMUS_MEDIA_TYPE_MEM: {
		u8 *buf = (u8 *)(uintptr_t)addrOrOffsetInBy;
...
static int optimus_storage_read(struct ImgBurnInfo *pDownInfo, u64 addrOrOffsetInBy,
				unsigned int readSzInBy, unsigned char *buff, char *errInfo)
{
	int ret = 0;
	const int MediaType = pDownInfo->storageMediaType;
	...
	case OPTIMUS_MEDIA_TYPE_STORE: {
		...
		} else if (!strncmp("bootloader-", partName, strlen("bootloader-"))) {
			DWN_MSG("read %s \n", partName);
			ret = store_bootloader_ops(_STORE_BOOT_OP_READ, partName, buff, readSzInBy);
		} else {
...
static u32 optimus_func_download_image(struct ImgBurnInfo *pDownInfo, u32 dataSz,
				       const u8 *data, char *errInfo)
{
	int burnSz = 0;
	int ret = 0;
				    ...
				    const u64 imgSz, const char *imgType, const char *mediaType, const u64 partBaseOffset)
{
	...
	if (OPTIMUS_MEDIA_TYPE_MEM > pDownInfo->storageMediaType) {//if command for burning partition
		if (strcmp("bootloader", partName) &&
		    strncmp(partName, "bootloader-boot", strlen("bootloader-boot")) &&
...
int optimus_parse_img_download_info(const char *partName, const u64 imgSz, const char *imgType,
				    const char *mediaType, const u64 partBaseOffset)
{
...
static int optimus_sha1sum_verify_partition(const char *partName, const u64 verifyLen, const u8 imgType, u8 *genSum)
{
	...
	if (imgType == IMG_TYPE_BOOTLOADER) {
	...
	DWN_MSG("To verify part %s in fmt %s\n", partName,
		(IMG_TYPE_SPARSE == imgType) ? "sparse" : "normal");
	if (imgType == IMG_TYPE_SPARSE) {//sparse image
		for (; leftLen;) {
			u32 spHeadSz   = 0;
			u32 chunkDataLen    = 0;
...
{
	static unsigned int _isBurnComplete;
	int rc = 0;

	switch (choice) {
	case OPTIMUS_BURN_COMPLETE__POWEROFF_AFTER_POWERKEY:
		{

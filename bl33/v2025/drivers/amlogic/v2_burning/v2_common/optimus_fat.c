...
static int fat_itr_isdir(_fat_itr *itr);
...
static __u8 get_contents_vfatname_block[MAX_CLUSTSIZE] __aligned(ARCH_DMA_MINALIGN);
	...
	*gotsize = 0;
	...
	do {
		/* search for consecutive clusters */ while (actsize < maxsize) {
			...
			if (newclust && (newclust - 1) != endclust)
				goto getit;
 ...
 */
static int fat_itr_root(_fat_itr *itr, fsdata *fsdata)
{
 ...
 */
static void fat_itr_child(_fat_itr *itr, _fat_itr *parent)
{
...
static void *next_cluster(_fat_itr *itr)
{
...
static dir_entry *next_dent(_fat_itr *itr)
{
	if (itr->remaining == 0) {
		struct dir_entry *dent = next_cluster(itr);
		unsigned int nbytes = itr->fsdata->sect_size *
...
static dir_entry *extract_vfat_name(_fat_itr *itr)
{
 ...
 *    current directory
 */
static int fat_itr_next(_fat_itr *itr)
{
	dir_entry *dent;
 ...
 */
static int fat_itr_resolve(_fat_itr *itr, const char *path, unsigned int type)
{
	const char *next;
...
{
	struct fopen_para *hFile = _hFile[fd];
	uintptr_t *filePos = &(hFile->fileOffset);
	const uintptr_t oldFileOffset = *filePos;
	uintptr_t pos = oldFileOffset;
	const unsigned int clusterSz = do_fat_get_bytesperclust(fd);
	fsdata *mydata = &(hFile->fsdata);
	unsigned int curclust = hFile->curclust;
	...
	if (pos >= hFile->filesize) {
		DWN_MSG("pos 0x%"  PRIxPTR ", filesize 0x%" PRIxPTR "\n", pos, hFile->filesize);
		DWN_ERR("seek 0x%" PRIxPTR " err from wherehence %d\n", offset, wherehence);
		return -__LINE__;
	}
	const uintptr_t newFileOffset = pos; ////////int enough as fat32
	DWN_DBG("Seek 0x%" PRIxPTR " from 0x%" PRIxPTR "\n", newFileOffset, oldFileOffset);
	if (newFileOffset == oldFileOffset)
		return 0;

	//Above already update field @.fileOffset
	//following update @.curClusterIndex
	unsigned int newClusterIndex = newFileOffset / clusterSz;
	unsigned int curClusterIndex = oldFileOffset / clusterSz;
	int      nCluster        = 0;

...
typedef struct _ImgSrcIf{
	unsigned int        devIf;             //mmc/usb/store
	unsigned int        devNo;          //0/1/2
	unsigned int        devAlignSz;     //64K for store
	unsigned int        reserv2Align64;
...
HIMAGE image_open(const char *interface, const char *device, const char *part, const char *imgPath)
{
	...
	if (!strcmp("store", interface)) {
		...
		if (_exfatok > 0) {
			pImgSrcIf->pkg_sz_part1 = (unsigned int)do_exfat_get_fileSz(imgPath);
		} else {
...
#ifdef CONFIG_FS_EXFAT
		if (_exfatok > 0) {
			ret = do_exfat_fread(pFile, (u8*)&hImg->imgHead, HeadSz);
		} else {
...
#ifdef CONFIG_FS_EXFAT
		if (_exfatok > 0) {
			pImgSrcIf->devAlignSz = do_exfat_get_bytesperclust(pFile);
		} else {
...
unsigned int image_get_crc(HIMAGE hImg)
{
...
#ifdef CONFIG_FS_EXFAT
	if (_exfatok > 0) {
	    if (_hFile >= 0)do_exfat_fclose(_hFile) , _hFile = -1;
	} else {
...
{
	ImgInfo_t *imgInfo = (ImgInfo_t *)hImg;
	const unsigned int imgVer = imgInfo->imgHead.version;
...
static int _do_pkg_fseek(HIMAGE hImg, s64 pkg_offset, int wherehence)
{
	...
	if (_is_part2_pkg) {
...
#ifdef CONFIG_FS_EXFAT
			if (_exfatok > 0) {
				do_exfat_fclose(_hFile);
				_hFile = do_exfat_fopen(env_get("usb_burn_part1_img"));
			} else {
		...
		if (pkg_offset >= pkg_sz_part1) {
			...
			if (_exfatok > 0) {
				do_exfat_fclose(_hFile);
				_hFile = do_exfat_fopen(env_get("usb_burn_part2_img"));
			} else {
				do_fat_fclose(_hFile);
...
#ifdef CONFIG_FS_EXFAT
	if (_exfatok > 0) {
		i = do_exfat_fseek(_hFile, pkg_offset, wherehence);
	} else {
...
unsigned int image_item_get_first_cluster_size(HIMAGE hImg, HIMAGEITEM hItem)
{
	const ImgInfo_t *imgInfo = (ImgInfo_t *)hImg;
	ItemInfo *pItem = (ItemInfo *)hItem;
	unsigned int itemSizeNotAligned = 0;
	const unsigned int fat_bytesPerCluste = imgInfo->imgSrcIf.devAlignSz;
...
int image_item_read(HIMAGE hImg, HIMAGEITEM hItem, void *pBuf, const __u32 wantSz)
{
	...
	if (IMAGE_IF_TYPE_STORE == imgInfo->imgSrcIf.devIf) {
		...
		int rc = 0;
		const unsigned int storeBlkSz      = imgInfo->imgSrcIf.devAlignSz;
		const unsigned int offsetNotAlign = offsetInPart & (storeBlkSz - 1);
		const unsigned int sizeNotAlignInFirstBlk = storeBlkSz - offsetNotAlign;//in the the first block and its offset not aligned
		...
		if (offsetNotAlign) {
			...
			const unsigned int bufLen = sizeNotAlignInFirstBlk < wantSz ? sizeNotAlignInFirstBlk : (wantSz);
			unsigned int thisTotalReadSz = wantSz;
...
#ifdef CONFIG_FS_EXFAT
			if (_exfatok > 0) {
				readSz = do_exfat_fread(_hFile, pBuf, part1_need);
			} else {
...
#ifdef CONFIG_FS_EXFAT
		if (_exfatok > 0) {
			readSz = do_exfat_fread(_hFile, pBuf + part1_need, part2_need);
		} else {
	...
	*hasBootloader = 0;
	for (i = 0; i < totalItemNum; i++) {
		...
		if (!strcmp(AML_SYS_RECOVERY_PART, sub_type)) {
			if (OPTIMUS_WORK_MODE_SYS_RECOVERY == optimus_work_mode_get())
				continue;
		}
...
int optimus_img_item2buf(HIMAGE hImg, const char *main, const char *sub, char *buf, int *bufsz)
{
	...
	unsigned int itemSzNotAligned = 0;
	int		 alignSz	= 0;
	...
	if (itemSzNotAligned /*& 0x7*/) {//Not Aligned 8bytes/64bits, mmc dma read will failed
		const unsigned int clusterSz = image_get_cluster_size(hImg);
	...
	int rc = image_item_read(hImg, hImgItem, buf, (unsigned int)itemSz);
	...
	if (alignSz)
		memmove(buf - alignSz, buf, itemSz);
...
int get_subtype_nm_by_index(HIMAGE hImg, const char *main_type, const char **sub_type,
			    const int itemIndex)
{
	int i = 0;
	int ret = 0;
...
int optimus_img_secureboot_signed(HIMAGE hImg)
{
	...
	if (n_usb_bootloader < 4) {
		return image_item_open(hImg, "USB", "DDR_ENC") != NULL;
	}
...
static int test_pack(const char *interface, const char *device, const char *part, const char *imgPath)
{
...
#ifdef CONFIG_FS_EXFAT
		if (_exfatok > 0) {
			fileSz = do_exfat_get_fileSz(imgPath);
		} else {
	...
	for (i = 0; i < itemNr ; i++) {
		...
		unsigned int wantSz = ImagBufLen > itemSz ? (unsigned int)itemSz : ImagBufLen;
		unsigned int itemSizeNotAligned = 0;
		char *readBuf = pBuf;
		unsigned int readSz = wantSz;

...
int optimus_progress_init(const unsigned int itemSzHigh, const unsigned int itemSzLow, const u32 startStep, const u32 endStep)
{
...
int optimus_update_progress(const unsigned int thisBurnSz)
{
...
int optimus_hex_data_2_ascii_str(const unsigned char *hexData, const unsigned int nBytes, char *outStr, const unsigned int strSz)
{
	int i = 1;
	if (strSz < 2 * nBytes + 1) {
...
unsigned int add_sum(const void *pBuf, const unsigned int size)
{
	unsigned int sum	= 0;
...
#define SCPI_CMD_USB_UNBOOT_X1	0xaa	//skip current store for romboot usb
...
static void _erase_bootloader(uint64_t arg0)
{
	...
	if (SCPI_CMD_CLEAR_BOOT == arg0)
		return;//dummy as not supported
...
int optimus_erase_bootloader(const char *extBootDev)
{
	if (!strcmp("usb", extBootDev)) {
		#ifdef CONFIG_MESON_X1
		aml_set_bootsequence(SCPI_CMD_USB_UNBOOT_X1);
		#else

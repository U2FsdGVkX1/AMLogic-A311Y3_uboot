...
#define _VIDEO_DEV_OPEN "echo 1; hdmitx hpd;hdmitx get_parse_edid; echo 2; osd open;osd clear; echo 3;vout output ${outputmode};"
#else
#define _VIDEO_DEV_OPEN "echo 11; hdmitx hpd;hdmitx get_parse_edid; echo 22; osd open;osd clear; echo 33;vout output ${outputmode};"
...
static int optimus_prepare_upgrading_bmps(HIMAGE hImg)
{
	...
	if (ret && hImg) {//Failed to load logo resources from memory, then Load it from package
		unsigned int imgItemSz = 0;
		...
		const unsigned int itemSzNotAligned = image_item_get_first_cluster_size(hImg, hItem);
...
int video_res_prepare_for_upgrade(HIMAGE hImg)
{
	...
	if (!env_get("upgrade_display_init"))
		env_set("upgrade_display_init", _VIDEO_DEV_OPEN);
	ret = run_command("printenv upgrade_display_init; run upgrade_display_init;", 0);
	if (ret) {
		DWN_ERR("Fail to run cmd for video prepare for upgrade\n");
	}
...
{
	int ret = 0;
	char bmpCmd[64] = "bmp display %s; bmp scale";
	...
	sprintf(bmpCmd, "bmp display %s; bmp scale", bmpAddrEnv);
...
typedef struct _uiProgress{
	...
	unsigned int    nDownBytesOnePercent_f;//update 1% if .nDownBytesOnePercent, (==.totalDownloadBytes/totalPercents)

	//For total
	int         totalPercents_f; //Total width for display the progress bar, should < 'display_width'

	int         curPercent;//current percent of the progress
	...
	unsigned int    totalProgressBarWidth_f;//total pixel width of the progress bar, use 'display_width'/100*100 ?
	unsigned int    nProgressBarOnePercent_f;//times to display bar.if .totalProgressBarWidth=2000, .progressBarWidth=2, .totalPercents=100, then .nProgressBarOnePercent=10
	...
	unsigned int    bmpAddr_f;
	unsigned int    reservToAlign64;
} UiProgress_t;


__hdle optimus_progress_ui_request(int totalPercents_f, int startPercent, unsigned int long bmpBarAddr,
				   int display_width,  int progressBarY_f)
{
	UiProgress_t *pUiProgress = NULL;
	bmp_header_t *bmpHeadInfo = (bmp_header_t *)bmpBarAddr;
	const unsigned int commonMultiple = totalPercents_f *bmpHeadInfo->width;
...
int optimus_progress_ui_set_smart_mode(__hdle hUiProgress, const u64 smartModeTotalBytes_f, const unsigned int smartModePercents)
{
...
int optimus_progress_ui_set_unfocus_bkg(__hdle hUiProgress, unsigned int long unfocusBmpAddr)
{
	UiProgress_t *pUiProgress   = (UiProgress_t *)hUiProgress;
	bmp_header_t *bkgBmpHead    = (bmp_header_t *)unfocusBmpAddr;
	const unsigned int barHeight    = pUiProgress->progressBarHeight_f;
	const unsigned int bkgHeight    = bkgBmpHead->height;
	const unsigned int bkgWidth     = bkgBmpHead->width;
...
int optimus_progress_ui_direct_update_progress(__hdle hUiProgress, const int percents)
{
	...
	for (i = 0; i < nProgressBar; ++i) {
		...
		const int progressBarY          = pUiProgress->progressBarY_f;
		const unsigned int bmpAddr          = pUiProgress->bmpAddr_f;
		char cmd[64];
...
int optimus_progress_ui_update_by_bytes(__hdle hUiPrgress, const unsigned int nBytes)
{
	UiProgress_t *pUiProgress   = (UiProgress_t *)hUiPrgress;
	const unsigned int nDownBytesOnePercent_f = pUiProgress->nDownBytesOnePercent_f;
	const unsigned int bytesNotReport         = nBytes + pUiProgress->smartModeLeftBytes;
...
__hdle optimus_progress_ui_request_for_sdc_burn(void)
{
	...
	unsigned int barAddr = simple_strtoul(getenv("upgrade_bar_offset"), NULL, 0);
	unsigned int display_width = simple_strtoul(getenv("fb_width"), NULL, 0);
	unsigned int display_height = simple_strtoul(getenv("fb_height"), NULL, 0);
	bmp_header_t *upgrading  = (bmp_header_t *)simple_strtoul(getenv("upgrade_upgrading_offset"), NULL, 0);
	const unsigned int loadingHeight = upgrading ? upgrading->height : 0;
	const unsigned int barYCor       =
		(3*display_height + loadingHeight)/4;//display_height - (display_height/2 - loadingHeight/2)/2;
	unsigned int unfocusBmpAddr = simple_strtoul(getenv("upgrade_unfocus_offset"), NULL, 0);
...
static int do_progress_bar_test(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (!hProgressBar) {
		...
		unsigned int display_height = simple_strtoul(getenv("fb_height"), NULL, 0);
		bmp_header_t *upgrading  = (bmp_header_t *)simple_strtoul(getenv("upgrade_upgrading_offset"), NULL, 0);
		const unsigned int loadingHeight = upgrading->height;
		const unsigned int barYCor       =
			(3*display_height + loadingHeight)/4;//display_height - (display_height/2 - loadingHeight/2)/2;
		unsigned int unfocusBmpAddr = simple_strtoul(getenv("upgrade_unfocus_offset"), NULL, 0);
	...
	if (!strcmp("dir", argv[1])) {
		...
		static u64 dataBytes;
		unsigned int nBytes = simple_strtoul(argv[2], NULL, 10);

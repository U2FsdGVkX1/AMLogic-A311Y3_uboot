...
static struct
{
	...
	unsigned int parsedPacketCrc;//crc value for packet that already parsed
	...
	if (!(SPARSE_HEADER_MAJOR_VER == header->major_version &&
	   FILE_HEAD_SIZE == header->file_hdr_sz && CHUNK_HEAD_SIZE == header->chunk_hdr_sz)) {
		sperr("want 0x [%x, %x, %x], but [%x, %x, %x]\n",
		      SPARSE_HEADER_MAJOR_VER, (unsigned int)FILE_HEAD_SIZE,
		      (unsigned int)CHUNK_HEAD_SIZE, header->major_version,
		      header->file_hdr_sz, header->chunk_hdr_sz);
		return 0;
	}
...
int optimus_simg_parser_init(const u8 *source)
{
	...
	memcpy((void *)(uintptr_t)_spPacketStates.chunkInfoBackAddr, header, sizeof(sparse_header_t));
...
//      Take care the size to align 64K for flash and and address to align sector!!!!
int optimus_simg_to_media(char *simgPktHead, const u32 pktLen, u32 *unParsedDataLen, const u32 flashAddrInSec)
{
	const unsigned int notWrBackSz4LongChunk = _spPacketStates.notWrBackSz4LongChunk;
	unsigned int unParsedBufLen = pktLen - _spPacketStates.pktHeadLen;
	...
	if (notWrBackSz4LongChunk && !_spPacketStates.pktHeadLen/*0 if head*/) {
		...
		//const unsigned int leftLongChunk_dataLen = pLastLongChunk->total_sz;
		const unsigned int leftLongChunk_flashAddr = _spPacketStates.nextFlashAddr4LastLongChunk;
		unsigned int writeLen = 0;
		unsigned int thisWriteLen = 0;
	...
	for (; _spPacketStates.leftChunkNum && !_spPacketStates.notWrBackSz4LongChunk;
	     _spPacketStates.leftChunkNum--) {
		//chunk data for ext4, but maybe empty in sparse, that is why called sparse format
		const unsigned int chunkDataLen = pChunk->chunk_sz  *_spPacketStates.sparseBlkSz;
		unsigned int thisWriteLen = 0;
		...
		case CHUNK_TYPE_RAW: {
			unsigned int wantWrLen = chunkDataLen;

			if (CHUNK_HEAD_SIZE + chunkDataLen != pChunk->total_sz) {
				sperr("sparse: bad chunk size: head 0x%x + data 0x%x != total 0x%x\n",
						(unsigned int)CHUNK_HEAD_SIZE, chunkDataLen, pChunk->total_sz);
				return -__LINE__;
			}
			//left data not enough for this chunk (chunk header + chunk data)
			if (pChunk->total_sz > unParsedBufLen) {
				const unsigned int unParseChunkDataLen = unParsedBufLen - CHUNK_HEAD_SIZE;
		...
		case CHUNK_TYPE_FILL: {
			const unsigned int fillVal = *(unsigned *)(pChunk + 1);
			unsigned int LeftDataLen   = chunkDataLen;
			unsigned int temp_flashAddrStart = flashAddrStart;
			unsigned int *pFillValBuf = (unsigned *)OPTIMUS_SPARSE_IMG_FILL_VAL_BUF;
			const unsigned int FillBufSz = OPTIMUS_SPARSE_IMG_FILL_BUF_SZ;
			static unsigned int _filledBufValidLen;
			const unsigned int thisChunkFilledLen = min(chunkDataLen, FillBufSz);
			...
			if (!_NeedFillAsNotErasedYet)
				_NeedFillAsNotErasedYet = (is_optimus_storage_inited()>>16) == 0;// == 0 means 'disk_inital 0'
			...
			if (_NeedFillAsNotErasedYet) {
				...
				if (_filledBufValidLen < thisChunkFilledLen) {
					...
					unsigned int *temBuf = pFillValBuf + i;

					while (i++ < (thisChunkFilledLen>>2))
						*temBuf++ = fillVal;
		...
		pChunk                           =  (chunk_header_t *)((uintptr_t)pChunk + pChunk->total_sz);
	}
...
int optimus_sparse_back_info_probe(void)
{
	...
	ret = optimus_simg_probe((u8 *)(uintptr_t)_spPacketStates.chunkInfoBackAddr, FILE_HEAD_SIZE);
...
int optimus_sparse_get_chunk_data(u8 **head, u32 *headSz, u32 *dataSz, u64 *dataOffset)
{
	chunk_header_t *pChunk =
		(chunk_header_t *)(_spPacketStates.chunkInfoBackAddr + FILE_HEAD_SIZE) + _spPacketStates.leftChunkNum;//chunk header
	...
	*headSz = *dataOffset = *dataSz = 0;
	...
	if (!_spPacketStates.leftChunkNum) {//file header
		...
		*head   = (u8 *)(uintptr_t)_spPacketStates.chunkInfoBackAddr;
	} else {
		*head = (u8 *)pChunk;
	}

...
#define TOTAL_SET_NUM   ARRAY_SIZE(_iniSets)
...
static int parse_set_custom_para(const char *key, const char *strVal)
{
	...
	const unsigned int cfgVal  = strVal ? simple_strtoul(strVal, NULL, 0) : 0;
...
int check_cfg_burn_parts(const ConfigPara_t *burnPara)
{
	...
	const uint64_t bitsMap  = pBurnParts->bitsMap4BurnParts;
	int mediaPathHasCfg = burnPara->burnEx.bitsMap.mediaPath;
	int i = 0;

	DWN_MSG("cfg burn num %d\n", cfgBurnNum);
	for (i = 0; i < cfgBurnNum; i++) {
		uint64_t b = bitsMap & (1U << i);

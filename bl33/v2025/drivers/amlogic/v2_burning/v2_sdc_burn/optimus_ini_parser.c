...
static int line_2_words(char *line, char *argv[], const int maxWords)
{
	...
	for (; cur = *line, cur; ++line) {
		if (!is_space_char(cur))
			continue;
		//following do with space character

		*line = 0;
		for (cur = *++line; is_space_char(cur) && cur; cur = *++line) {

		} //ignore all space between words
...
int _optimus_parse_buf_2_lines(char *pTextBuf, const unsigned int textSz,
			       const char *lines[], unsigned int *totalLineNum,
...
int parse_ini_file_2_valid_lines(const char *filePath, char *iniBuf, const unsigned int bufSz, char *lines[])
{
	...
	if (_exfatok > 0) {
	    fileSz = (unsigned)do_exfat_get_fileSz(filePath);
	} else {
...
#ifdef CONFIG_FS_EXFAT
	if (_exfatok > 0) {
		readLen = do_exfat_fread(hFile, (u8*)iniBuf, fileSz);
	} else {
					 ...
					 int (*pParseCfgVal)(const char *setName, const char *keyName, const char *keyVal))
{
	...
	while (i < MaxUsefulSets)
		cacheSetNames[i++] = NULL;
	...
	for (lineIndex = 0; lineIndex < lineNum ; lineIndex++) {
		...
		if (!curLine)
			continue;//comment or space lines

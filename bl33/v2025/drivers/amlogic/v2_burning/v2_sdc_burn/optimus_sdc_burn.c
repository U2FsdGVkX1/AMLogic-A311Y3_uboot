...
static int optimus_burn_one_partition(const char *partName, HIMAGE hImg, __hdle hUiProgress, int NeedVerify)
{
	...
	const unsigned int ItemReadBufSz = OPTIMUS_LOCAL_UPGRADE_SLOT_SZ;//read this size from image item each time
	unsigned int sequenceNo = 0;
	const char *fileFmt = NULL;
	/*static */char _errInfo[512];
	unsigned int itemSizeNotAligned = 0;
	unsigned int itemSizePreload = 0;
	...
	if (itemSizeNotAligned >= imgItemSz)
		...
		itemSizePreload = itemSizeNotAligned;
	...
	for (; leftItemSz > 0; leftItemSz -= thisReadLen, sequenceNo++) {
		...
		if (hUiProgress)
			optimus_progress_ui_update_by_bytes(hUiProgress, thisReadLen) ;
	}
...
_finish:
	...
	if (rcode) {
		...
		optimus_progress_ui_printf("Failed at burn part[%s] before VERIFY\n", partName);
		return rcode;
	}
...
#ifdef AML_BOOTLOADER_TPL_COPY0
	DWN_MSG("burn tpl cpy0\n");
	ret = optimus_burn_one_partition(AML_BOOTLOADER_TPL_COPY0, hImg, NULL, 0);
	if (ret) {
		DWN_ERR("Fail in burn TPL copy0\n");
		return -__LINE__;
	}
#endif
...
static int optimus_sdc_burn_sheader_load(HIMAGE hImg, int cpy_index)
{
	...
	char *transferBuf  = (char *)(uintptr_t)partBaseOffset;
	int bufsz = 0x4<<20;
...
static int _optimus_sdc_burn_dtb_load(HIMAGE hImg, int is_gpt)
{
	...
	unsigned char *dtbTransferBuf     = (unsigned char *)(uintptr_t)partBaseOffset;
...
#if 1
	const unsigned int itemSzNotAligned = image_item_get_first_cluster_size(hImg, hImgItem);
	...
	if (!rc && !is_gpt) {
		...
		if (!check_valid_dts(dtbTransferBuf)) {
			env_set_hex("dtb_mem_addr", OPTIMUS_DTB_LOAD_ADDR);
			memmove((char *)OPTIMUS_DTB_LOAD_ADDR, (char *)env_get_hex("dt_addr",
				(long)dtbTransferBuf), itemSz);
		}
	} else if (is_gpt) {
...
static int sdc_burn_get_user_key_names(HIMAGE hImg, const char ***pKeysName, unsigned int *keysNum)
{
	int rc = 0;
	HIMAGEITEM hImgItem = NULL;
	...
	const unsigned int thisReadBufSz   = (OPTIMUS_SPARSE_IMG_FILL_BUF_SZ >> 1);
...
int sdc_burn_aml_keys(HIMAGE hImg, const int keyOverWrite, int licenseKey, int imgKey)
{
	...
	env_set("_keyinf", "sdc");
	if (env_get_ulong("usb_update", 0, 0))
		env_set("_keyinf", "usb");
	...
	for (index = 0; index < keysNum + num_img_keys; ++index) {
		...
		if (!strcmp(keyName, "secure_boot_objects")) {
			run_command("echo obj_file_burn ${_keyinf} license/secure_boot_objects", 0);
			rc = run_command("obj_file_burn ${_keyinf} license/secure_boot_objects", 0);
			if (!rc) {
				DWN_MSG("Okay burn efuse obj file\n");
				continue;
			}
			DWN_ERR("Fail in burn efuse obj\n");
			return __LINE__;
		}
		...
		unsigned int keySz = OPTIMUS_DOWNLOAD_SLOT_SZ;//buffer size
...
int optimus_burn_with_cfg_file(const char *cfgFile)
{
	...
	if (optimus_img_chk_soctype(hImg)) {
		...
		int need_chk = !CONFIG_IS_ENABLED(SUPPORT_SDC_KEYBURN) && erase_bootloader;
	...
	if (erase_bootloader && is_bootloader_old()) {
		if (usbDiskUpgrade) {//upgrade new bootloader
			...
			setenv("usbDiskUpgrade", "usb_burn $sdcburncfg");
			setenv("preboot", "printenv usbDiskUpgrade; run usbDiskUpgrade");
			run_command("saveenv", 0);
			run_command("echo Resetting boot-info;store erase misc 0 0", 0);
			run_command("echo reboot quiescent; reboot quiescent", 0);
		} else {//sdc_burn
	...
	if (optimus_work_mode_get() == OPTIMUS_WORK_MODE_SDC_PRODUCE) {

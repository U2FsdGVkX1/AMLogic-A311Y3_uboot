...
typedef struct _burnEx{
	...
	char        mediaPath[128];
	struct {
		unsigned int pkgPath    : 1;
		unsigned int mediaPath  : 1;
		unsigned int reserv     : 32 - 2;
	} bitsMap;
} BurnEx_t;
...
typedef struct _customPara{
	...
	int         eraseDdrPara;
	struct{
		unsigned int eraseBootloader    : 1;
		unsigned int eraseFlash         : 1;
		unsigned int rebootAfterBurn    : 1;
		unsigned int keyOverwrite       : 1;
		unsigned int eraseDdrPara       : 1;
		unsigned int resev              : 32 - 5;
	} bitsMap;
} CustomPara_t;
...
typedef struct _ConfigPara{
	...
	struct {
		unsigned int    burnParts : 1;
		unsigned int    custom    : 1;
		unsigned int    burnEx    : 1;
		unsigned int    display   : 1;
		unsigned int    reserv    : 32 - 4;
	} setsBitMap;
} ConfigPara_t;

//ini parser
int _optimus_parse_buf_2_lines(char *pTextBuf, const unsigned int textSz, const char *lines[],
			       unsigned int *totalLineNum,
			       const unsigned int MaxLines);//parse text context to lines delimitted by (\r)\n

int parse_ini_file_2_valid_lines(const char *filePath, char *iniBuf, const unsigned int bufSz, char *lines[]);
int parse_ini_buf_2_valid_lines(char *iniBuf, const unsigned int bufSz, char *lines[]);

int _optimus_abandon_ini_comment_lines(char *lines[], const unsigned int lineNum);
int optimus_ini_trans_lines_2_usr_params(const char *const lines[], const unsigned int lineNum,
...
int optimus_burn_partition_image(const char *partName, const char *imgItemPath,
				 const char *fileFmt, const char *verifyFile, const unsigned int itemSizeNotAligned);
...
int optimus_sdc_keysprovider_get_keyval(const void *pHdle, u8 *pBuf, unsigned int *keySz);
int optimus_sdc_keysprovider_update_license(const void *pHdle);

int optimus_keysburn_onekey(const char *keyName, u8 *keyVal, unsigned int keyValLen);

extern int _exfatok;

//for fat fs
long do_fat_fopen(const char *filename);
long do_fat_fread(int fd, __u8 *buffer, unsigned int long maxsize);
void do_fat_fclose(int fd);
s64 do_fat_get_fileSz(const char *imgItemPath);
int do_fat_fseek(int fd, const uintptr_t offset, int wherehence);
unsigned int do_fat_get_bytesperclust(int fd);
...
int do_fat_get_file_format(const char *imgFilePath, unsigned char *pbuf, const unsigned int bufSz);

...
int optimus_device_probe(const char *interface, const char *inPart)
{
	...
	if (optimus_exfat_register_device(interface, inPart) != 0)
#else
...
#ifdef CONFIG_FS_EXFAT
	if (_exfatok > 0) {
		hFile = do_exfat_fopen(imgItemPath);
	} else {
...
int opt_file_read(__hFileHdl hFile, u8 *buffer, const unsigned int size)
{
	unsigned int readSz;

#ifdef CONFIG_FS_EXFAT
	if (_exfatok > 0) {
		readSz = do_exfat_fread(hFile, buffer, size);
	} else {
		readSz = do_fat_fread(hFile, buffer, size);
	}
#else
	readSz = do_fat_fread(hFile, buffer, size);
#endif
...
s64 storage_get_partition_size_in_byte(const char *partName)
{
	...
	if (!strncmp(partName, "bootloader-boot", strlen("bootloader-boot"))) {
		size = store_boot_copy_size("bootloader");
		if (!size) {
			SDC_ERR("Fail get sz for boot0\n");
			return 0;
		}
		SDC_MSG("bootsz 0x%llx\n", size);
		return size;
	}
...
int optimus_burn_partition_image(const char *partName, const char *imgItemPath,
				 const char *fileFmt, const char *verifyFile,
				 const unsigned int itemSizeNotAligned)
{
	int rcode = 0;
	s64 imgItemSz       = 0;
	s64 leftItemSz      = 0;
	...
	const unsigned int ItemReadBufSz = OPTIMUS_DOWNLOAD_SLOT_SZ;//read this size from image item each time
	unsigned int sequenceNo = 0;
	__hdle hUiProgress = NULL;

#ifdef CONFIG_FS_EXFAT
	if (_exfatok > 0) {
		imgItemSz = leftItemSz = do_exfat_get_fileSz(imgItemPath);
	} else {
	...
	if ((imgItemSz>>20) > CONFIG_USB_UPDATE_UI_SZ) {
		if (video_res_prepare_for_upgrade(NULL)) {
		...
		rcode = optimus_progress_ui_set_smart_mode(hUiProgress, imgItemSz,
				UPGRADE_STEPS_AFTER_BURN_BOOTLOADER_OK - UPGRADE_STEPS_AFTER_DISK_INIT_OK);
		if (rcode) {
	...
	for (; leftItemSz > 0; leftItemSz -= thisReadLen, sequenceNo++) {
		...
		rcode = optimus_buf_manager_get_buf_for_bulk_transfer(&downTransBuf, thisReadLen,
								      sequenceNo, _errInfo);
		if (rcode) {
...
_finish:
	...
	if (hUiProgress)
		optimus_progress_ui_report_upgrade_stat(hUiProgress, !rcode);
	if (hUiProgress)
...
#ifdef CONFIG_FS_EXFAT
		if (_exfatok > 0) {
			rcode = do_exfat_get_file_format(imgItemPath, (u8 *)OPTIMUS_DOWNLOAD_TRANSFER_BUF_ADDR, (8 * 1024));
		} else {

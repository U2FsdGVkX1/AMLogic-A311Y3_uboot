...
static int v2_key_read(const char *keyName, u8 *keyVal, const unsigned int keyValLen,
		       char *errInfo, unsigned int *fmtLen)
{
	FB_ERR("burn key not supported as CONFIG_UNIFY_KEY_MANAGE undef!!");
	return -1;
}

static unsigned v2_key_burn(const char *keyName, const u8 *keyVal,
			    const unsigned int keyValLen, char *errInfo)
{
	FB_ERR("burn key not supported as CONFIG_UNIFY_KEY_MANAGE undef!!");
	return -1;
}
...
static struct {
	...
	int                 dataBufCap;
	char               *dataBuf;
	char               *verifyInfo;//verifyInfo saved while downloading, and used while verify
	int                 inited;
} _imgTransferInfo;

static UsbDownInf _usbDownInf = {0};
static UsbUpInf _usbUpInf = {0};
static s64 _rawImgFileOffset;
static s64 _rawImgVryLen;
...
int v3tool_buffman_img_verify_sha1sum(unsigned char *vrySum)
{
	...
	const char             *part        = (char *)imgCmnPara->partName;
...
static int _buffman_img_init_mmc(const char *partName, const int isDownload, const s64 imgSize, const s64 partStartOff)
{
	...
	const s64 capacity = mmc->capacity_user;
...
int v3tool_buffman_img_init(ImgTransPara *imgPara, const int isDownload)
{
	...
	case V3TOOL_MEDIA_TYPE_MEM: {//check mem address is able to access yourself if media is mem
		_imgTransferInfo.dataBuf = (char *)(uintptr_t)commonInf->partStartOff;
	...
	case V3TOOL_MEDIA_TYPE_MMC: {
		...
		if (ret)
			FB_EXIT("Fail in init mmc %s\n", partName);
	} break;
	default:
		FB_EXIT("Exception, err media type 0x%x\n", commonInf->mediaType);
	}
	if (imgPara->download.imgFmt == V3TOOL_PART_IMG_FMT_SPARSE  && isDownload) {
	...
	_rawImgFileOffset  = 0;
	_rawImgVryLen      = 0;
	optimus_progress_init((u32)(imgSize>>32), (u32)imgSize, 0, 100);
...
static int _v3tool_buffman_next_download_info_rawimg(ImgDownloadPara *imgPara)
{
	...
	case V3TOOL_MEDIA_TYPE_MEM: {
		...
		s64 leftLen        = cmnInf->imgSzTotal - _rawImgFileOffset;
...
int v3tool_buffman_next_download_info(UsbDownInf **downloadInf)
{
	...
	*downloadInf = NULL;
	if (!_imgTransferInfo.inited || !_imgTransferInfo.isDownload) {
		FBS_ERR(_ACK, "buffman %d, %d\n", _imgTransferInfo.inited,
			_imgTransferInfo.isDownload);
		return -__LINE__;
	}
	...
	case V3TOOL_PART_IMG_FMT_RAW: {
		...
		if (ret)
			FB_EXIT("FAil in get buf for raw img\n");
...
static int _v3tool_mmc_rw(const char *partName, const int write, const int thisTransferLen,
			  loff_t partOffset, void *dataBuf)
{
#if CONFIG_IS_ENABLED(MMC_WRITE)
	int dev = *partName - '0';
	...
	if (mmc_init(mmc)) {
		FBS_ERR(_ACK, "fail mmc_init[%s]", partName);
		return -__LINE__;
	}
	...
	if (write)
		n = blk_dwrite(mmc_get_blk_desc(mmc), blk, cnt, dataBuf);
	else
		n = blk_dread(mmc_get_blk_desc(mmc), blk, cnt, dataBuf);
...
int v3tool_buffman_data_complete_download(const UsbDownInf *downloadInf)
{
	...
	case V3TOOL_PART_IMG_FMT_RAW: {
		...
		case V3TOOL_MEDIA_TYPE_STORE: {
			if (!strcmp("bootloader", partName)) {
				...
				ret = store_logic_write(partName, partOffset,
							thisTransferLen, dataBuf);
			}
		} break;
		case V3TOOL_MEDIA_TYPE_MEM:
				break;
	...
	case V3TOOL_PART_IMG_FMT_SPARSE: {
		...
		ret = v3tool_simg2img_write_img(downloadInf,
						&_imgTransferInfo.imgTransPara.download);
	} break;
	...
	if (!ret)
		optimus_update_progress(thisTransferLen);
...
int v3tool_buffman_next_upload_info(UsbUpInf **uploadInfo)
{
	...
	*uploadInfo = NULL;
	...
	int dataSize = _usbUpInf.dataSize;
	loff_t partOffset = _rawImgFileOffset + cmnInf->partStartOff;

	switch (cmnInf->mediaType) {
	case V3TOOL_MEDIA_TYPE_MEM: {
		_usbUpInf.dataBuf = (char *)(uintptr_t)(cmnInf->partStartOff + _rawImgFileOffset);
	} break;
	case V3TOOL_MEDIA_TYPE_STORE: {
		if (!strcmp("bootloader", partName)  ||
		...
		if (!strcmp("bootloader", partName)) {
			...
			ret = store_logic_read(partName, _rawImgFileOffset + cmnInf->partStartOff,
					       dataSize, dataBuf);
		}
		if (ret) {

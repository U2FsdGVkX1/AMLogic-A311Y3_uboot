...
static int is_bootloader_discrete(bool *discreteMode)
{
...
int bootloader_write(u8 *dataBuf, unsigned off, unsigned binsz)
{
	...
	if (is_bootloader_discrete(&discreteMode))
		return -__LINE__;
...
static int _bootloader_read(u8 *pBuf, unsigned off, unsigned binsz, const char *bootName)
{
	...
	for (iCopy = 0; iCopy < bootCpyNum; ++iCopy) {
		...
		binsz = binsz > (0x400000 - 512) ? (0x400000 - 512) : binsz;//for coverity
		if (ret) {
			FB_ERR("Fail to read boot[%s] at copy[%d]\n", bootName, iCopy);
			continue;
		}
		if (iCopy && actVldCpyNum) {
...
int v3tool_storage_init(const int eraseFlash, unsigned int dtbImgSz, unsigned int gptImgSz)
{
	...
	if (initFlag > 1) {
		FB_MSG("defenv in usb burning mode\n");
		run_command("env default -a; env set upgrade_step 3; saveenv", 0);
		run_command("echo store rsv erase ddr-parameter;store rsv erase ddr-parameter", 0);
	}

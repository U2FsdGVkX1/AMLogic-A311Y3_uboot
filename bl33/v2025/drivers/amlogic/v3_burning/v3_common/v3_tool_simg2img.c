...
static struct
{
	unsigned int leftChunkNum;//chunks that not parsed yet
	unsigned int chunksBufLen;//>=OPTIMUS_DOWNLOAD_SPARSE_TRANSFER_SZ
	unsigned int sparseBlkSz;//block size of sparse format packet
	int             pktHeadLen;
	...
	s64         leftTransLen;
	s64         nextFlashAddr;
	s64         fileOffset; //fileOffset for usb download
	s64         imgTotalLen; //fileOffset for usb download
	chunk_header_t *chunkInfo;

} _spPacketStates;

static int _downloadState;
...
static int _simg_probe(const char *source, const u32 length)
{
	...
	if (header->magic != SPARSE_HEADER_MAGIC) {
		spmsg("sp bad magic, expect 0x%x but 0x%x\n", SPARSE_HEADER_MAGIC, header->magic);
		return 0;
	}

	if (!(SPARSE_HEADER_MAJOR_VER == header->major_version &&
	    FILE_HEAD_SIZE == header->file_hdr_sz && CHUNK_HEAD_SIZE == header->chunk_hdr_sz)) {
		sperr("want 0x [%x, %x, %x], but [%x, %x, %x]\n", SPARSE_HEADER_MAJOR_VER,
		      (unsigned int)FILE_HEAD_SIZE, (unsigned int)CHUNK_HEAD_SIZE,
...
{
	if (downPara->imgFmt != V3TOOL_PART_IMG_FMT_SPARSE) {
	...
	if (downPara->needVerify)
		...
		FB_MSG("don't save inf for verify!\n");
...
int v3tool_simg2img_get_img(UsbDownInf *downInf)
{
	...
	if (_spPacketStates.leftTransLen <= 0) {
		...
		FBS_ERR(fb_response_str, "Excep:NO leftTransLen but img not end, fileOffset %llx,"
			"leftChunkNum %d",
...
static int check_chunk_info(const chunk_header_t *chunk, unsigned int *pDataLen,
			    unsigned int *pFlashAddr, unsigned int *fillVal)
{
	...
	if (pDataLen)
		*pDataLen = chunkDataLen;
	if (pFlashAddr)
		*pFlashAddr = nextFlashAddr;
	return 0;
}

static int simg2img_fill_chunk_write(const char *partName, int fillLen,
				     const unsigned int *pFillVal, s64 flashAddr)
{
	const int BufSz = 256 * 1024;//from LarsonJ, 128K is max burst in emmc control read/write
	...
	unsigned int *tmpFill = (unsigned int *)fillBuf;

	//spmsg("Fill start 0x%llx by value 0x%x\n", flashAddr, fillVal);
	for (; i < szFill; i += 4)
		*tmpFill++ = fillVal;
	while (LeftDataLen > 0) {
		const unsigned int thisWriteLen = min(LeftDataLen, BufSz);
		int ret =  0;

		FB_DBG("fill off/sz/val 0x%08llx %x %x\n", flashAddr, thisWriteLen, fillVal);
		ret = store_logic_write(partName, flashAddr, thisWriteLen, fillBuf);
		if (ret) {
...
int v3tool_simg2img_write_img(const UsbDownInf *downInf, const ImgDownloadPara *downPara)
{
	...
	if (hasFileHeader) /*code part 1*/ {
		...
		_spPacketStates.leftChunkNum    = header->total_chunks - 1;//left chunkinfo
		...
		_spPacketStates.chunkInfoBackAddr = (chunk_header_t *)
			((sparse_header_t *)_spPacketStates.chunkInfoBackAddr + 1);
	...
	if (hasNextChunkInfo)
		dataSize   -= CHUNK_HEAD_SIZE;
	if (hasChunkBody) {
		const chunk_header_t *pChunk = _spPacketStates.chunkInfo;//this download info
		const char *part = (char *)downPara->commonInf.partName;
		s64 flashAddr = _spPacketStates.nextFlashAddr + downPara->commonInf.partStartOff;
		unsigned int flashWrLen = dataSize;
		const unsigned int chunkFlashSpace = pChunk->chunk_sz * _spPacketStates.sparseBlkSz;
		...
		if (chunkType == CHUNK_TYPE_RAW) {
			...
			const unsigned int *fillVal = (unsigned int *)dataBuf;
			...
			if (simg2img_fill_chunk_write((char *)part,
			    chunkFlashSpace, fillVal, flashAddr)) {
			...
			FB_DBG("donnot care: off/sz 0x%08llx flashWrLen",
			       _spPacketStates.nextFlashAddr, chunkFlashSpace);
			_spPacketStates.nextFlashAddr += chunkFlashSpace;
		} else {
...
int v3tool_simg2img_verify_img(sha1_context *ctx, const char *partName, s64 partBase)
{
	const sparse_header_t *header = (sparse_header_t *)V3_DOWNLOAD_VERIFY_INFO;
	const int nChunk = header->total_chunks;
	const unsigned int vry_buf_len  = (1 << 20);//small buffer to reply while verify long
	...
	static int _iChunk;
	static s64 _part_off;
	static unsigned long _lastReportTick;
	static unsigned int _dataChunkLeft;//initial value or chunk verified end

	if (_downloadState == SP_DOWNLOAD_OK) {//first time
		_iChunk = 0;
		_part_off = partBase;
		_dataChunkLeft = 0;
	...
	for (; _iChunk < nChunk; ++_iChunk, ++backInf) {
		const s64 sz_flash = backInf->chunk_sz * (s64)_spPacketStates.sparseBlkSz;

		if (!_dataChunkLeft)
			sha1_update(ctx, (u8 *)backInf, CHUNK_HEAD_SIZE);

		switch (backInf->chunk_type) {
		case CHUNK_TYPE_RAW: {
			chunkDataLen = sz_flash;
			if (CHUNK_HEAD_SIZE + chunkDataLen != backInf->total_sz) {
				sperr("sparse: bad chunk sz: head 0x%x + data 0x%x != total 0x%x\n",
				      (unsigned int)CHUNK_HEAD_SIZE,
				      chunkDataLen, backInf->total_sz);
				goto _verify_end;
			}
		} break;
		case CHUNK_TYPE_DONT_CARE: {
			if (backInf->total_sz != CHUNK_HEAD_SIZE) {
				sperr("bogus DONT CARE chunk\n");
				goto _verify_end;
			}
			chunkDataLen = 0;
		...
		if (!_dataChunkLeft)
			_dataChunkLeft = chunkDataLen;
		s64 flashAddr = _part_off + chunkDataLen - _dataChunkLeft;

		while (_dataChunkLeft) {
			const int thisLen = min(vry_buf_len, _dataChunkLeft);

...
static void usb_parameter_init(int time_out)
{
	...
	adnl_enum_timeout = 0;//clear every time
	adnl_identify_timeout = 0;//clear every time
...
int aml_v3_usbburning(unsigned int timeout, unsigned int pcToolWaitTime)
{
	int ret;
	...
	while (1) {
		...
		if (_disintr) {
			...
			aml_dnl_unregister();
			udelay(2 * 10 * 1000);
			aml_dnl_register("usb_dnl_amlogic");
			udelay(2 * 10 * 1000);
			_disintr = 0;
		}
#endif
		...
		if (adnl_identify_timeout && pcToolWaitTime) {
			const unsigned int waitIdentifyTime = get_timer(adnl_identify_timeout);
...
#if (defined CONFIG_USB_DEVICE_V2) && !defined(CONFIG_USB_GADGET_CRG) && !defined(CONFIG_USB_GADGET_DWC3)
		if ((get_timer(curTime_sof) > 0x200) && (_sofintr)) {
			_sofintr = 0;
			//dwc_otg_power_off_phy_fb();
		}
#endif// #if (defined CONFIG_USB_DEVICE_V2) && !defined(CONFIG_USB_GADGET_CRG)
		dm_usb_gadget_handle_interrupts(NULL);
	}
...
{
	unsigned int timeout = (argc >= 2) ? simple_strtoul(argv[1], NULL, 0) : 0;
	unsigned int pcToolWaitTime	= (argc >= 3) ? simple_strtoul(argv[2], NULL, 0) : 0;
...
U_BOOT_CMD(adnl,	3,	0,	do_aml_DNL,

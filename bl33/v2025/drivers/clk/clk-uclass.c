...
static struct clk *clk_set_parent_before(struct clk *clkp, struct clk *parent_clkp)
{
	struct clk *old_parent_clkp = clk_get_parent(clkp);

	/* enable old_parent & parent if CLK_OPS_PARENT_ENABLE is set */
	if (clkp->flags & CLK_OPS_PARENT_ENABLE) {
		clk_enable(old_parent_clkp);
		clk_enable(parent_clkp);
	}
	...
	return old_parent_clkp;
}

static void clk_set_parent_after(struct clk *clkp, struct clk *parent_clkp,
				 struct clk *old_parent_clkp)
{
	/*
	 ...
	 */
	if (clkp->enable_count)
		clk_disable(old_parent_clkp);

	/* re-balance ref counting if CLK_OPS_PARENT_ENABLE is set */
	if (clkp->flags & CLK_OPS_PARENT_ENABLE) {
		clk_disable(parent_clkp);
		clk_disable(old_parent_clkp);
	}
}
...
{
	const struct clk_ops *ops;
	struct clk *clkp, *parent_clkp, *old_parent_clkp;
	int ret;
	...
	if (CONFIG_IS_ENABLED(CLK_CCF)) {
		clk_get_priv(clk, &clkp);
		clk_get_priv(parent, &parent_clkp);
		old_parent_clkp = clk_set_parent_before(clkp, parent_clkp);
	}
	...
	if (CONFIG_IS_ENABLED(CLK_CCF)) {
		ret = device_reparent(clkp->dev, parent_clkp->dev);
		if (ret)
			goto fail;

		clk_set_parent_after(clkp, parent_clkp, old_parent_clkp);
		clk_clean_rate_cache(clkp);
	}
...
fail:
	if (CONFIG_IS_ENABLED(CLK_CCF))
		clk_set_parent_after(clkp, old_parent_clkp, parent_clkp);

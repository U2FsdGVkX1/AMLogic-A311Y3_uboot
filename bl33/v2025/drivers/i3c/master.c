...
#ifdef TEST_FREAMWK
	ret = 0;
	dev->info.pid = (((u64)0x1234 << 32) | (0x5678)) + i;
	dev->info.bcr = (u8)0x1234 + i;
	dev->info.dcr = (u8)0x5678 + i;
	i++;
#else
...
err_release_static_addr:
	log_debug("err_release_static_addr\n");
	if (dev->info.static_addr)
		i3c_bus_set_addr_slot_status(&master->bus,
					     dev->info.static_addr,
					     I3C_ADDR_SLOT_FREE);
...
i3c_master_register_new_i3c_devs(struct i3c_master_controller *master)
{
	...
	i3c_bus_for_each_i3cdev(&master->bus, desc) {
		...
		ret = device_register(&desc->dev->dev);//Is invalid in uboot
		if (ret)
			dev_err(master->dev,
...
static int i3c_master_bus_init(struct i3c_master_controller *master)
{
	...
	if (ret) {
		...
		dev_err(master->dev,"fail to do DAA\n");
#endif
		goto err_rstdaa;
	}
#endif
				  ...
				  u8 addr)
{
	...
	if (ret) {
		log_debug("fail attach i3c dev\n");
		goto err_free_dev;
	}
	ret = i3c_master_retrieve_dev_info(newdev);//get it pid
	if (ret)
		goto err_detach_dev;
	i3c_master_attach_boardinfo(newdev);// attach new dev info with dtb via pid.
	...
	if (ret) {
		log_debug("fail reattach i3c dev\n");
		goto err_detach_dev;
	}
	/*
				 ...
				 struct device_node *node)
{
	...
	if (!master || !node)
		return -EINVAL;
#ifdef CONFIG_AMLOGIC_MODIFY
	ret = ofnode_read_u32_array(np_to_ofnode(node), "reg", reg, ARRAY_SIZE(reg));
#else
 ...
 *
 * Free all IBI slots allocated by a generic IBI pool.
 */

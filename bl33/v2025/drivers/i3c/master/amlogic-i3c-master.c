...
#define SVC_I3C_CLK_SLOW_CTRL        0x224
#define SVC_AOI3C_CLK_SLOW_CTRL      0x300
#define SVC_I3C_CLK_SLOW_CTRL_EN     BIT(9)
#define SVC_I3C_CLK_SLOW_CTRL_DIV(x) FIELD_PREP(GENMASK(7, 0), (x))
#define SVC_I3C_CLK_SLOW_CTRL_DIV_EN BIT(8)

#define SVC_AOI3C_FCLK_CTRL        0x304
#define SVC_AOI3C_FCLK_CTRL_EN     BIT(9)
#define SVC_AOI3C_FCLK_CTRL_DIV(x) FIELD_PREP(GENMASK(7, 0), (x))
#define SVC_AOI3C_FCLK_CTRL_DIV_EN BIT(8)
...
#define I3C_TIMEOUT_MS				2
#define EE_SVC_I3C_FCLK_SRC_RATE			25000000
#define AO_SVC_I3C_FCLK_SRC_RATE			166000000
#define EE_SVC_I3C_PCLK_SRC_RATE			166000000
...
static void svc_i3c_master_clk_init(struct svc_i3c_master *master)
{
	/* enable slow clock */
	writel(SVC_I3C_CLK_SLOW_CTRL_EN,
	       master->regs + SVC_I3C_CLK_SLOW_CTRL);
	writel(SVC_I3C_CLK_SLOW_CTRL_EN,
	       master->regs + SVC_AOI3C_CLK_SLOW_CTRL);
	//temporary patch fclock 25Mhz
}
...
static void svc_i3c_master_reset(struct svc_i3c_master *master)
{
	...
	svc_i3c_master_clk_init(master);
}
					...
					u8 *addrs, unsigned int *count)
{
	...
	while (true) {
		...
		if (SVC_I3C_MSTATUS_RXPEND(reg)) {
			...
			continue;
			#endif
			printf("SVC_I3C_MSTATUS_IBIWON NOT SUPPORT \n");
			break;
...
static int svc_i3c_master_do_daa(struct i3c_master_controller *m)
{
	 ...
	 */
	for (i = 0; i < dev_nb; i++) {
		log_debug("%s add i3c_dev seq: %d\n", master->dev->name, i);
		ret = i3c_master_add_i3c_dev_locked(m, addrs[i]);//alloc i3cdesc get pid
#ifdef CONFIG_AMLOGIC_MODIFY
		if (ret)
			log_debug("fail add i3c_dev req:%d ret=%d\n", i, ret);
#endif
	}
			       ...
			       u8 *in, unsigned int len)
{
	...
	while (!completed) {
		...
		if (get_timer(start) > I3C_TIMEOUT_MS) {
			       ...
			       unsigned int *actual_len, bool continued, bool repeat_start)
{
	...
	while (retry--) {
		...
		ret = readl_poll_timeout(master->regs + SVC_I3C_MSTATUS, reg,
					SVC_I3C_MSTATUS_MCTRLDONE(reg), 1000);
		if (ret)
			goto emit_stop;
		...
		if (SVC_I3C_MSTATUS_IBIWON(reg)) {
			printf("SVC_I3C_MSTATUS_IBIWON NOT SUPPORT2\n");
			goto emit_stop;
			#ifdef DO_NEXT
			ret = svc_i3c_master_handle_ibi_won(master, reg);
			if (ret)
	...
	ret = readl_poll_timeout(master->regs + SVC_I3C_MSTATUS, reg,
				 SVC_I3C_MSTATUS_COMPLETE(reg), 1000);
	if (ret)
		goto emit_stop;
	...
	if (!continued) {
		...
		readl_poll_timeout(master->regs + SVC_I3C_MSTATUS, reg,
				   SVC_I3C_MSTATUS_STATE_IDLE(reg), 1000);
	}
...
svc_i3c_master_alloc_xfer(struct svc_i3c_master *master, unsigned int ncmds)
{
	...
	xfer = kzalloc(sizeof(struct svc_i3c_xfer), GFP_KERNEL);
	if (!xfer)
		return NULL;
	cmds = kzalloc(sizeof(struct svc_i3c_cmd) * ncmds, GFP_KERNEL);
	if (!cmds)
		goto alloc_cmds_err;
...
#ifdef DO_NEXT //uboot only support signal thread.
...
#ifdef DO_NEXT //uboot only support signal thread.
	if (!wait_for_completion_timeout(&xfer->comp, msecs_to_jiffies(I3C_TIMEOUT_MS)))
...
static int svc_i3c_master_probe(struct udevice *dev)
{
	struct svc_i3c_master *master = dev_get_priv(dev);
	int ret;

	master->drvdata = (struct svc_i3c_drvdata *)dev_get_driver_data(dev);
	if (!master->drvdata)
		return -EINVAL;

	master->regs = dev_read_addr_ptr(dev);

	ret = clk_get_by_name(dev, "pclk", &master->pclk);
	if (ret) {
		dev_err(dev, "Can't get pclock: %d\n", ret);
		return ret;
	}

	ret = clk_get_by_name(dev, "fclk", &master->fclk);
	if (ret) {
		dev_err(dev, "Can't get fclock: %d\n", ret);
		return ret;
	}
	ret = clk_get_by_name(dev, "div", &master->div);
	if (ret) {
		dev_err(dev, "Can't get div clock: %d\n", ret);
		return ret;
	}
	clk_set_rate(&master->div, EE_SVC_I3C_FCLK_SRC_RATE);
	clk_enable(&master->fclk);
	clk_enable(&master->pclk);
	master->pclk_rate = clk_get_rate(&master->pclk);
	master->fclk_rate = clk_get_rate(&master->fclk);
	if (master->pclk_rate == 0) {
		dev_err(dev, "Can't get pclk_rate rate\n");
		return -EINVAL;
	}
	if (master->fclk_rate != EE_SVC_I3C_FCLK_SRC_RATE) {
		dev_err(dev, "Can't get fclk_rate rate\n");
		return -EINVAL;
	}
	master->fclk_rate = master->fclk_rate *2;
#ifdef DO_NEXT
	...
	if (ret)
	{
		dev_err(dev, "fail register i3c host\n");
		goto err_disable_clks;
	}
	return 0;
...
static int svc_i3c_master_priv_read(struct udevice *dev, u32 dev_number,
				    u8 *buf, u32 buf_size)
{
	struct svc_i3c_master *master = dev_get_priv(dev);
#ifdef CONFIG_AMLOGIC_MODIFY
...
#else
	struct i3c_dev_desc *i3cdev = master->descs[dev_number];
#endif
	struct i3c_priv_xfer i3c_xfers;

#ifdef CONFIG_AMLOGIC_MODIFY
	if (dev_number > SVC_I3C_MAX_DEVS - 1)
		return EBADSLT;
	i3cdev = master->descs[dev_number];
#endif
	i3c_xfers.data.in = buf;
	i3c_xfers.len = buf_size;
	i3c_xfers.rnw = I3C_MSG_READ;

	return svc_i3c_master_priv_xfers(i3cdev, &i3c_xfers, 1);
}

static int svc_i3c_master_priv_write(struct udevice *dev, u32 dev_number,
				     u8 *buf, u32 buf_size)
{
	struct svc_i3c_master *master = dev_get_priv(dev);
#ifdef CONFIG_AMLOGIC_MODIFY
...
static const struct dm_i3c_ops svc_i3c_ops = {
	.i3c_xfers = svc_i3c_master_priv_xfers,
	.read = svc_i3c_master_priv_read,
	.write = svc_i3c_master_priv_write,
};
...
static const struct udevice_id svc_i3c_master_of_match_tbl[] = {
	{ .compatible = "nuvoton,npcm845-i3c", .data = (ulong)&npcm845_drvdata },
	{ .compatible = "silvaco,i3c-master-v1", .data = (ulong)&svc_default_drvdata },
	{ .compatible = "silvaco,i3c-master-meson", .data = (ulong)&npcm845_drvdata },
	{ /* sentinel */ },
};

U_BOOT_DRIVER(svc_i3c_master_driver) = {
	.name = "silvaco-i3c-master",
	.id = UCLASS_I3C,
	.of_match = svc_i3c_master_of_match_tbl,
	.probe = svc_i3c_master_probe,
	.priv_auto = sizeof(struct svc_i3c_master),
	.ops    = &svc_i3c_ops,
};

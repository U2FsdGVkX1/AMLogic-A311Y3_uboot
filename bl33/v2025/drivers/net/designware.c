	...
	if (clock_nb > 0) {
		...
		for (i = 0; i < clock_nb; i++) {
			...
			if (err && err != -ENOSYS && err != -ENOTSUPP) {
				...
				clk_release_all(&priv->clocks[i], 1);
				goto clk_err;
			}
			priv->clock_count++;
		}
...
static int eqos_start(struct udevice *dev)
{
	...
	if (soc_num == 5) {
		writel(0x30018, &eqos->mtl_regs_p->txq0_operation_mode);
		writel(0x30000, &eqos->mtl_regs_p->rxq0_operation_mode);
	}
...
int eqos_free_pkt(struct udevice *dev, uchar *packet, int length)
{
	...
	/* Invalidate RX packet DMA buffer */
...
static int eqos_phy_init(struct udevice *dev)
{
	...
	phydev = phy_find_by_mask(eqos->bus, mask);
	if (!phydev)
		return -1;

	phy_connect_dev(phydev, dev, eqos->interface);
...
U_BOOT_DRIVER(eth_qos) = {
	...
	.remove = eqos_remove,
	.ops = &eqos_ops,
	.priv_auto = sizeof(struct eqos_eth_dev),

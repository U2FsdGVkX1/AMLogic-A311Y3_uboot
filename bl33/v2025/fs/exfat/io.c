...
static char g_fname[FS_DIRENT_NAME_LEN];
...
#if defined(CONFIG_AMLOGIC_MODIFY)
	memset(g_fname, 0, FS_DIRENT_NAME_LEN);
	memset(&g_ef, 0, sizeof(struct exfat));
	memset(&g_node, 0, sizeof(struct exfat_node));
#endif
...
#if defined(CONFIG_AMLOGIC_MODIFY)
int exfat_fs_bytespercluster(void)
{
	struct exfat* ef;
	int bpc;
	...
	ef = &ctxt.ef;
	if (ef->dev) {
		bpc = ((int) ef->sb->sector_bits + (int) ef->sb->spc_bits);
		if (bpc <= 26)
			return 1 << bpc;
		return -1;
	}
			  ...
			  const char* path)
{
	...
	if (!strncmp(g_fname, path, strlen(path))) {
		ef = &g_ef;
		*node = &g_node;
		return nret;
	}
...
#if defined(CONFIG_AMLOGIC_MODIFY)
	nret= exfat_lookup(ef, node, real_path);
	if (nret == 0) {
		memcpy(&g_ef, ef, sizeof(struct exfat));
		memcpy(&g_node, *node, sizeof(struct exfat_node));
		strcpy(g_fname, path);
	}

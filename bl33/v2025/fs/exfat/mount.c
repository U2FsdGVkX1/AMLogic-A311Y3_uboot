...
static void exfat_preload_fat_table(struct exfat *ef)
{
	...
	if (ef->fat_table)
		return;
	fat_size = le32_to_cpu(ef->sb->fat_sector_count) * (1 << ef->sb->sector_bits);
	/* alloc more page to avoid overflow */
	mem = malloc(fat_size + 4096);
	if (!mem)
		return;
	off = le32_to_cpu(ef->sb->fat_sector_start) << ef->sb->sector_bits;
	ret = exfat_pread(ef->dev, mem, fat_size, off);
	if (ret < 0) {
		printf("preload fat table %d failed, ret:%d\n", fat_size, ret);
		free(mem);
		return;
	}
	printf("preload fat table, bytes %d, buffe:%p\n", fat_size, mem);
	ef->fat_table = mem;
}
#endif
...
#if defined(CONFIG_AMLOGIC_MODIFY)
	debug("exfat super dump:");
	debug("sector_start:%llx\n", le64_to_cpu(ef->sb->sector_start));
	debug("sector_count:%llx\n", le64_to_cpu(ef->sb->sector_count));
	debug("fat_sector_start:%x\n", le32_to_cpu(ef->sb->fat_sector_start));
	debug("fat_sector_count:%x\n", le32_to_cpu(ef->sb->fat_sector_count));
	debug("cluster_sector_start:%x\n", le32_to_cpu(ef->sb->cluster_sector_start));
	debug("cluster_count:%x\n", le32_to_cpu(ef->sb->cluster_count));
	debug("sector_bits:%x\n", ef->sb->sector_bits);
	debug("sectors_per_cluster:%x\n", ef->sb->spc_bits);
	debug("fat_count:%x\n", ef->sb->fat_count);
	exfat_preload_fat_table(ef);
#endif

...
#if !defined(CONFIG_ARM64) && defined(CONFIG_AMLOGIC_MODIFY)
...
#if !defined(CONFIG_ARM64) && defined(CONFIG_AMLOGIC_MODIFY)
	days = div_u64(unix_time, SEC_IN_DAY);
	year = div_u64((4 * days), (4 * 365 + 1));
#else
...
#if !defined(CONFIG_ARM64) && defined(CONFIG_AMLOGIC_MODIFY)
	rem = unix_time % SEC_IN_DAY;
	hour = div_u64_rem(unix_time, SEC_IN_HOUR, &rem);
	rem = unix_time % SEC_IN_HOUR;
	min = div_u64_rem(unix_time, SEC_IN_MIN, &rem);
	rem = unix_time % SEC_IN_MIN;
	twosec = div_u64_rem(unix_time, 2, &rem);
#else

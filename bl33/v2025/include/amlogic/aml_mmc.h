...
struct boot_info_emmc {
	struct {
		/*
		 * we provide a array of fixed clock providers in
		 * ROM, such as GP0, FCLK_DIV2 source clock.
		 */
		unsigned char src_index;
		/* clock divider */
		unsigned char clk_div;
		unsigned char co_phase;
		/* rx adjust index, to adjust the sampling point */
		unsigned char rx_adj;
...
#define BOOTINFO_INFO_VER	1
	...
	/* 8 Bytes for only nand/spinand,  eMMC skip */
	struct {
...
/* flash write rsv */
extern int store_ddr_save_fastboot_para(size_t size, void *buf);
/* mmc caps quirks */

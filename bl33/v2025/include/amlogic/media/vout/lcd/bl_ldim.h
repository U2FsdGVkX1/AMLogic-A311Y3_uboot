...
struct ldim_dev_driver_s {
	...
	int use_ctrl_cs;/*1:ctrl-cs, 0:gpio-cs*/
	int cs2clk_delay;
	int clk2cs_delay;
	int en_gpio;
	int en_gpio_on;
	...
	unsigned short adim;
	unsigned short pdim;
	unsigned char datamapping_en;
	unsigned char uboot_dim_en;
	...
	unsigned int *custom_params;
	unsigned int custom_params_cnt;
...
struct aml_ldim_driver_s {
	...
	int (*power_on)(struct aml_ldim_driver_s *ldim_drv);
	int (*power_off)(struct aml_ldim_driver_s *ldim_drv);
	int (*set_level)(struct aml_ldim_driver_s *ldim_drv, int level);

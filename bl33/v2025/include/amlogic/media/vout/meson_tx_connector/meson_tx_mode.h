...
#ifndef __MESON_TX_MODE_H_
#define __MESON_TX_MODE_H_
...
enum hdmi_vic {
	...
	/*Vesa mode which don't have vic, we specify value for them also*/
...
struct tx_timing {
	...
	/*
	 * bit0: DISPLAY_ID_TYPE_VII_ONLY_SUPPORT_DSC
	 * bit1: DISPLAY_ID_TYPE_VII_YUV420
	 */
	u32 flags;
...
/* always return valid struct tx_timing.
 * for invalid vic, return NULL.
 */
const struct tx_timing *meson_tx_mode_vic_to_timing(enum hdmi_vic vic);
/* always return valid struct tx_timing.
 * when failed, return timing with invalid vic(0).
 */
const struct tx_timing *meson_tx_mode_match_dtd_timing(struct dtd *t);
/* always return valid struct tx_timing.
 * when failed, return timing with invalid vic(0).
 */
const struct tx_timing *meson_tx_mode_match_vesa_timing(struct vesa_standard_timing *t);
/* match mode/timing table to get complete timing parameters */
const struct tx_timing *meson_tx_match_detail_timing(struct tx_timing *t);
/* check if timing t1 match with the given timing t2 */
bool meson_tx_mode_compare(struct tx_timing *t1, struct tx_timing *t2);

void meson_tx_mode_print_timing(const struct tx_timing *timing);
void meson_tx_mode_print_all_mode_table(void);

unsigned int meson_tx_mode_get_frame_duration(const struct tx_timing *t);
...
enum hdmi_picture_aspect meson_tx_mode_get_vic_aspect_ratio(enum hdmi_vic vic);
#endif

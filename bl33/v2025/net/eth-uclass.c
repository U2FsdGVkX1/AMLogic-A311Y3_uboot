...
static int eth_post_probe(struct udevice *dev)
{
	...
	/* Derive eth1addr from ethaddr for the second ethernet node
	 * in kernel DT, since U-Boot only probes one eth device.
	 * Only needed on special platform which has dual ethernet in
	 * kernel DT but single eth device in U-Boot.
	 */
#if defined(CONFIG_MESON_A9)
	if (dev_seq(dev) == 0 && !env_get("eth1addr")) {
		if (eth_env_get_enetaddr_by_index("eth", 0, env_enetaddr)) {
			if (env_enetaddr[5] == 0xff)
				env_enetaddr[5]--;
			else
				env_enetaddr[5]++;

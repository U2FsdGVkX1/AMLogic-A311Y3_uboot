...
pem_to_bin() {
    ...
    if ! command -v python2 >/dev/null 2>&1; then
        if [ "$n0inv128_enable" = True ] || [ "$n0inv128_enable" = true ]; then
            "${SCRIPT_PATH}/pem_extract_pubkey_py3.py" --n0inv128Enable "$input" "$output"
        else
            "${SCRIPT_PATH}/pem_extract_pubkey_py3.py" "$input" "$output"
        fi
    else

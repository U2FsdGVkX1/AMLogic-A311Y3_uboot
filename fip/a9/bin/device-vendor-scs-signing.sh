...
function mk_uboot() {
	...
	seek=0
	seek_sector=0
	dateStamp=A9-${part}-`date +%y%m%d%H%M%S`
...
if [ -f "${input_dir}/bl33.bin.org" ]; then
	${EXEC_BASEDIR}/add-device-keys-bl33.sh --add-device-keys --key-dir ${key_dir} --project ${part} --sig-scheme ${sig_scheme} \
		--input ${input_dir}/bl33.bin.org --output ${input_dir}/bl33.bin.temp --rootkey-index ${rootkey_index}
	...
	${EXEC_BASEDIR}/add-device-keys-bl33.sh --add-device-keys --key-dir ${key_dir} --project ${part} --sig-scheme ${sig_scheme} \
		--input ${input_dir}/bl33-payload.bin --output ${input_dir}/bl33-payload.bin --rootkey-index ${rootkey_index}
fi

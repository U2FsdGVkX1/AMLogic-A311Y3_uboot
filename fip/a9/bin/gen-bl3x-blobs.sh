...
if [ "$CS_SIGNING_SCHEME" == "rsa-mldsa" ]; then
  EXEC_ARGS="${EXEC_ARGS} --infile-signkey-bl${BLOB_NAME}-chipset-lvl3-pqc=${BASEDIR_RSAKEY_LVLX_DIR}/bl${BLOB_NAME}-level-3-mldsa-priv.pem"
fi
...
EXEC_ARGS="${EXEC_ARGS} --size-payload-bl30=${BL30_BIN_SIZE}"

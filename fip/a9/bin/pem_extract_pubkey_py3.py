...
def compute_mod_parameters(modulus, exp):
  ...
  # -1 / N127~0 mod 2^128
  B128 = (1 << 128)
  w128 = w[3] * (1 << 96) + w[2] * (1 << 64) + w[1] * (1 << 32) + w[0]
  ...
  return {'exp':int(bytes.hex(exp), 16), 'len':wordCount, 'mod':w, 'rr':rr_words, 'n0inv':n0inv, 'n0inv128':n0inv128}
...
def dump_blob(n0inv128Enable, params):
  ...
  rsvd_padding = b'\x00\x00\x00\x00' * 2
  blob = len_bin + e_bin + rsvd_padding + mod_bin + rr_bin

  if n0inv128Enable:
    n0inv128_bin = params['n0inv128'].tobytes()
    blob += n0inv128_bin
  return blob
...
def main():
  ...
  parser = argparse.ArgumentParser(
      description='Converts RSA PEM key to public mincrypt key format',
      formatter_class=argparse.ArgumentDefaultsHelpFormatter)
  parser.add_argument("--headerMode", default=False, action="store_true",
                    help="Print the public key parameters for a C header file.")
  parser.add_argument("--n0inv128Enable", default=False, action="store_true",
                    help="Also include the n0inv128 bytes in the output file.")

  parser.add_argument(
      'input_key_pem_file',
      help='Path to the RSA public or private key file in PEM format.')
  parser.add_argument(
      'out_pub_bin_file',
      help='Path to the output public key bin file. '
           'Output format is public mincrypt key format')

  args = parser.parse_args()

  pub_key_bytes = extract_pubkey(args.input_key_pem_file,
                                 headerMode=args.headerMode,
                                 n0inv128Enable=args.n0inv128Enable)
  if pub_key_bytes:
    with open(args.out_pub_bin_file, 'wb') as f:

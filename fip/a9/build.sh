...
function init_vari() {
	...
	# script can use chipset variant to override config variant
...
function mk_bl2ex() {
	...
	./${FIP_FOLDER}${CUR_SOC}/binary-tool/acpu-imagetool create-boot-blobs \
			--chipset-authen-algorithm=rsa,mldsa \
			--device-authen-algorithm=rsa,mldsa \
...
function mk_devfip() {
	...
	./${FIP_FOLDER}${CUR_SOC}/binary-tool/acpu-imagetool create-device-fip \
			...
			--header-layout=mini \
			--size-payload-bl30=${BL30_BIN_SIZE}	\
			--chipset-authen-algorithm=rsa,mldsa \
			--device-authen-algorithm=rsa,mldsa
...
function mk_uboot() {
	...
	seek=0
	seek_sector=0
	dateStamp=A9-${CHIPSET_NAME}-`date +%y%m%d%H%M%S`
...
function blx_dv_sig_scheme() {
	# Detects DV sig scheme from double-signed scheme blx file
	# and is not yet DV signed.
	# $1: filename
	# returns rsa, rsa-mldsa, or mldsa
	sig_scheme=""

	# Partial hash common header magic
	readonly rsa_magic="\x40\x4D\x4C\x20\x95\xC8\xAF\x9A"
	readonly mldsa_magic="\x40\x4D\x4C\x20\x59\x69\x60\x8D"
	area=$((5728 + 32 + 4096 ))

	offset=$(tail -c $area "$1" |\
			LC_ALL=C awk -v RS="$rsa_magic" '{print length($0); exit}')
	if [ $offset -ne $area ]; then
		sig_scheme="rsa"
	fi
	offset=$(tail -c $area "$1" |\
			LC_ALL=C awk -v RS="$mldsa_magic" '{print length($0); exit}')
	if [ $offset -ne $area ]; then
		if [ -n "$sig_scheme" ]; then
			sig_scheme+="-"
		fi
		sig_scheme+="mldsa"
	fi
...
function convert_and_rename_blx() {
	# Remove dv/cs sig scheme extension because some places don't use BLX_BIN_NAME
	for loop in "${!BLX_NAME[@]}"; do
		local f="${BUILD_PATH}/${BLX_BIN_NAME[$loop]}"
		local cleaned="${f%.rsa-mldsa.${CS_SIGNING_SCHEME}}"

		if [ ! -f "$f" ]; then
			# BLX_BIN_NAME doesn't exist, try with target scheme suffix"
			# in case user provided binaries
			f="${f%.rsa-mldsa.${CS_SIGNING_SCHEME}}.${DV_SIGNING_SCHEME}.${CS_SIGNING_SCHEME}"
		fi
		if [ ! -f "$f" ]; then
			# Check target filename, in case passed in as ./mk --blxx argument
			f="$cleaned"
			if [ ! -f "$f" ]; then
				continue
			fi
		fi
		if [ -z "${f##*.${DV_SIGNING_SCHEME}.${CS_SIGNING_SCHEME}}" ]; then
			# Suffix already matches target DV+CS signing scheme. Rename it only
			cleaned="${f%.${DV_SIGNING_SCHEME}.${CS_SIGNING_SCHEME}}"
		fi

		file_dv_sig_scheme=$(blx_dv_sig_scheme "$f")

		if [ ! -z "$file_dv_sig_scheme" ] &&
				[ "$file_dv_sig_scheme" != "$DV_SIGNING_SCHEME" ]; then
			# Convert layout from rsa-mldsa to rsa for DV
			local cmd
			local in_arg
			local out_arg
			local cs_algo_arg
			case "${BLX_NAME[$loop]}" in
				bl2)
					cmd=("create-boot-blobs"
						"--scs-family=a9")
					in_arg=--infile-blob-bb1st-ref
					out_arg=--outfile-bb1st
					;;
				bl2e|bl2x|bl30|bl31|bl32|bl40)
					case "${BLX_NAME[$loop]}" in
						bl2e|bl2x)
							cmd=("create-boot-blobs"
								"--scs-family=a9")
							;;
						*)
							cmd=("create-device-fip"
								"--header-layout=mini"
								"--size-payload-bl30=${BL30_BIN_SIZE}")
							;;
					esac
					in_arg=--infile-blob-${BLX_NAME[$loop]}
					out_arg=--outfile-blob-${BLX_NAME[$loop]}
					;;
				*)
					echo "Error: Unhandled BLX_NAME"
					exit 1 ;;
			esac
			case "${CS_SIGNING_SCHEME}" in
				rsa) cs_algo_arg="rsa,none" ;;
				rsa-mldsa) cs_algo_arg="rsa,mldsa" ;;
				*) echo "Error: CS_SIGNING_SCHEME $CS_SIGNING_SCHEME"
					exit 1 ;;
			esac
			./${FIP_FOLDER}${CUR_SOC}/binary-tool/acpu-imagetool "${cmd[@]}" \
					--chipset-authen-algorithm=$cs_algo_arg \
					--device-authen-algorithm=rsa,none \
					--blobin-device-authen-algorithm=rsa,mldsa \
					${in_arg}="$f" \
					${out_arg}="$cleaned"
			ret_arg=$?
			if [ $ret_arg -ne 0 ]; then
				echo Error: acpu-imagetool returned error $ret_arg
				exit 1
			fi
		elif [ "$f" != "$cleaned" ]; then
			mv "$f" "$cleaned" -f -v
...
function process_blx() {
	...
	for loop in ${!BLX_NAME[@]}; do
		if [ "NULL" != "${BLX_RAWBIN_NAME[$loop]}" ] && \
			...
			# blx signed output filename is without rsa-mldsa.rsa-mldsa suffix because it does
			# not need to be converted.
			local output="${BUILD_PATH}/${BLX_BIN_NAME[$loop]%.rsa-mldsa.${CS_SIGNING_SCHEME}}"
			if [ -n "${CONFIG_FORMER_SIGN}" ]; then
					...
					./${FIP_FOLDER}${CUR_SOC}/bin/sign-blx.sh --blxname ${BLX_NAME[$loop]} --input ${BUILD_PATH}/${BLX_RAWBIN_NAME[$loop]} \
						--output $output --chipset_name ${CHIPSET_NAME} --chipset_variant ${CHIPSET_VARIANT} \
					...
					if [ -n "${CONFIG_JENKINS_SIGN}" ]; then
						...
						/usr/bin/python3 ./sign.py --type ${BLX_NAME[$loop]} --in ${BUILD_PATH}/${BLX_RAWBIN_NAME[$loop]} \
							--out $output --chip ${CHIPSET_NAME}  --chipVariant ${CHIPSET_VARIANT} \
						...
						/usr/bin/python3 ./${FIP_FOLDER}/jenkins_sign.py --type ${BLX_NAME[$loop]} --in ${BUILD_PATH}/${BLX_RAWBIN_NAME[$loop]} \
							--out $output --chip ${CHIPSET_NAME} --chipVariant ${CHIPSET_VARIANT} --keyType ${AMLOGIC_KEY_TYPE} \
	...
	if [ ! -f ${BUILD_PATH}/blob-bl40.bin.signed.${DV_SIGNING_SCHEME}.${CS_SIGNING_SCHEME} ] ||
			[ ! -f ${BUILD_PATH}/blob-bl40.bin.signed.rsa-mldsa.${CS_SIGNING_SCHEME} ]; then
		echo "Warning: local bl40"
		cp bl40/bin/${CUR_SOC}/${BLX_BIN_SUB_CHIP}/blob-bl40.bin.signed.rsa-mldsa.${CS_SIGNING_SCHEME} ${BUILD_PATH}
	fi

	convert_and_rename_blx
	...
	template="$CHIPSET_TEMPLATES_PATH/$CONFIG_SOC_NAME/$CHIPSET_NAME/bb1st.bin.${DV_SIGNING_SCHEME}.${CS_SIGNING_SCHEME}"
	if [ -n "$CHIPSET_VARIANT_SUFFIX" ]; then
		template="${BUILD_PATH}/bb1st.sto${CHIPSET_VARIANT_SUFFIX}.bin.signed"
	fi
	./${FIP_FOLDER}${CUR_SOC}/bin/add-dvinit-params.sh "${template}" ${BUILD_PATH}/bb1st.sto${CHIPSET_VARIANT_SUFFIX}.bin.signed ${BUILD_PATH}/blob-bl2e.sto${CHIPSET_VARIANT_SUFFIX}.bin.signed ${BUILD_PATH}/dvinit-params.bin ${BUILD_PATH}/bb1st.sto${CHIPSET_VARIANT_SUFFIX}.bin.signed ${CUR_SOC}
	if [ -n "$CHIPSET_VARIANT_SUFFIX" ]; then
		template="${BUILD_PATH}/bb1st.usb${CHIPSET_VARIANT_SUFFIX}.bin.signed"
	fi
	./${FIP_FOLDER}${CUR_SOC}/bin/add-dvinit-params.sh "${template}" ${BUILD_PATH}/bb1st.usb${CHIPSET_VARIANT_SUFFIX}.bin.signed ${BUILD_PATH}/blob-bl2e.usb${CHIPSET_VARIANT_SUFFIX}.bin.signed ${BUILD_PATH}/dvinit-params.bin ${BUILD_PATH}/bb1st.usb${CHIPSET_VARIANT_SUFFIX}.bin.signed ${CUR_SOC}
...
#		dd if=${BUILD_PATH}/bl33.bin.org.lz4 of=${BUILD_PATH}/bl33.bin bs=1 skip=1824 >& /dev/null
	...
	if [ ! -f ${BUILD_PATH}/device-fip-header.bin${template_ext} ]; then
		...
		cp ${CHIPSET_TEMPLATES_PATH}/${CUR_SOC}/${BLX_BIN_SUB_CHIP}/device-fip-header.bin${template_ext} ${BUILD_PATH}/device-fip-header.bin
...
function build_signed() {
	...
	./${FIP_FOLDER}tools/gen_spl_tpl.sh ${postfix} ${BL30_BIN_SIZE} ${BLX_BIN_SIZE[5]} ${BLX_BIN_SIZE[6]} ${BLX_BIN_SIZE[7]} ${BL33_BIN_SIZE}
	...
	if [ "y" == "${CONFIG_AML_SIGNED_UBOOT}" ]; then
		...
		if [ "y" == "${CONFIG_DEVICE_NO_CRYPTO_AES}" ]; then
			DEVICE_NO_CRYPTO_AES="true"
		else
			DEVICE_NO_CRYPTO_AES="false"
		fi
		...
		./${FIP_FOLDER}${CUR_SOC}/bin/device-vendor-scs-signing.sh --key-dir ${UBOOT_SRC_FOLDER}/${BOARD_DIR}/device-keys/ --project ${CHIPSET_NAME} \
			...
			--arb-config ${UBOOT_SRC_FOLDER}/${BOARD_DIR}/fw_arb.cfg --disable-device-aes-crypto ${DEVICE_NO_CRYPTO_AES} --out-dir ${BUILD_PATH}

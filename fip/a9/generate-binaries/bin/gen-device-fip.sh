...
if [ "$DV_SIGNING_SCHEME" == "rsa-mldsa" ] || [ "$DV_SIGNING_SCHEME" == "mldsa" ]; then
  EXEC_ARGS="${EXEC_ARGS} --infile-signkey-bl30-device-lvl3-pqc=${BASEDIR_FIP_SIGKEY_ROOT}/key/bl30-level-3-mldsa-priv.pem"
  EXEC_ARGS="${EXEC_ARGS} --infile-signkey-bl33-device-lvl3-pqc=${BASEDIR_FIP_SIGKEY_ROOT}/key/bl33-level-3-mldsa-priv.pem"
  EXEC_ARGS="${EXEC_ARGS} --infile-signkey-bl40-device-lvl3-pqc=${BASEDIR_FIP_SIGKEY_ROOT}/key/bl40-level-3-mldsa-priv.pem"
  EXEC_ARGS="${EXEC_ARGS} --infile-signkey-bl31-device-lvl3-pqc=${BASEDIR_FIP_SIGKEY_ROOT}/key/bl31-level-3-mldsa-priv.pem"
  EXEC_ARGS="${EXEC_ARGS} --infile-signkey-bl32-device-lvl3-pqc=${BASEDIR_FIP_SIGKEY_ROOT}/key/bl32-level-3-mldsa-priv.pem"
fi

if [ "$CS_SIGNING_SCHEME" == "rsa" ]; then
  EXEC_ARGS="${EXEC_ARGS} --chipset-authen-algorithm=rsa,none"
elif [ "$CS_SIGNING_SCHEME" == "rsa-mldsa" ]; then
  EXEC_ARGS="${EXEC_ARGS} --chipset-authen-algorithm=rsa,mldsa"
elif [ "$CS_SIGNING_SCHEME" == "mldsa" ]; then
  EXEC_ARGS="${EXEC_ARGS} --chipset-authen-algorithm=none,mldsa"
fi
if [ "$DV_SIGNING_SCHEME" == "rsa" ]; then
  EXEC_ARGS="${EXEC_ARGS} --device-authen-algorithm=rsa,none"
elif [ "$DV_SIGNING_SCHEME" == "rsa-mldsa" ]; then
  EXEC_ARGS="${EXEC_ARGS} --device-authen-algorithm=rsa,mldsa"
elif [ "$DV_SIGNING_SCHEME" == "mldsa" ]; then
  EXEC_ARGS="${EXEC_ARGS} --device-authen-algorithm=none,mldsa"
fi
...
#v1.3.0-test12 support 3 sizes of bl30: 94208, 90112, default 65536
EXEC_ARGS="${EXEC_ARGS} --size-payload-bl30=102400"

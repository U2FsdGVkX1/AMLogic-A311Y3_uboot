...
ACPU_IMAGETOOL=
OPENSSL_PQC=${OPENSSL_PQC:-${EXEC_BASEDIR}/../../bin/openssl-mldsa}
...
declare -A ml_dsa_openssl_name_list
ml_dsa_openssl_name_list["2"]="ml-dsa-44"
ml_dsa_openssl_name_list["3"]="ml-dsa-65"
ml_dsa_openssl_name_list["5"]="ml-dsa-87"

declare -A ml_dsa_version_list
ml_dsa_version_list["draft1"]="draft1"
ml_dsa_version_list["final"]="final"
...
ml_dsa_sig() {
    ...
    do
        if [ $ops == "verify" ]; then
            trace "${OPENSSL_PQC} dgst -sha3-512 -verify $path/$f-pub.pem -keyform pem -signature $path/test-payload-${payload}-$f-pub.sig $test_vector_file"
            ${OPENSSL_PQC} dgst -sha3-512 -verify $path/$f-pub.pem -keyform pem -signature $path/test-payload-${payload}-$f-pub.sig $test_vector_file
        else
            trace "${OPENSSL_PQC} dgst -sha3-512 -sign $path/$f-priv.pem -keyform pem -out $path/test-payload-${payload}-$f-pub.sig $test_vector_file"
            ${OPENSSL_PQC} dgst -sha3-512 -sign $path/$f-priv.pem -keyform pem -out $path/test-payload-${payload}-$f-pub.sig $test_vector_file
        fi
    done
    fi
}
...
ml_dsa_gen() {
    ...
    do
        ...
        if [ $version == "final" ]; then
            echo "Generate Final Spec ML-DSA key, Algorithm ${ml_dsa_openssl_name_list[$size]}"
            ${OPENSSL_PQC} genpkey -algorithm ${ml_dsa_openssl_name_list[$size]} -outform PEM -out $kpriv
        else
            echo "Generate Draft Spec ML-DSA key, Algorithm dilithium${size}"
            ${OPENSSL_PQC} genpkey -algorithm dilithium${size} -outform PEM -out $kpriv
        fi
        echo $kpriv
        echo $kpub
        ${OPENSSL_PQC} pkey -in $kpriv -outform PEM -pubout -out $kpub
    done
    fi
}
...
if [ $stage == "root" ]; then
    ...
    ml_dsa_gen "Root" "$root_key_path/key" "${PREFIX}root${ml_dsa_algo_name}-0 ${PREFIX}root${ml_dsa_algo_name}-1 ${PREFIX}root${ml_dsa_algo_name}-2 ${PREFIX}root${ml_dsa_algo_name}-3" $ml_dsa_level $ml_dsa_version
...
if [ $stage == "boot-blobs" ]; then
    ...
    do
        ...
        if [ $link_gen_file -eq 1 ]; then
            ...
            ml_dsa_gen $i "${boot_blobs_key_path}/key" "${PREFIX}level-1-2-${ml_dsa_algo_name}" $ml_dsa_level $ml_dsa_version
            if [ $is_rsa -eq 1 ]; then
            ...
            ml_dsa_gen $i "${boot_blobs_key_path}/key" "${PREFIX}level-1-${ml_dsa_algo_name} ${PREFIX}level-2-${ml_dsa_algo_name}" $ml_dsa_level $ml_dsa_version
...
if [ $stage == "fip" ]; then
    ...
    do
        ...
        exit 1
            ...
            if [ $is_ml_dsa -eq 1 ]; then
                ml_dsa_gen $i "${fip_key_path}/key" "${PREFIX}bl31-level-3-${ml_dsa_algo_name} ${PREFIX}bl32-level-3-${ml_dsa_algo_name} ${PREFIX}bl40-level-3-${ml_dsa_algo_name}" $ml_dsa_level $ml_dsa_version
            fi
            ...
            if [ "$prefix_name" == "dv" ] || [ "$prefix_name" == "none" ]; then
                ...
                if [ $is_ml_dsa -eq 1 ]; then
                    ml_dsa_gen $i "${fip_key_path}/key" "${PREFIX}bl30-level-3-${ml_dsa_algo_name} ${PREFIX}bl33-level-3-${ml_dsa_algo_name} ${PREFIX}krnl-level-3-${ml_dsa_algo_name}" $ml_dsa_level $ml_dsa_version
                fi

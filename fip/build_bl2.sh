...
function check_branch() {
	...
	if [ "$1" == "s7d" ]; then
		dest_branch="projects/amlogic-dev"
	elif [ "$1" == "t6w" ] || [ "$1" == "t6x" ]; then
		dest_branch="projects/project_s5_family"
	fi
...
function build_bl2() {
	...
	# $1: src_folder, $2: bin_folder, $3: soc $4: times

	cd $1
	set -e
	if [ "$ADVANCED_BOOTLOADER" == "1" ]; then
		if [ "$CONFIG_SKIP_SWITCHBRANCH" != "1" ]; then
			echo "check_branch $3"
		fi
		if [ "$4" == "0" ]; then

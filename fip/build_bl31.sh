...
declare -a BL31_VER2_7_SOC_LIST=("t3x" "a4" "s1a" "s7" "s7d" "s6" "t6d")
declare -a BL31_VER2_12_SOC_LIST=("t6w" "t6x" "x1" "a9" "c5")
declare -a NONE_BL31_SOC_LIST=("c4")
declare -a BL3_TEE_SOC_LIST=("x1" "c4" "c5")
declare BL31_V2_12_SRC_FOLDER="bl31/bl31_2.12/src"
...
declare BL31_V2_12_BIN_FOLDER="bl31/bl31_2.12/bin"
...
function check_bl31_ver() {
	...
	for soc_list in ${!BL31_VER2_12_SOC_LIST[@]}; do
	if [ "$1" == "${BL31_VER2_12_SOC_LIST[${soc_list}]}" ]; then
		ver=3
	fi
	done

	# for BL3-TEE
	for soc_list in ${!BL3_TEE_SOC_LIST[@]}; do
	if [ "$1" == "${BL3_TEE_SOC_LIST[${soc_list}]}" ]; then
		ver=10000
	fi
	done
	echo ${ver}
...
function switch_bl31() {
	...
	echo "check_bl31_ver ver=${version}"

	if [ ${version} -eq 10000 ]; then
		return
	fi

	if [ ${version} == 3 ]; then
		echo "check bl31 ver: use v2.12"
		BLX_SRC_FOLDER[$bl31_index]=${BL31_V2_12_SRC_FOLDER}
		BLX_BIN_FOLDER[$bl31_index]=${BL31_V2_12_BIN_FOLDER}
	...
	echo BLX_BIN_FOLDER3: ${BLX_BIN_FOLDER[0]}
}

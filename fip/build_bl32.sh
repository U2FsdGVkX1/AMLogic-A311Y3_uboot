...
function build_bl32() {
	...
	elif [ $CONFIG_BL32_FASTBOOT == "y" ]; then
		if [ $CONFIG_BL32_128M == "y" ]; then
			eval 'CFG_DRAM_128M=y CFG_C4_FASTBOOT=y ./build.sh $3 ${CONFIG_CAS}'
		else
			eval 'CFG_C4_FASTBOOT=y ./build.sh $3 ${CONFIG_CAS}'
		fi
	else

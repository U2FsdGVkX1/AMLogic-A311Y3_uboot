...
function build_uboot() {
	echo "Build uboot...Please Wait...$1...$2...$3...$4...$5...$6...$7...$8"
	...
	if [ "${CONFIG_MDUMP_COMPRESS}" = "1" ]; then
		...
		make -j SYSTEMMODE=$1 AVBMODE=$2 BOOTCTRLMODE=$3 FASTBOOTMODE=$4 AVB2RECOVERY=$5 TESTKEY=$6 GPTMODE=$7 JUMPBL33=$8 CHIPMODE=${CONFIG_CHIP_NOCS} \
			CONFIG_MDUMP_COMPRESS=${CONFIG_MDUMP_COMPRESS} \
			CONFIG_TEE_TYPE=${CONFIG_TEE_TYPE} # &> /dev/null
	else
		echo "### BL33 CONFIG_MDUMP_COMPRESS = 0 ### CONFIG_TEE_TYPE ${CONFIG_TEE_TYPE}"
		make -j SYSTEMMODE=$1 AVBMODE=$2 BOOTCTRLMODE=$3 FASTBOOTMODE=$4 AVB2RECOVERY=$5 TESTKEY=$6 GPTMODE=$7 JUMPBL33=$8 CHIPMODE=${CONFIG_CHIP_NOCS} \
			CONFIG_TEE_TYPE=${CONFIG_TEE_TYPE} # &> /dev/null
	fi
	set +e
	...
	skiped=("c1" "c2" "c3" "g12a" "g12b" "sm1" "t5w")
	if [[ "${skiped[@]}"  =~ "${SOC_GROUP}" ]]; then
		...
		if [ -f "./bl33z/Makefile" ]; then
			set -e
			if [ "${CONFIG_AML_AARCH32:-0}" = "1" ]; then
				BL33Z_ARCH=aarch32
				echo "AArch32 build. (CONFIG_AML_AARCH32 is defined)"
			else
				BL33Z_ARCH=aarch64
				echo "AArch64 build"
			fi
			make -C bl33z/ PLAT="${SOCNAME}" AARCH="${BL33Z_ARCH}" distclean
			make -C bl33z/ PLAT="${SOCNAME}" AARCH="${BL33Z_ARCH}"
			set +e
			if [ -f "./bl33z/build/bl33z.bin" -a -f "./build/u-boot.bin" ]; then
				...
				END_ALIGN=4096
				BL33Z_LOAD=$(( ( (END_LENS + END_ALIGN - 1) / END_ALIGN ) * END_ALIGN ))

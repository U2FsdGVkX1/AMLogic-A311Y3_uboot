...
types = ["ta", "vmxta", "irdetota", "bl32", "bl31", "bl3tee", "bl2", "bl2e", "bl2x", "bl40", "aucpufw", "vdecfw"]
...
def get_args():
    ...
    parser.add_argument("--in", dest="inputFilePath", default="null", help="input file")
    parser.add_argument(
        "--chipAcsFile", dest="chipAcsFilePath", default="null", help="chip acs file"
    )
    parser.add_argument(
        "--bl31", dest="bl31FilePath", default="null", help="bl31 file"
    )
    parser.add_argument(
        "--bl32", dest="bl32FilePath", default="null", help="bl32 file"
    )
    parser.add_argument(
        "--bl3_tee", dest="bl3TeeFilePath", default="null", help="bl3 tee file"
    )
    parser.add_argument(
...
def getJobRootUrl(type):
    if type == "ta":
        ...
        return urljoin(serverRootUrl, "CAS/job/Irdeto/job/Sign_Irdeto_TA_for_s905c1/")
    elif type == "bl31":
        ...
        return urljoin(serverRootUrl, "Signing/job/Sign_Bl3tee/")
    elif type == "aucpufw":
        return urljoin(serverRootUrl, "Firmware/job/Sign_AUCPU_FW/")
    elif type == "vdecfw":
        return urljoin(serverRootUrl, "Firmware/job/Sign_VDEC_FW/")
    else:  # bl2e, bl2x, bl40
...
def submitSignBl3teeJob(
    ...
    bl31FileName = os.path.basename(bl31FilePath)
    bl32FileName = os.path.basename(bl32FilePath)
    bl3TeeFileName = os.path.basename(bl3TeeFilePath)

    if not os.path.isfile(bl31FilePath):
        bl31FilePath = 'null'

    if not os.path.isfile(bl32FilePath):
        bl32FilePath = 'null'

    if not os.path.isfile(bl3TeeFilePath):
        bl3TeeFilePath = 'null'

    if(bl31FilePath != 'null' and bl32FilePath != 'null' and bl3TeeFilePath != 'null'):
        uploadFile = {
            "bl31file": (bl31FileName, open(bl31FilePath, 'rb')),
            "bl32file": (bl32FileName, open(bl32FilePath, 'rb')),
            "bl3teefile": (bl3TeeFileName, open(bl3TeeFilePath, 'rb'))
        }
    elif(bl31FilePath != 'null' and bl32FilePath != 'null' and bl3TeeFilePath == 'null'):
        uploadFile = {
            "bl31file": (bl31FileName, open(bl31FilePath, 'rb')),
            "bl32file": (bl32FileName, open(bl32FilePath, 'rb'))
        }
        ...
        print("Invalid input file for bl3 tee signing")
...
def main():
    ...
    if args.type == "bl3tee":
        submitSignBl3teeJob(
            type=args.type,
            chipType=args.chip,
            bl31FilePath=args.bl31FilePath,
            bl32FilePath=args.bl32FilePath,
            bl3TeeFilePath=args.bl3TeeFilePath,

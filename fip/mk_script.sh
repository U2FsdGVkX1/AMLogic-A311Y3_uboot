...
function parse_bl33_global_config() {
	...
	for line in `cat ${SOURCE_FILE}`; do
		...
		if [[ "${line}" == "CONFIG_BL32_FASTBOOT"* ]] || [[ "${line}" == "CONFIG_BL32_128M"* ]] ; then
			tmp=${line%=*}	# delete =y
			BL33_GLOBAL_CONFIG="${BL33_GLOBAL_CONFIG}"" -D""${tmp}"
		fi
		if [[ "${line}" == "CONFIG_BYPASS_AOCPU"* ]] || [[ "${line}" == "CONFIG_BYPASS_BL32"* ]] ; then
			tmp=${line%=*}	# delete =y
			BL33_GLOBAL_CONFIG="${BL33_GLOBAL_CONFIG}"" -D""${tmp}"
		fi
...
function build_blx_src() {
	# compile $name $src_folder $bin_folder $soc $loop
	local name=$1
	local src_folder=$2
	local bin_folder=$3
	local soc=$4
	local times=$5
	#dbg "compile - name: ${name}, src_folder: ${src_folder}, bin_folder: ${bin_folder}, soc: ${soc} loop: $times"

	if [ $name == ${BLX_NAME_GLB[0]} ]; then
		# bl2
		build_bl2 $src_folder $bin_folder $soc $times
	elif [ $name == ${BLX_NAME_GLB[5]} ]; then
		# bl2e
		build_bl2e $src_folder $bin_folder $soc $times
...
# Checks if original filename should be kept when copying BIN_PATH to
# FIP_BUILD_FOLDER or if BLX_BIN_NAME should be used.
# s7d and s6 need to keep the original filename in some cases.
# $1: CUR_SOC
# $2: filename from BIN_PATH
# Returns 0 for true, 1 for false
function should_keep_filename() {
	local cur_soc="$1"
	local f="$2"
	case "$cur_soc" in
		s7d|s6|a9) ;;
	...
	# Simply check for suffix. Doesn't handle everything though.
	local pattern="(\.rsa|\.rsa-mldsa){2}$"
    if [[ $f =~ $pattern ]]; then
		return 0
	fi
	return 1
...
function build_blx() {
	...
	for loop in ${!BLX_NAME[@]}; do
		...
		if [ "null" == "${BIN_PATH[$loop]}" ]; then
			...
			build_blx_src ${BLX_NAME[$loop]} ${BLX_SRC_FOLDER[$loop]} ${FIP_BUILD_FOLDER} ${CUR_SOC} $loop
		else
			if [ ! -e ${BIN_PATH[$loop]} ]; then
				...
				if [ "y" == "${CONFIG_FIP_IMG_SUPPORT}" ] && \
				   ...
				   [ "NULL" != "${BLX_IMG_NAME[$loop]}" ]; then
					if should_keep_filename ${CUR_SOC} ${BIN_PATH[$loop]}; then
...
function clean() {
	if [ "${SKIP_BL33_CLEAN}" == "1" ]; then
		echo "Skip clean used"
		cd ${MAIN_FOLDER}
...
function build() {
	...
	if [ ! $CONFIG_AML_JUMP_BL33 ]; then
		CONFIG_AML_JUMP_BL33=null
	fi
	...
	build_uboot ${CONFIG_SYSTEM_AS_ROOT} ${CONFIG_AVB2} ${CONFIG_CMD_BOOTCTOL_VAB} ${CONFIG_FASTBOOT_WRITING_CMD} ${CONFIG_AVB2_RECOVERY} ${CONFIG_TESTKEY} ${CONFIG_AB_UPDATE} ${CONFIG_AML_GPT} ${CONFIG_AML_JUMP_BL33}
	...
	while [ $i -lt $# ]; do
		...
		case "$arg" in
			...
			--build-unsign)
				...
				continue ;;
			--skip-switchbranch)
				CONFIG_SKIP_SWITCHBRANCH=1
				export CONFIG_SKIP_SWITCHBRANCH
				continue ;;
			--nasc_nagra_tier_1)
			...
			--gpt)
				...
				continue ;;
			--jump_bl33)
				CONFIG_AML_JUMP_BL33=1
				export CONFIG_AML_JUMP_BL33=1
				continue ;;
			*)
		esac
	done

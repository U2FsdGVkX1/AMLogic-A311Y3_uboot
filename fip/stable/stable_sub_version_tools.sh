...
declare -a OPENLINUX_BOARD_TYPE=("ohm_wv4" "oppen_wv4" "tyson" "bluebell" "qurra" "ross" "raman")
...
declare -a OPENLINUX_BOARD_CONFIG=("sc2_ah212" "s4_ap222" "s5_ax201" "t7_an400_lpddr4x_bluebell" "s7_bh201" "s7d_bm201" "s6_bl201")
		...
		if [ -d $rootdir/${BLX_PATH[$i]} ]; then
			...
			if [ "${STABLE_VER_TOPIC[$i]}" != "NULL" ]; then
				...
				if [ "${STABLE_VER_TOPIC[$i]}" == "bl2-sc2" ];then
					...
					echo "PUSH bl2-s7d ==>>"
					BL2_SC2_LAST_COMMIT=`git log --pretty=format:"%h" | head -1  | awk '{print $1}'`
					git checkout ${BL2_S7D_BRANCH} > /dev/null
					git cherry-pick ${BL2_SC2_LAST_COMMIT}
...
function compile_stable_uboot_bin() {
	...
	for((i=0;i<${#OPENLINUX_BOARD_TYPE[@]};i++)); do
		...
		if [ -d ${ANDROID_DIR_LIST[$i]} ];then
			...
			git branch -D master
			git checkout -t trunk/master
			git branch -D $ANDROID_TARGET_BRANCH
			git checkout -t  trunk/$ANDROID_TARGET_BRANCH
			git branch -D master
			git fetch --all && git reset --hard && git pull
			cd -
		else
		...
		if [ -f "./build-${OPENLINUX_BOARD_TYPE[i]}-${PUSH_TOPIC_COMMON}/u-boot.bin.signed" ]; then
			...
			cd -
			runcmd="./mk ${OPENLINUX_BOARD_CONFIG[i]} --update-bl2 --update-bl2e --update-bl2x --update-bl31 --update-bl32 --avb2 --vab --fastboot-write"
			echo "[run cmd]: ${runcmd}"
...
local ${OPENLINUX_BOARD_TYPE[$i]}"
		...
		git push review HEAD:refs/for/${ANDROID_TARGET_BRANCH}%topic=${PUSH_TOPIC_COMMON} || err_exit " push bootloader img error."
		cd -

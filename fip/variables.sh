...
declare -a  COMPILE_PARA_LIST=("--h" "--help" "--config" "--chip-variant" "--check-compile" \
			...
			"--fastboot-write" "--signpipe" "--avb2-recovery" "--patch" "--gpt" "--jump_bl33" \
			"--display-pipeline" "--testkey" "--ipc-type" "--ipc-ddr-size" \
			"--ipc-tee-type" "--ddrfip-indep" "--skip-switchbranch"  "--skip-clean" \
			)
